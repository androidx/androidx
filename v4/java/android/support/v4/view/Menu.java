...
public interface Menu {
    ...
    /**
     * Category code for the order integer for items/groups that are part of a container -- or/add
     * this with your base value.
     */
    static final int CATEGORY_CONTAINER = 0x00010000;

    /**
     * Category code for the order integer for items/groups that are provided by the system --
     * or/add this with your base value.
     */
    static final int CATEGORY_SYSTEM = 0x00020000;

    /**
     * Category code for the order integer for items/groups that are user-supplied secondary
     * (infrequently used) options -- or/add this with your base value.
     */
    static final int CATEGORY_SECONDARY = 0x00030000;

    /**
     * Category code for the order integer for items/groups that are alternative actions on the data
     * that is currently displayed -- or/add this with your base value.
     */
    static final int CATEGORY_ALTERNATIVE = 0x00040000;

    /**
     * Flag for {@link #addIntentOptions}: if set, do not automatically remove any existing menu
     * items in the same group.
     */
    static final int FLAG_APPEND_TO_GROUP = 0x0001;

    /**
     * Flag for {@link #performShortcut}: if set, do not close the menu after executing the
     * shortcut.
     */
    static final int FLAG_PERFORM_NO_CLOSE = 0x0001;

    /**
     * Flag for {@link #performShortcut(int, KeyEvent, int)}: if set, always close the menu after
     * executing the shortcut. Closing the menu also resets the prepared state.
     */
     ...
     * Add a new item to the menu. This item displays the given title for its label.
     *
     * @param groupId The group identifier that this item should be part of. This can be used to
     *                define groups of items for batch state changes. Normally use {@link #NONE} if
     *                an item should not be in a group.
     * @param itemId  Unique item ID. Use {@link #NONE} if you do not need a unique ID.
     * @param order   The order for the item. Use {@link #NONE} if you do not care about the order.
     ...
     * identifier instead of the string itself.
     *
     * @param groupId  The group identifier that this item should be part of. This can also be used
     *                 to define groups of items for batch state changes. Normally use {@link #NONE}
    ...
    /**
     * Add a new sub-menu to the menu. This item displays the given title for its label. To modify
     * other attributes on the submenu's menu item, use {@link SubMenu#getItem()}.
     *
     * @param title The text to display for the item.
    ...
    /**
     * Add a new sub-menu to the menu. This item displays the given <var>title</var> for its label.
     * To modify other attributes on the submenu's menu item, use {@link SubMenu#getItem()}. <p/>
     * Note that you can only have one level of sub-menus, i.e. you cannnot add a subMenu to a
     * subMenu: An {@link UnsupportedOperationException} will be thrown if you try.
     *
     * @param groupId The group identifier that this item should be part of. This can also be used
     *                to define groups of items for batch state changes. Normally use {@link #NONE}
     *                if an item should not be in a group.
    ...
    /**
     * Variation on {@link #addSubMenu(int, int, int, CharSequence)} that takes a string resource
     ...
     * <p>See {@link android.content.pm.PackageManager#queryIntentActivityOptions}
     ...
     *
     * <p>All of the menu items of possible options for the intent will be added
     ...
     *
     * @param groupId          The group identifier that the items should be part of. This can also
     *                         be used to define groups of items for batch state changes. Normally
     *                         use {@link #NONE} if the items should not be in a group.
     * @param itemId           Unique item ID. Use {@link #NONE} if you do not need a unique ID.
     * @param order            The order for the items. Use {@link #NONE} if you do not care about
     *                         the order. See {@link MenuItem#getOrder()}.
     * @param caller           The current activity component name as defined by
     *                         queryIntentActivityOptions().
     * @param specifics        Specific items to place first as defined by queryIntentActivityOptions().
     * @param intent           Intent describing the kinds of items to populate in the list as
     *                         defined by queryIntentActivityOptions().
     * @param flags            Additional options controlling how the items are added.
     * @param outSpecificItems Optional array in which to place the menu items that were generated
     *                         for each of the <var>specifics</var> that were requested. Entries may
     *                         be null if no activity was found for that specific action.
     ...
     *
     * @param id The item to be removed.  If there is no item with this identifier, nothing
     *           happens.
     */
     ...
     *
     * @param groupId The group to be removed.  If there are no items in this group, nothing
     *                happens.
     */
    ...
    /**
     * Control whether a particular group of items can show a check mark.  This is similar to
     * calling {@link MenuItem#setCheckable} on all of the menu items with the given group
     * identifier, but in addition you can control whether this group contains a mutually-exclusive
     * set items.  This should be called after the items of the group have been added to the menu.
     *
     * @param group     The group of items to operate on.
     * @param checkable Set to true to allow a check mark, false to disallow.  The default is
     *                  false.
     * @param exclusive If set to true, only one item in this group can be checked at a time;
     *                  checking an item will automatically uncheck all others in the group.  If set
     *                  to false, each item can be checked independently of the others.
    ...
    /**
     * Get the number of items in the menu.  Note that this will change any times items are added or
    ...
    /**
     * Control whether the menu should be running in qwerty mode (alphabetic shortcuts) or 12-key
     * mode (numeric shortcuts).
     *
     * @param isQwerty If true the menu will use alphabetic shortcuts; else it will use numeric
     *                 shortcuts.
     */
