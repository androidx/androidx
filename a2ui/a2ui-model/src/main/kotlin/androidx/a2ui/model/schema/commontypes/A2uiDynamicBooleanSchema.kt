 ...
 */
public class A2uiDynamicBooleanSchema(public override val description: String? = null) :
    A2uiCompositeSchema() {
    override val definitionName: String = "DynamicBoolean"
    override val schemaId: String = SCHEMA_ID_COMMON_TYPES

    public override fun getDefinition(): A2uiSchema =
        createDynamicTypeSchema(
            literalSchema = A2uiBooleanSchema(),
            returnType = FunctionReturnType.BOOLEAN,
            description =
                "A boolean value that can be a literal, a path to a boolean in the data model, or a function call returning a boolean.",
        )

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is A2uiDynamicBooleanSchema) return false
    ...
    override fun toString(): String {
        return "A2uiDynamicBooleanSchema(description=$description)"
    }

    public companion object {
        @JvmField public val DEFAULT_INSTANCE: A2uiDynamicBooleanSchema = A2uiDynamicBooleanSchema()
    }
}
