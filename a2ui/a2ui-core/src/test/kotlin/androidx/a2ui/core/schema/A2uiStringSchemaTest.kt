...
class A2uiStringSchemaTest {
    @Test
    fun toJsonSchema_withoutDescription_returnsCorrectJsonObject() {
        assertThat(Json.parseToJsonElement(A2uiStringSchema().toJsonSchema()))
            .isEqualTo(buildJsonObject { put(TYPE_FIELD, STRING_TYPE) })
    }

    @Test
    fun toJsonSchema_withDescription_returnsCorrectJsonObject() {
        assertThat(Json.parseToJsonElement(A2uiStringSchema(TEST_DESCRIPTION_1).toJsonSchema()))
            .isEqualTo(
                buildJsonObject {
    ...
    fun toString_withoutDescription_returnsExpectedFormat() {
        val schema = A2uiStringSchema(null)
        assertThat(schema.toString()).isEqualTo("String(description=null)")
    }
}
