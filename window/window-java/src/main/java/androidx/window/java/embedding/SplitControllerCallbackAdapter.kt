...
/**
 * An adapted interface for [SplitController] that provides callback shaped APIs to report the
 * latest split information with [SplitInfo] list.
 *
...
private constructor(
     ...
     * of. An activity can be in zero, one or more active splits. More than one active split is
     * possible if an activity created multiple containers to side, stacked on top of each other. Or
     * it can be in two different splits at the same time - in a secondary container for one (it was
     * launched to the side) and in the primary for another (it launched another activity to the
     * side). The reported splits in the list are ordered from bottom to top by their z-order, more
     * recent splits appearing later. Guaranteed to be called at least once to report the most
     * recent state.
     *
