...
@JvmName("map")
@CheckResult
public fun <T : Any, R : Any> PagingData<T>.mapAsync(transform: (T) -> Single<R>): PagingData<R> =
    map {
        transform(it).await()
    }
...
@JvmName("filter")
@CheckResult
public fun <T : Any> PagingData<T>.filterAsync(predicate: (T) -> Single<Boolean>): PagingData<T> =
    filter {
        predicate(it).await()
    }
