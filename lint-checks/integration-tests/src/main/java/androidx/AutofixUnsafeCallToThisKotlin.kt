...
@Suppress("unused")
abstract class AutofixUnsafeCallToThisKotlin : ViewGroup(null) {
    /** Method making the unsafe reference on an implicit this. */
    fun unsafeReferenceOnImplicitThis() {
    ...
    fun unsafeReferenceOnExplicitThis() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
    ...
    fun unsafeReferenceOnExplicitSuper() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
