...
abstract class VerifyPlaygroundGradleConfigurationTask : DefaultTask() {
    ...
    private fun compareProperties(): List<String> {
        ...
        return validateProperties(rootProperties, playgroundProperties)
    }
    ...
    private fun validateProperties(
        ...
        return propertyKeys.mapNotNull { key ->
            val rootValue = rootProperties[key]
            val playgroundValue = playgroundProperties[key]
            if (
                (exceptedProperties.containsKey(key) &&
                    exceptedProperties[key] != playgroundValue) ||
                    (rootValue != playgroundValue && !ignoredProperties.contains(key))
            ) {
