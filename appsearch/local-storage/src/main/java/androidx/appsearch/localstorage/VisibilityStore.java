...
/**
 * Manages any visibility settings for all the package's databases that AppSearchImpl knows
 * about. Persists the visibility settings and reloads them on initialization.
 *
...
class VisibilityStore {
    ...
    private static final AppSearchSchema PACKAGE_ACCESSIBLE_SCHEMA =
            new AppSearchSchema.Builder(PACKAGE_ACCESSIBLE_TYPE)
                    .addProperty(new AppSearchSchema.PropertyConfig.Builder(PACKAGE_NAME_PROPERTY)
                            .setCardinality(AppSearchSchema.PropertyConfig.CARDINALITY_OPTIONAL)
                            .setDataType(AppSearchSchema.PropertyConfig.DATA_TYPE_STRING)
                            .build())
                    .addProperty(new AppSearchSchema.PropertyConfig.Builder(SHA_256_CERT_PROPERTY)
                            .setCardinality(AppSearchSchema.PropertyConfig.CARDINALITY_OPTIONAL)
     ...
     */
    static final String VISIBILITY_STORE_PREFIX = AppSearchImpl.createPrefix(PACKAGE_NAME,
            DATABASE_NAME);
     ...
     *
     * Although the prefix key isn't used for lookup, it's helpful in ensuring that all previous
     ...
     */
    VisibilityStore(@NonNull AppSearchImpl appSearchImpl, @NonNull Context context,
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            ...
            mGlobalQuerierPackageUid = Api24Impl.getGlobalQuerierPackageUid(context,
     ...
     * {@link AppSearchImpl#setSchema} will call {@link #setVisibility}. We
     * need to have both AppSearchImpl and VisibilityStore fully initialized for this call flow
     * to work.
     *
    ...
    public void initialize() throws AppSearchException {
        if (!mAppSearchImpl.hasSchemaTypeLocked(PACKAGE_NAME, DATABASE_NAME, VISIBILITY_TYPE)
                || !mAppSearchImpl.hasSchemaTypeLocked(PACKAGE_NAME, DATABASE_NAME,
                ...
                if (packageAccessibleDocuments != null) {
                    for (int i = 0; i < packageAccessibleDocuments.length; i++) {
                        ...
                        Set<PackageIdentifier> packageIdentifiers =
                                schemaToPackageIdentifierMap.get(
                                        prefixedSchema);
     ...
     * @param schemasPackageAccessible      Map of prefixed schemas to a list of package
     *                                      identifiers that have access to the schema.
    ...
    public void setVisibility(@NonNull String prefix,
        ...
        GenericDocument.Builder visibilityDocument = new GenericDocument.Builder(
                /*uri=*/ addUriPrefix(prefix), VISIBILITY_TYPE)
            ...
            for (int i = 0; i < entry.getValue().size(); i++) {
                ...
                GenericDocument packageAccessibleDocument =
                        new GenericDocument.Builder(/*uri=*/"placeholder", PACKAGE_ACCESSIBLE_TYPE)
                                .setNamespace(NAMESPACE)
                                .setPropertyString(PACKAGE_NAME_PROPERTY,
        ...
        if (!packageAccessibleDocuments.isEmpty()) {
            visibilityDocument.setPropertyDocument(PACKAGE_ACCESSIBLE_PROPERTY,
    ...
    public boolean isSchemaSearchableByCaller(@NonNull String prefix,
        ...
        if (callerUid == mGlobalQuerierPackageUid && isSchemaPlatformSurfaceable(prefix,
                prefixedSchema)) {
            return true;
        }
     ...
     */
    private boolean isSchemaPlatformSurfaceable(@NonNull String prefix,
    ...
    /**
     * Returns whether the schema is accessible by the {@code callerUid}. Checks that the
     * callerUid has one of the allowed PackageIdentifier's package. And if so, that the package
     * also has the matching certificate.
     *
     * <p>This supports packages that have certificate rotation. As long as the specified
     * certificate was once used to sign the package, the package will still be granted access.
     * This does not handle packages that have been signed by multiple certificates.
     */
    private boolean isSchemaPackageAccessible(@NonNull String prefix,
            @NonNull String prefixedSchema, int callerUid) {
        ...
        for (PackageIdentifier packageIdentifier : packageIdentifiers) {
            ...
            if (Api28Impl.hasSigningCertificate(mContext, packageIdentifier.getPackageName(),
     ...
     *
     * <p>Even though wrapping a call to a method from an API above minSdk inside an SDK_INT
     * check makes it runtime safe, it is not optimal. When ART tries to optimize a class, it
     * will do so regardless of the execution path, and will fail if it tries to resolve a method
     * at a higher API if that method is being referenced somewhere in the class, even if that
     * method would never be called at runtime due to the SDK_INT check. ART will however only
     * try to optimize a class the first time it's referenced at runtime, this means if we wrap
     * our above minSdk method calls inside classes that are only referenced at runtime at the
     * appropriate API level, then we guarantee the ability to resolve all the methods.
     */
    @RequiresApi(24)
    private static class Api24Impl {
         ...
         */
        static int getGlobalQuerierPackageUid(@NonNull Context context,
                @NonNull String globalQuerierPackage) {
            try {
    ...
    private static class Api28Impl {
         ...
         */
        static boolean hasSigningCertificate(@NonNull Context context,
                @NonNull String packageName, @NonNull byte[] sha256Certificate) {
            return context.getPackageManager().hasSigningCertificate(packageName, sha256Certificate,
