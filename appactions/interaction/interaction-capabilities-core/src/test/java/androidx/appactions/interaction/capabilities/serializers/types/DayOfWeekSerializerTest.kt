...
class DayOfWeekSerializerTest {
  ...
  fun DayOfWeek_conversion_success() {
    val MONDAY_VALUE = Value.newBuilder().setStructValue(
      Struct.newBuilder()
      .putFields("@type", Value.newBuilder().setStringValue("DayOfWeek").build())
      .putFields(
        "canonicalUrl",
        Value.newBuilder().setStringValue(DayOfWeek.MONDAY.canonicalUrl).build()
      )
    ).build()
