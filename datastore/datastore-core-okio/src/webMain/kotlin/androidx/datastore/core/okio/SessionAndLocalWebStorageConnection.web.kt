...
internal class SessionAndLocalWebStorageConnection<T>(
    ...
    override suspend fun <R> readScope(block: suspend ReadScope<T>.(locked: Boolean) -> R): R {
        ...
        return coordinator.tryLock { locked ->
            SessionAndLocalWebReadScope(domStorage, name, serializer).use { block(it, locked) }
        }
    }
    ...
    override suspend fun writeScope(block: suspend WriteScope<T>.() -> Unit) {
        checkNotClosed()
        coordinator.lock {
            SessionAndLocalWebWriteScope(domStorage, name, serializer).use { block(it) }
        }
    }
...
internal open class SessionAndLocalWebReadScope<T>(
    private val domStorage: DomStorage,
    private val name: String,
    private val serializer: OkioSerializer<T>,
) : ReadScope<T> {
...
internal class SessionAndLocalWebWriteScope<T>(
    private val domStorage: DomStorage,
    private val name: String,
    private val serializer: OkioSerializer<T>,
) : SessionAndLocalWebReadScope<T>(domStorage, name, serializer), WriteScope<T> {
