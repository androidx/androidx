...
actual constructor(
    ...
    private var cachedFileHandle: JsFileSystemFileHandle? = null
    ...
    internal suspend fun getFileHandle(): JsFileSystemFileHandle {
        if (cachedFileHandle == null) {
            val root = getJsOpfsRoot().await<JsFileSystemDirectoryHandle>()
            cachedFileHandle =
                root.getFileHandle(name, createCreateOptions()).await<JsFileSystemFileHandle>()
        }
...
internal actual open class OpfsWebReadScope<T>(
    ...
    private val getFileHandle: suspend () -> JsFileSystemFileHandle,
) : ReadScope<T> {
    private val closed = AtomicBoolean(false)
...
internal actual class OpfsWebWriteScope<T>(
    ...
    actual override suspend fun writeData(value: T) {
        checkClose()
        val fileHandle = getFileHandle()
        val stream = fileHandle.createWritable().await<JsFileSystemWritableFileStream>()
        try {
            val buffer = Buffer()
