...
class SimpleActorTest {
    ...
    @get:Rule val timeout = Timeout(3, TimeUnit.MINUTES)
    ...
    fun testSimpleActor() =
        runTest(UnconfinedTestDispatcher()) {
            ...
            val actor =
                SimpleActor<Int>(this, onComplete = {}, onUndeliveredElement = { _, _ -> }) {
                    msgs.add(it)
                }
    ...
    @Test
    fun testOnCompleteIsCalledWhenScopeIsCancelled() =
        runBlocking<Unit> {
            ...
            try {
                withTimeout(5.seconds) { called.await() }
    ...
    fun testManyConcurrentCalls() =
        runBlocking<Unit> {
            ...
            val actor =
                SimpleActor<Int>(scope, onComplete = {}, onUndeliveredElement = { _, _ -> }) {
            ...
            repeat(numCalls) { scope.launch { actor.offer(it) } }
    ...
    @Test
    fun testManyConcurrentCalls_withDelayBeforeSettingValue() =
        runBlocking<Unit> {
            val scope =
                CoroutineScope(Job() + Executors.newFixedThreadPool(4).asCoroutineDispatcher())
            val numCalls = 500
    ...
    @Test
    fun testOnUndeliveredElementsCallback() =
        runBlocking<Unit> {
            val scope = CoroutineScope(Job())
            ...
            deferreds.forEach { assertThrows<CancellationException> { it.await() } }
        }

    @Test
    fun testAllMessagesAreConsumedIfOfferSucceeds() =
        runBlocking<Unit> {
    ...
    @Test
    fun testAllMessagesAreRespondedTo() =
        runBlocking<Unit> {
            ...
            val waiters =
                (0 until 10_000).map {
                    myScope.async {
                        try {
                            CompletableDeferred<Unit?>().also { actor.offer(it) }.await()
