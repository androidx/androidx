...
class PackageManagerHelperTest {
    ...
    @Test
    fun testNullComponentInfoFromMock_assumesDisabled() {
        val mockPm = mock(PackageManager::class.java)
        val testContext =
            object : ContextWrapper(context) {
                override fun getPackageManager(): PackageManager = mockPm
            }
        val componentName = ComponentName(testContext, TestReceiver::class.java)
        `when`(mockPm.getComponentEnabledSetting(eq(componentName)))
            .thenReturn(PackageManager.COMPONENT_ENABLED_STATE_DEFAULT)

        setReceiverEnabled(testContext, TestReceiver::class.java, true)
