...
/**
 * A factory object that creates [ListenableWorker] instances. The factory is invoked every time a
 * work runs. You can override the default implementation of this factory by manually initializing
...
abstract class WorkerFactory {
     ...
     *
     * Throwing an [Exception] here and no [ListenableWorker] will be created. If a [WorkerFactory]
     * is unable to create an instance of the [ListenableWorker], it should return `null` so it can
     * delegate to the default [WorkerFactory].
     *
     * Returns a new instance of the specified `workerClassName` given the arguments. The returned
     * worker must be a newly-created instance and must not have been previously returned or invoked
     * by WorkManager. Otherwise, WorkManager will throw an [IllegalStateException].
     *
     ...
     * is found, default reflection-based code will be used to instantiate the worker with the
     * current ClassLoader. The returned worker should be a newly-created instance and must not have
    ...
    fun createWorkerWithDefaultFallback(
        ...
        if (worker.isUsed) {
            val message =
                "WorkerFactory (${javaClass.name}) returned an instance of" +
