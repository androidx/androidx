...
/**
 * Returns a new [MutableScatterSet] with the specified contents, given as
 * a list of pairs where the first component is the key and the second
 * is the value. If multiple pairs have the same key, the resulting set
 ...
 * accommodate newly added entries to the table.
 *
 ...
 * mutable implementation is provided by [MutableScatterSet].
 *
...
public sealed class ScatterSet<E> {
    ...
    /**
     * Returns the number of key-value pairs that can be stored in this set
    ...
    /**
     * Returns `true` if this set has at least one entry.
     */
    ...
    public inline fun first(): E {
        forEach { return it }
        throw NoSuchElementException("The FlatHashSet is empty")
    }
    ...
    /**
     * Iterates over every key/value pair stored in this set by invoking
     ...
     * [block] lambda.
     * @param block called for each element in the set with a sequential index and the element.
     */
    public inline fun forEachIndexed(block: (index: Int, element: E) -> Unit) {
        ...
        forEachIndex { index ->
            @Suppress("UNCHECKED_CAST")
            block(i++, k[index] as E)
        }
    }

    /**
     * Returns true if all entries match the given [predicate]. If there are no elements in the
     ...
     * - Contains key/value pairs equal to this set's pair
     */
     ...
     */
    internal inline fun findElementIndex(key: E): Int {
        val hash = hash(key)
        val hash2 = h2(hash)
...
public class MutableScatterSet<E>(
     ...
     * @return `true` if the set was changed or `false` if none of the values were present.
     */
    ...
    /**
     * Scans the set to find the index at which we can store a value
     * for the given [element]. If the key already exists in the set, its index
