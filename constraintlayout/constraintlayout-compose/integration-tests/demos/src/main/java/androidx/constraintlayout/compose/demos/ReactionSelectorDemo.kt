...
fun ReactionSelectorDemo() {
    ...
    val scene = MotionScene {
        ...
        val start1 = constraintSet {
            ...
            emojiIds.forEach { constrain(it) { top.linkTo(parent.top, 10.dp) } }
    ...
    LaunchedEffect(Unit) {
        selectedFlow.collect {
            ...
            progress.animateTo(targetValue = 1f, animationSpec = tween(800))
        }
    }
    ...
    Column {
        MotionLayout(
            ...
            emojiNames.forEach { name ->
                Text(text = name, color = Color.White, modifier = Modifier.layoutId(name))
            }
        }
    }
}
