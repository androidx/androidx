...
/**
 * Interface for accessing various EGL facilities independent of EGL versions. That is each EGL
 * version implements this specification.
 *
...
interface EGLSpec {
     ...
     * separated list of the capabilities.
     *
     ...
     * attributes are defined as part of the OpenGL specification here:
     ...
     *
     * If the EGL surface is not current to any thread, eglDestroySurface destroys it immediately.
     * Otherwise, surface is destroyed when it becomes not current to any thread. Furthermore,
     * resources associated with a pbuffer surface are not released until all color buffers of that
     * pbuffer bound to a texture object have been released. Deferral of surface destruction would
     * still return true as deferral does not indicate a failure condition
     *
     ...
     * not be loaded, null is returned
     *
     ...
     * context, EGL_NO_CONTEXT is returned
     *
     ...
     * is set to EGL_SUCCESS. When an EGL function could potentially generate several different
     * errors (for example, when passed both a bad attribute name, and a bad attribute value for a
     * legal attribute name), the implementation may choose to generate any one of the applicable
     * errors.
     *
     ...
     * EGLClientBuffer and an [EGLImageKHR] from the client buffer.
     *
     * When this [EGLImageKHR] instance is no longer necessary, consumers should be sure to call the
     * corresponding method [eglDestroyImageKHR] to deallocate the resource.
     *
     ...
     *
     * This can return null if the EGL_ANDROID_image_native_buffer and EGL_KHR_image_base extensions
     * are not supported or if allocation of the buffer fails.
     *
     ...
     * any additional [EGLImageKHR] target resources within any client API contexts, although
     * existing [EGLImageKHR] siblings may continue to be used. `true` is returned if
     ...
     * returns a handle to the new object. The configuration of the returned [EGLSyncKHR] object is
     * specified by the provided attributes.
     *
     ...
     * of sync object. If no errors are generated, this returns true and the value of the queried
     * attribute is stored in the value array at the offset position. If this method returns false,
     * the provided value array is unmodified.
     *
     * See: https://www.khronos.org/registry/EGL/extensions/KHR/EGL_KHR_fence_sync.txt
     *
     ...
     * outstanding on the same [sync] at any given time. When there are multiple threads blocked on
     * the same [sync] and the [sync] object has signalled, all such threads are released, but the
     * order in which they are released is not defined.
     *
     * If the value of [timeoutNanos] is zero, then [eglClientWaitSyncKHR] simply tests the current
     * status of sync. If the value of [timeoutNanos] is the special value [EGL_FOREVER_KHR], then
     * [eglClientWaitSyncKHR] does not time out. For all other values, [timeoutNanos] is adjusted to
     * the closest value allowed by the implementation-dependent timeout accuracy, which may be
     * substantially longer than one nanosecond.
     *
     * [eglClientWaitSyncKHR] returns one of three status values describing the reason for
     * returning. A return value of [EGL_TIMEOUT_EXPIRED_KHR] indicates that the specified timeout
     * period expired before [sync] was signalled, or if [timeoutNanos] is zero, indicates that
     * [sync] is not signaled. A return value of [EGL_CONDITION_SATISFIED_KHR] indicates that [sync]
     * was signaled before the timeout expired, which includes the case when [sync] was already
     * signaled when [eglClientWaitSyncKHR] was called. If an error occurs then an error is
     ...
     * then [eglClientWaitSyncKHR] may wait forever. To help prevent this behavior, if the
     * [EGL_SYNC_FLUSH_COMMANDS_BIT_KHR] is set on the flags parameter and the [sync] is unsignaled
     * when [eglClientWaitSyncKHR] is called, then the equivalent flush will be performed for the
     * current EGL context before blocking on sync. If no context is current bound for the API, the
     * [EGL_SYNC_FLUSH_COMMANDS_BIT_KHR] bit is ignored.
     *
     * @param sync EGLSyncKHR object to wait on
     * @param flags Optional flags to provide to handle flushing of pending commands
     * @param timeoutNanos Optional timeout value to wait before this method returns, measured in
     *   nanoseconds. This value is always consumed as an unsigned long value so even negative
    ...
    companion object {
        ...
        val V14 =
            object : EGLSpec {
                ...
                private val contextAttributes =
                    intArrayOf(
                        EGL14.EGL_CONTEXT_CLIENT_VERSION,
                        2, // GLES VERSION 2
                        // HWUI provides the ability to configure a context priority as well but
                        // that only
                ...
                override fun eglInitialize(): EGLVersion {
                    // eglInitialize is destructive so create 2 separate arrays to store the major
                    // and
                    // minor version
        ...
        /**
         * Return a string representation of the corresponding EGL status code. If the provided
