...
/**
 * For some devices, setting [HardwareRenderer.isOpaque] to true with [ImageReader.getMaxImages]
 * set to 1 will end up preserving the contents of the buffers across renders. For low latency
 * rendering this is ideal in order to only draw the deltas of content across render requests.
 * However, not all devices support this optimization and will clear the contents regardless before
 * each render request.
 * This class is used to verify if the device does support preservation of contents across renders
 * and can be used to signal for a fallback solution that will re-render the scene before proceeding
 * for consistency.
 */
...
internal class PreservedBufferContentsVerifier {
     ...
     *
     * If this device **does not** support preserving content, then the left hand side will be
     * red as the red pixels would be rendered underneath transparent content if the buffer was
     * cleared in advance.
     */
    fun supportsPreservedRenderedContent(): Boolean {
        ...
        return if (hardwareBitmap != null) {
            ...
            val result = copyBitmap.getPixel(0, 0) == Color.GREEN &&
