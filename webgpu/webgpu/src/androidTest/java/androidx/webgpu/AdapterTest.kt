...
class AdapterTest {
    ...
    /**
     * Helper map for default limit values as defined by the WebGPU spec. A full implementation
     * would have all limits.
     */
    companion object {
        private val kDefaultLimits =
            mapOf(
    ...
    /**
     * Verifies that requesting a device with default parameters returns a device with Core features
     * enabled.
     */
    @Test
    ...
    /**
     * Verifies that requesting a device with default parameters returns a device with the correct,
     * specification-defined default limits.
     */
    @Test
     ...
     *
     * @see <a
     *   href="https://github.com/gpuweb/cts/blob/main/src/webgpu/api/operation/adapter/requestAdapter.spec.ts">WebGPU
     *   CTS Test</a>
     */
    @Test
    ...
    fun requestDeviceWithWorseThanDefaultLimitClamps() {
        ...
        val device = runBlocking {
            requestTestDevice(limits = GPULimits(maxBindGroups = worseLimit))
        }
    ...
    fun requestDeviceWithInvalidAlignmentLimitFails() {
        ...
        assertThrows(
            "Alignment limit not a power of 2 should fail",
            DeviceLostException::class.java,
        ) {
            runBlocking {
                requestTestDevice(
                    limits = GPULimits(minUniformBufferOffsetAlignment = invalidAlignment)
                )
            }
        }
    }
