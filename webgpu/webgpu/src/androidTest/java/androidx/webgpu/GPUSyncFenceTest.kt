...
class GPUSyncFenceTest {
                ...
                GPUInstanceDescriptor().apply {
                    dawnTogglesDescriptor =
                        GPUDawnTogglesDescriptor(
                            enabledToggles =
                                arrayOf("allow_unsafe_apis") // Required to enable experimental
                            // SharedTextureMemoryAHardwareBuffer features
                        )
                }
            )
        val adapter = instance.requestAdapter()
        val hasRequiredFeatures =
            with(adapter) {
            ...
            createWebGpu(
                ...
                deviceDescriptor =
                    GPUDeviceDescriptor(
                        ...
                        dawnTogglesDescriptor =
                            GPUDawnTogglesDescriptor(enabledToggles = arrayOf("allow_unsafe_apis")),
                        requiredFeatures =
                            intArrayOf(
    ...
    fun testSyncFence_lifecycleAndAwaiting() = runBlocking {
        val unused = webGpu.execute {
            ...
            try {
                ...
                val nextFence =
                    requireNotNull(wrapper.endAccess()) { "Sync fence should not be null" }
    ...
    fun testSyncFence_fromPlatformSyncFence() = runBlocking {
        val unused = webGpu.execute {
            ...
            try {
                ...
                val platformSyncFence =
                    requireNotNull(fence.toSyncFence()) { "Platform SyncFence should not be null" }

                // Await signaling of the platform SyncFence
                val signaled =
                    platformSyncFence.await(java.time.Duration.ofMillis(FENCE_TIMEOUT_MS))
                assertTrue(signaled)
    ...
    private fun createTestTextureWrapper(): Pair<HardwareBuffer, GPUHardwareBufferTexture> {
        ...
        val wrapper =
            GPUAndroidHardwareBufferUtil.createTexture(
                device,
                rgbBuffer,
