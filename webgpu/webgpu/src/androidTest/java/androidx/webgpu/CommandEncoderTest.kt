...
class CommandEncoderTest {
     ...
     *
     * This test uses an error scope to assert that no validation errors occur when pushing and
     * immediately popping a debug group on a command encoder.
     */
    @Test
    fun testDebugGroups() {
        val encoder = device.createCommandEncoder()
     ...
     *
     * This confirms that attempting to begin a second pass before the first has ended invalidates
     * the encoder, causing a validation error upon calling `finish()`.
     */
    @Test
     ...
     *
     * This test provides a valid `GPURenderPassDescriptor` and asserts that the returned encoder is
     * not null, confirming the command's successful initiation.
     */
    @Test
    ...
    /**
     * Verifies that a render pass with `LoadOp.Clear` correctly clears a texture to a specific
     * color.
     */
    @Test
    ...
    /**
     * Verifies that the `clearBuffer` command can be successfully encoded without validation
     * errors.
     *
     * This test pushes a validation error scope, encodes the `clearBuffer` command for a valid
     * buffer and range, and then pops the error scope to assert that the operation was valid.
     */
    @Test
    fun testClearBuffer() {
        val buffer =
     ...
     *
     * This test creates a `QuerySet` and a destination buffer, then encodes a command to resolve
     * the query results into the buffer. It uses an error scope to assert that the command is
     * considered a valid operation by the encoder.
     */
    @Test
