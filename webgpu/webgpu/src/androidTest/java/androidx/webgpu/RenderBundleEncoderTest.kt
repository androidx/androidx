...
class RenderBundleEncoderTest {
    ...
    private fun createIndirectBuffer(data: IntArray): GPUBuffer {
        val byteBuffer =
            ByteBuffer.allocateDirect(data.size * Int.SIZE_BYTES).order(ByteOrder.nativeOrder())
                ...
                val bgPipeline =
                    device.createRenderPipeline(
                        GPURenderPipelineDescriptor(
                            ...
                            primitive =
                                GPUPrimitiveState(topology = PrimitiveTopology.TriangleList),
                        )
                    )
    ...
    /**
     * Verifies that recording draw commands in a RenderBundle succeeds when a required BindGroup is
     * correctly set.
     */
    @MediumTest
    @Test
    fun testDrawWithBindGroupSetSucceeds() {
        runBlocking {
            val unused = webGpu.execute {
                ...
                val buffer =
                    device.createBuffer(GPUBufferDescriptor(size = 4, usage = BufferUsage.Uniform))
                ...
                // Create the actual bind group, linking the buffer to binding 0 according to the
                // layout.
                val bindGroup =
