...
class QuerySetTest {
    ...
    private fun createResolveBuffer(size: Long): GPUBuffer {
        return device.createBuffer(
            GPUBufferDescriptor(
                size = size,
                usage =
                    BufferUsage.QueryResolve or
                        BufferUsage.CopySrc, // CopySrc for potential readback.
            )
        )
    }
    ...
    fun testResolveQuerySetValid() {
        runBlocking {
            val unused = webGpu.execute {
                ...
                val destinationBuffer =
                    createResolveBuffer((QUERY_COUNT * Long.SIZE_BYTES).toLong())
    ...
    fun testResolveQuerySetInvalidDestinationUsage() {
        runBlocking {
            val unused = webGpu.execute {
                ...
                val invalidBuffer =
                    device.createBuffer(GPUBufferDescriptor(size = 8, usage = BufferUsage.CopySrc))
    ...
    /**
     * Helper function to execute a render pass, resolve an occlusion query, and read the result
     * back from the GPU.
     *
    ...
    private suspend fun executeQueryResolveTest(
        ...
        val shaderModuleFrag =
            device.createShaderModule(
                GPUShaderModuleDescriptor(
                    shaderSourceWGSL = GPUShaderSourceWGSL(fragmentShaderCode)
                )
            )
