...
class GPUHardwareBufferTest {
    ...
    private fun getCenterPixelColor(hardwareBuffer: HardwareBuffer): PixelColor {
        ...
        try {
            ...
            try {
                val centerPixel =
                    softwareBitmap.getPixel(softwareBitmap.width / 2, softwareBitmap.height / 2)
                return PixelColor(
    ...
    private fun verifyColorSpaceConversion(
        ...
        sampler: GPUSampler,
        srcBuffer: HardwareBuffer,
        dstBuffer: HardwareBuffer,
        dstWrapper: GPUHardwareBufferTexture,
        width: Int,
        height: Int,
        primary: Int,
        transfer: Int,
        ...
        val extDescriptor =
            createExternalTextureDescriptor(
                cropWidth = width,
                cropHeight = height,
                primary = primary,
                transfer = transfer,
        ...
        val srcExtWrapper =
            GPUAndroidHardwareBufferUtil.createExternalTexture(
                device,
                srcBuffer,
                extDescriptor,
            )
                        ...
                        arrayOf(
                            ...
                            GPUBindGroupEntry(
                                ...
                                externalTextureBindingEntry =
                                    GPUExternalTextureBindingEntry(srcExtWrapper.externalTexture),
                            ),
                        ),
                )
            )
            ...
            val unused = webGpu.execute {
                ...
                val descriptor =
                    GPUTextureDescriptor(
                        size =
                            GPUExtent3D(width = 1000000, height = 1000000, depthOrArrayLayers = 1),
                ...
                List(numThreads) {
                    launch {
                        for (i in 0 until 20) {
                            ...
                            // Gate after every single iteration to guarantee threads strictly
                            // interleave
                            barrier.await()
                        }
                    }
    ...
    /**
     * Verifies GPU-to-CPU zero-copy memory visibility after rendering into an imported
     * HardwareBuffer. Crashes with Vulkan image layout transition mismatch if GPUImportedAHB does
     * not use VK_IMAGE_LAYOUT_GENERAL.
     */
    @Test
    @MediumTest
    @ApiRequirement(minApi = 29, onlySkipOnEmulator = true)
    ...
    /**
     * Verifies YUV external texture descriptors are parsed correctly across the JNI boundary
     * without error.
     */
    @Test
    @MediumTest
            ...
            val unused = webGpu.execute {
                ...
                val extDescriptor =
                    createExternalTextureDescriptor(
                        ...
                        cropHeight = 96,
                        mirrored = true,
                        rotation = ExternalTextureRotation.Rotate180Degrees,
                    )
    ...
    /**
     * Verifies RGB external texture descriptors are parsed correctly across the JNI boundary
     * without error.
     */
    @Test
    @MediumTest
            ...
            val unused = webGpu.execute {
                ...
                val extDescriptor =
                    createExternalTextureDescriptor(
                        ...
                        cropHeight = 48,
                        mirrored = true,
                        rotation = ExternalTextureRotation.Rotate90Degrees,
                    )
    ...
    private fun createWrappedHardwareBuffer(
        ...
        val wrapper =
            GPUAndroidHardwareBufferUtil.createTexture(device, buffer, usage = textureUsage)
    ...
    private fun populateTextureWithSolidColor(
        ...
        val byteBuffer =
            java.nio.ByteBuffer.allocateDirect(width * height * 4).apply {
    ...
    private fun validateExternalTextureImport(
        buffer: HardwareBuffer,
        extDescriptor: GPUExternalTextureDescriptor,
    ) {
        val wrapper =
            GPUAndroidHardwareBufferUtil.createExternalTexture(device, buffer, extDescriptor)
        assertNotNull(wrapper)
