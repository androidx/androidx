...
/**
 * A thread-safe helper object to convert raw file descriptors to Android `SyncFence` objects using
 * EGL.
 *
 * This object maintains a single persistent EGL context and pbuffer surface to avoid the overhead
 * of creating and destroying them on every conversion. It also ensures that the calling thread's
 * existing EGL state is preserved.
 */
...
internal object GPUSyncFenceHelper {
    ...
    /**
     * Ensures that a dedicated EGL context and a pbuffer surface are initialized. This ensures that
     * a current EGL context exists on the calling thread, which is required by some drivers for
     * EGLSync operations.
     */
     ...
     * 3. Once `EGL15.eglCreateSync` is successfully called, the underlying EGL driver takes
     *    absolute ownership of the file descriptor and guarantees its closure upon sync object
     *    destruction.
     *
     * @param pfd The ParcelFileDescriptor representing a synchronization fence.
     * @return A valid [SyncFence] duplicated from the EGL sync object, or null if
     *   unsupported/failed.
     */
    @OptIn(ExperimentalWebGpuApi::class)
    internal fun createSyncFenceFromFd(pfd: ParcelFileDescriptor): SyncFence? {
        // Ensures all EGL context operations and driver calls are perfectly synchronized across
        // threads.
        synchronized(lock) {
            ...
            // Preserve the calling thread's existing EGL context state to ensure rendering
            // continuity.
            ...
            try {
                // Detach ownership of the file descriptor from Java, as EGL is about to assume
                // ownership.
                ...
                val attributes =
                    longArrayOf(
                        EGL_SYNC_NATIVE_FENCE_FD_ANDROID.toLong(),
                        syncFd.toLong(),
                ...
                if (eglSync == null || eglSync == EGL15.EGL_NO_SYNC) {
                    // EGL failed to take ownership. We must adopt and close since it was already
                    // detached.
                ...
                // Guaranteed Restoration: Reset the calling thread to its original EGL display and
                // context.
