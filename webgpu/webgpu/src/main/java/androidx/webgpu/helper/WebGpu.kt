 ...
 *
 * The native Dawn engine is not thread-safe for concurrent access. To prevent race conditions and
 * segmentation faults, all WebGPU operations (including initialization, processEvents polling,
...
public abstract class WebGpu : AutoCloseable {
    ...
    /**
     * Runs a continuous loop on the WebGPU dispatcher to poll for and process WebGPU events. This
     * is required for asynchronous operations (e.g., buffer mapping callbacks) to trigger.
     */
 ...
 *
 * @param dispatcher A custom [CoroutineDispatcher] to use for executing WebGPU JNI operations. Must
 *   be single-threaded.
 * @param surface An optional Android [Surface] to bind to WebGPU.
 * @param instanceDescriptor Options for configuring the native WebGPU instance.
 * @param requestAdapterOptions Options for selecting the GPU adapter (e.g., backend type).
