...
public suspend fun <T> awaitGPURequest(block: (callback: GPURequestCallback<T>) -> Unit): T =
 ...
 *
 * This handles the "Transport Layer" errors. If the native side reports a non-Success status (like
 * DeviceLost or Unknown), this class handles it immediately.
 *
...
internal abstract class BaseGPURequestRunnable<T>(
    ...
    /**
     * Called only if status == Status.Success. Implementations should check their specific 'result'
     * payload here.
     */
