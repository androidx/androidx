...
internal class CredentialProviderBeginSignInController(context: Context) :
        ...
        val context =
            contextReference.get()
                ?: throw GetCredentialUnknownException(
                    "A valid context is required to convert the request to play services, but the context reference is null."
                )
        return BeginSignInControllerUtility.constructBeginSignInRequest(request, context)
    }

    @VisibleForTesting
    public override fun convertResponseToCredentialManager(
        response: SignInCredential
    ): GetCredentialResponse {
        ...
        if (response.password != null) {
            ...
            cred =
                PublicKeyCredential(
                    PublicKeyCredentialControllerUtility.Companion.toAssertPasskeyResponse(response)
                )
        } else {
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun getInstance(context: Context): CredentialProviderBeginSignInController {
