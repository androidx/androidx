 ...
 *
 * This request will not contain all parameters needed to create the public key. Provider must use
 * the initial parameters to determine if the public key can be created, and return a
 * [BeginCreateCredentialResponse] containing a list of [CreateEntry], denoting the accounts/groups
 * where the public key can be registered. When user selects one of the returned [CreateEntry], the
 * corresponding [PendingIntent] set on the [CreateEntry] will be fired. The [Intent] invoked
 * through the [PendingIntent] will contain the complete [CreatePublicKeyCredentialRequest]. This
 * request will contain all required parameters to actually register a public key.
 *
 ...
 * @param candidateQueryData the raw bundle containing the request parameters, not expected to be
 *   used directly as all basic parameters are parsed into structured properties in this class like
 *   [requestJson] and [clientDataHash]
 ...
 */
public class BeginCreatePublicKeyCredentialRequest
@JvmOverloads
constructor(
