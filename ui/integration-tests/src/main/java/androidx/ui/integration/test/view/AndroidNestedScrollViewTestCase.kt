...
class AndroidNestedScrollViewTestCase : AndroidTestCase,
    ...
    override fun getContent(activity: Activity): ViewGroup {
        val scrollView = activity.layoutInflater
            .inflate(androidx.ui.integration.test.R.layout.simple_store, null) as ViewGroup
        ...
        firstScrollView = scrollView.findViewById(androidx.ui.integration.test.R.id.first_row)
        return scrollView
    }
    ...
    fun visitImages(viewGroup: ViewGroup, block: (View) -> Unit) {
        for (i in 0 until viewGroup.childCount) {
            ...
            } else if (child.id == androidx.ui.integration.test.R.id.item_image) {
                block(child)
            }
        }
    }
