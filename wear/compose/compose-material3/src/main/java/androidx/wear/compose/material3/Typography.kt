 ...
 *
 * Title styles are hierarchical text used as a mechanism for way-finding, like a page,
 * section title, or sub-section title (in the case of Title Small).
 *
 * Label styles are used for component level text that describes an action that would happen
 * if interacted with. The most common and widely used application for label is for text
 * nested within a button.
 *
 * Body styles are reserved for content text like paragraphs of body copy, text used in
 * complex data visualisation, time stamps and metadata.
 *
 * Numeral text styles are used for numerical digits, usually limited to a few characters.
 * These can take on more expressive properties at the larger display sizes. They give flexibility
 * to expand width axis with minimal localization and font scaling concerns.
 *
 ...
 *
 * @property arcMedium ArcMedium is for arc headers and titles. Arc is for text along a curved
 * path on the screen, reserved for short header text strings at the very top or bottom of the
 * screen like page titles.
 *
 * @property arcSmall  ArcSmall is for arc headers and titles. Arc is for text along a curved path
 ...
 * or sans serif typeface is recommended.
 *
 * @property numeralExtraLarge NumeralExtraLarge is the largest role for digits.
 * Numerals use tabular spacing by default. They highlight and express glanceable numbers
 * that are limited to a two or three characters only, where no localization is required
 * like the charging screen.
 *
 * @property numeralLarge NumeralLarge is the second largest role for digits.
 * Numerals use tabular spacing by default. They are large sized number strings that are limited
 * to big displays of time, where no localization is required like a timer countdown.
 *
 * @property numeralMedium NumeralMedium is the third largest role for digits. Numerals use
 * tabular spacing by default. They are medium sized numbers that are limited to short strings of
 * digits, where no localization is required like a steps count.
 *
 ...
 *
 * @property numeralExtraSmall NumeralExtraSmall is the smallest role for digits.
 * Numerals use tabular spacing by default. They are for numbers that need to accommodate longer
 * strings of digits, where no localization is required like in-workout metrics.
 */
@Immutable
