...
import androidx.wear.compose.remote.material3.samples.RemoteCheckboxButtonSample
import androidx.wear.compose.ui.tooling.preview.WearPreviewDevices

@Composable
fun RemoteCheckboxButtonDemos(modifier: Modifier = Modifier) {
        ...
        TransformingLazyColumn(state = columnState, contentPadding = contentPadding) {
            item {
                ListHeader(
                    ...
                    Text(
                        "RemoteCheckboxButton Demos",
                        modifier = Modifier.fillMaxWidth(),
                        textAlign = TextAlign.Center,
                    )
                }
            }
            remoteDemoItem("Checked") { RemoteCheckboxButtonChecked() }
            remoteDemoItem("Unchecked") { RemoteCheckboxButtonUnchecked() }
            remoteDemoItem("Disabled Checked") { RemoteCheckboxButtonDisabledChecked() }
            remoteDemoItem("Disabled Unchecked") { RemoteCheckboxButtonDisabledUnchecked() }
            remoteDemoItem("With Icon") { RemoteCheckboxButtonWithIcon() }
            remoteDemoItem("With Secondary Label") { RemoteCheckboxButtonWithSecondaryLabel() }
            remoteDemoItem("Sample") { RemoteCheckboxButtonSample() }
        }
    }
}
...
@WearPreviewDevices
@Composable
private fun RemoteCheckboxButtonDemosPreview() {
