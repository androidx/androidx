...
import androidx.wear.compose.remote.material3.samples.RemoteLinearProgressIndicatorAnimatedSample
import androidx.wear.compose.ui.tooling.preview.WearPreviewDevices

@Composable
fun RemoteLinearProgressIndicatorDemos(modifier: Modifier = Modifier) {
        ...
        TransformingLazyColumn(state = columnState, contentPadding = contentPadding) {
            item {
                ListHeader(
                    ...
                    Text(
                        "RemoteLinearProgressIndicator Demos",
                        modifier = Modifier.fillMaxWidth(),
                        textAlign = TextAlign.Center,
                    )
                }
            }
            remoteDemoItem("Default") { RemoteLinearProgressDefault() }
            remoteDemoItem("Custom Width") { RemoteLinearProgressCustomWidth() }
            remoteDemoItem("Scaled Dot") { RemoteLinearProgressScaledDot() }
            remoteDemoItem("Custom Color") { RemoteLinearProgressCustomColor() }
            remoteDemoItem("Disabled") { RemoteLinearProgressDisabled() }
            remoteDemoItem("Small Stroke") { RemoteLinearProgressSmallStroke() }
            remoteDemoItem("Zero") { RemoteLinearProgressZero() }
            remoteDemoItem("Complete") { RemoteLinearProgressComplete() }
            remoteDemoItem("Animated") { RemoteLinearProgressIndicatorAnimatedSample() }
        }
    }
}
...
@WearPreviewDevices
@Composable
private fun RemoteLinearProgressIndicatorDemosPreview() {
