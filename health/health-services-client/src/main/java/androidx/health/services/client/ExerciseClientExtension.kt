 ...
 *
 * Once called, Health Services will warmup the sensors based on the [ExerciseType] and requested
 * [DataType]s.
 *
 * If the calling app already has an active exercise in progress or if it does not have the required
 * permissions, then this call throws [HealthServicesException]. If another app owns the active
 * exercise then this call will succeed.
 *
 * Sensors available for warmup are GPS [DataType.LOCATION] and HeartRate [DataType.HEART_RATE_BPM].
 * Other [DataType]s requested for warmup based on exercise capabilities will be a no-op for the
 * prepare stage.
 *
 ...
 * active exercise currently in progress before starting the new one. If this occurs, clients can
 * expect to receive an [ExerciseUpdate] with [ExerciseState.ENDED], indicating that their exercise
 * has been superseded and that no additional updates will be sent. Clients can use
 * [getCurrentExerciseInfo] (described below) to check if they or another app has an active exercise
 * in-progress.
 *
 ...
 * matches the [ExerciseCapabilities] returned by [getCapabilities] since Health Services will
 ...
 *
 * Before transitioning to [ExerciseState.USER_PAUSED], Health Services will flush and return the
 * sensor data. While the exercise is paused, active time and cumulative metrics such as distance
 * will not accumulate. Instantaneous measurements such as speed and heart rate will continue to
 * update if requested in the [ExerciseConfig].
 *
 * Note that GPS and other sensors may be stopped when the exercise is paused in order to conserve
 * battery. This may happen immediately, or after some time. (The exact behavior is hardware
 * dependent.) Should this happen, access will automatically resume when the exercise is resumed.
 *
 * If the exercise is already paused then this method has no effect. If the exercise has ended then
 * [HealthServicesException] is thrown.
 *
 ...
 *
 * If the exercise has been started but is not currently paused this method has no effect. If the
 * exercise has ended then [HealthServicesException] is thrown.
 *
 * @throws HealthServicesException if the exercise has ended or if Health Service fails to process
 *   the call
 */
 ...
 *
 * Health Services will flush and then shut down the active sensors and return an [ExerciseUpdate]
 * with [ExerciseState.ENDED] along with the reason [ExerciseEndReason.USER_END] to the
 * [ExerciseUpdateCallback]. If the exercise has already ended then this call fails with a
 * [HealthServicesException].
 *
 * No additional metrics will be produced for the exercise and any on device persisted data about
 * the exercise will be deleted after the summary has been sent back.
 *
 ...
 * marked lap and starts a new lap. If the exercise supports laps this method can be called at any
 * point after an exercise has been started and before it has been ended regardless of the exercise
 * status.
 *
 * The metrics in the lap summary will start from either the start time of the exercise or the last
 * time a lap was marked to the time this method is being called.
 *
 ...
 *
 * This can be used by clients to determine if they or another app already owns an active exercise
 * being tracked by Health Services. For example, if an app is killed and it learns it owns the
 * active exercise it can register a new [ExerciseUpdateCallback] and pick tracking up from where it
 * left off.
 *
 ...
 *
 * Takes into account equivalent milestones (i.e. milestones which are not equal but are different
 * representation of a common milestone. e.g. milestone A for every 2kms, currently at threshold of
 * 10kms, and milestone B for every 2kms, currently at threshold of 8kms).
 *
 ...
 *
 * This can be used to determine what [ExerciseType]s and [DataType]s this device supports. Clients
 * should use the capabilities to inform their requests since Health Services will typically reject
 * requests made for [DataType]s or features (such as auto-pause) which are not enabled for the
 * rejected [ExerciseType].
 *
 ...
 *
 * This can be used to update the configurable attributes for the ongoing exercise, as defined in
 * [ExerciseTypeConfig].
 *
