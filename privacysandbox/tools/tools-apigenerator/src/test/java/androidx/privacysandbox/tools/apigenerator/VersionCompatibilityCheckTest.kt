...
class VersionCompatibilityCheckTest {
    ...
    fun sdkDescriptorWithMissingMetadata_throws() {
        assertThrows<IllegalArgumentException> { runGeneratorWithResources(mapOf()) }
            .hasMessageThat()
            .contains("Missing tool metadata in SDK API descriptor")
    }
    ...
    fun sdkDescriptorWithInvalidMetadataContent_throws() {
        assertThrows<IllegalArgumentException> {
                runGeneratorWithResources(mapOf(Metadata.filePath to "bogus data".toByteArray()))
            }
            .hasMessageThat()
            .contains("Invalid Privacy Sandbox tool metadata")
    }

    @Test
    fun sdkDescriptorWithIncompatibleVersion_throws() {
        val sdkMetadata = ToolMetadata.newBuilder().setCodeGenerationVersion(999).build()
        assertThrows<IllegalArgumentException> {
                runGeneratorWithResources(mapOf(Metadata.filePath to sdkMetadata.toByteArray()))
            }
            .hasMessageThat()
            .contains("SDK uses incompatible Privacy Sandbox tooling (version 999)")
    }

    @Test
    fun sdkDescriptorWithLowerVersion_isCompatible() {
        val sdkMetadata = ToolMetadata.newBuilder().setCodeGenerationVersion(0).build()
        runGeneratorWithResources(mapOf(Metadata.filePath to sdkMetadata.toByteArray()))
    }

    private fun runGeneratorWithResources(resources: Map<Path, ByteArray>) {
        val aidlCompilerPath =
            System.getProperty("aidl_compiler_path")?.let(::Path)
                ?: throw IllegalArgumentException("aidl_compiler_path flag not set.")
        val frameworkAidlPath =
            System.getProperty("framework_aidl_path")?.let(::Path)
