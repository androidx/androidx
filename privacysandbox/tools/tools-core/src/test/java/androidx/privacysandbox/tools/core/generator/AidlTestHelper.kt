...
internal object AidlTestHelper {
    fun runGenerator(api: ParsedApi): AidlGenerationOutput {
        ModelValidator.validate(api).also {
            assertWithMessage(
                    "Tried to generate AIDL code for invalid interface:\n" +
        ...
        val aidlGeneratedSources =
            tmpDir
                .toFile()
                .walk()
                .filter { it.extension == "aidl" }
                .map { AidlSource(tmpDir.relativize(it.toPath()).toString(), it.readText()) }
                .toList()
