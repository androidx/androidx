...
/** Integration tests for [RuntimeProcessor]. */
class RuntimeProcessorTest {
    ...
    fun testSucceedsWithoutWarnings() {
        ...
        CompilationSubject.assertThat(compile(testClass)).succeededWithoutWarnings()
    }

    private fun compile(vararg sources: JavaFileObject): Compilation {
        return Compiler.javac().withProcessors(RuntimeProcessor()).compile(*sources)
    }
}
