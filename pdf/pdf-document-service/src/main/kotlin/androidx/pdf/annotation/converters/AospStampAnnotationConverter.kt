...
/** Converts a [AospStampAnnotation] to a [ParcelableStampAnnotation]. */
@RequiresExtension(extension = Build.VERSION_CODES.S, version = 18)
internal class AospStampAnnotationConverter :
    Converter<AospStampAnnotation, ParcelableStampAnnotation> {
    override fun convert(from: AospStampAnnotation, vararg args: Any): ParcelableStampAnnotation {
        require(args.isNotEmpty() && args[0] is Int) {
            "First parameter is required to be pagenum."
        }
        val pdfObjects = mutableListOf<ParcelablePdfObject>()
        ...
        return ParcelableStampAnnotation(pageNum, from.bounds, pdfObjects)
    }
}
