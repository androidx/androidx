...
internal class CyclicSparseArrayIterator(private val visiblePage: Int) {
    ...
    fun current(): QueryResultsIndex = lock.withLock {
    ...
    fun next(): QueryResultsIndex = lock.withLock {
    ...
    fun prev(): QueryResultsIndex = lock.withLock {
        val pageNumIndex = searchData.indexOfKey(currentPageNum)
        val resultsOnPage = searchData.valueAt(pageNumIndex)
     ...
     */
    fun moveToIndex(index: Int) = lock.withLock {
        val pageNumIndex = searchData.indexOfKey(currentPageNum)
     ...
     * match found.
     */
    fun addMatches(pageNum: Int, matches: List<PageMatchBounds>) = lock.withLock {
