...
/**
 * `FileProvider` is a helper class designed to handle file operations specifically for storing and
...
class FileProvider() {
    companion object {
         ...
         */
        const val FILE_PROVIDER_AUTHORITIES = "androidx.core.telecom.reference.fileprovider"
         ...
         */
        fun writeCallIconBitMapToFile(context: Context, bitmap: Bitmap, callId: String): IconData? =
            try {
                val directory = createDirectory(context)
                val imageFile = File(directory, "${callId}.png")
