...
class CombineObjectFilesTaskTest : BaseClangTest() {
    ...
    fun configureAndExecute() {
        ...
        val taskProvider =
            project.tasks.register("combineTask", CombineObjectFilesTask::class.java) {
                it.outputDirectory.set(taskOutputDir)
            }
        taskProvider.configureFrom(multiTargetNativeCompilation) { it in chosenTargets }
        ...
        chosenTargets.forEach { chosenTarget ->
            multiTargetNativeCompilation.sharedObjectOutputFor(chosenTarget).get().asFile.also {
        ...
        val outputContents =
            taskOutputDir
                .walkTopDown()
                .filter { it.isFile }
                .map { it.relativeTo(taskOutputDir).path to it.readText() }
                .toList()
