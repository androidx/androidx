...
class AffectedModuleDetectorImplTest {
    ...
    @Test
    fun `Only LibraryVersion change`() {
        val detector = AffectedModuleDetectorImpl(
            projectGraph = projectGraph,
            dependencyTracker = dependencyTracker,
            logger = logger,
        ...
        MatcherAssert.assertThat(
            detector.shouldInclude("p1", allowSkippingForVersionBumps = true),
            CoreMatchers.`is`(
                false
            )
        )
        MatcherAssert.assertThat(
            detector.shouldInclude("p1", allowSkippingForVersionBumps = false),
            CoreMatchers.`is`(
                true
            )
        )
    ...
    @Test
    fun `LibraryVersion and API txt change`() {
        val detector = AffectedModuleDetectorImpl(
            projectGraph = projectGraph,
            dependencyTracker = dependencyTracker,
            logger = logger,
            ...
            changedFilesProvider = {
                listOf(
                    libraryVersions,
                    convertToFilePath("p1", "api/1.0.0.txt"),
                )
            }
        )
        MatcherAssert.assertThat(
    ...
    @Test
    fun `LibraryVersion, API txt, and project change`() {
        val detector = AffectedModuleDetectorImpl(
            projectGraph = projectGraph,
            dependencyTracker = dependencyTracker,
            logger = logger,
...
private const val libraryVersions =
