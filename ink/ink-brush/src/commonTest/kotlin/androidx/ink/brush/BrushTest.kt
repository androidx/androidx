...
class BrushTest {
    ...
    @Test
    fun constructor_usesPassedInFamily() {
        val family = BrushFamily(clientBrushFamilyId = "/brush-family:inkpen:1")
        val brush = Brush.createWithColorIntArgb(family, color.toArgb(), size, epsilon)
        assertThat(brush.family).isSameInstanceAs(family)
    }
    ...
    fun copy_returnsTheSameBrush() {
        ...
        // A copy with no arguments returns the same instance since Brushes are immutable.
    ...
    @Test
    fun stockBrushes_canBeLoaded() {
        val marker = StockBrushes.marker()
        assertThat(marker).isNotNull()
        val pressurePen = StockBrushes.pressurePen()
        assertThat(pressurePen).isNotNull()
        val highlighter = StockBrushes.highlighter()
        assertThat(highlighter).isNotNull()
        val dashedLine = StockBrushes.dashedLine()
        assertThat(dashedLine).isNotNull()
    }
