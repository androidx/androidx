...
private constructor(
...
) : TerminalNode(nativeAlloc, listOf(angleInput, magnitudeInput)) {
    ...
    internal companion object {
        internal fun wrapNative(
            ...
            return PolarTargetNode(nativeAlloc, angleInput, magnitudeInput)
        }
    }

    /** The vector brush tip property to affect. */
    public val target: PolarTarget =
        PolarTargetNode.PolarTarget.fromInt(PolarTargetNodeNative.getTargetInt(nativePointer))

    /** The start of the angle range for the target property. */
    public val angleRangeStart: Float
        get() = PolarTargetNodeNative.getAngleRangeStart(nativePointer)

    /** The end of the angle range for the target property. */
    public val angleRangeEnd: Float
        get() = PolarTargetNodeNative.getAngleRangeEnd(nativePointer)

    /** The start of the magnitude range for the target property. */
    public val magnitudeRangeStart: Float
        get() = PolarTargetNodeNative.getMagnitudeRangeStart(nativePointer)

    /** The end of the magnitude range for the target property. */
    public val magnitudeRangeEnd: Float
        get() = PolarTargetNodeNative.getMagnitudeRangeEnd(nativePointer)
 ...
 */
expect internal object PolarTargetNodeNative {
    fun create(
        polarTarget: Int,
        angleRangeStart: Float,
        angleRangeEnd: Float,
    ...
    fun getAngleRangeStart(nativePointer: Long): Float

    fun getAngleRangeEnd(nativePointer: Long): Float

    fun getMagnitudeRangeStart(nativePointer: Long): Float

    fun getMagnitudeRangeEnd(nativePointer: Long): Float

    fun getPolarTargetMinimumRequiredVersion(targetInt: Int): Int
}
