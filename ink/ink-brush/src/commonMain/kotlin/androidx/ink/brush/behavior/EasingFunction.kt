...
/**
 * An `EasingFunction` defines a curve between input x and output y "progress values" similar to the
 * CSS easing function: https://www.w3.org/TR/css-easing-1/#easing-functions
 *
...
public abstract class EasingFunction private constructor(pointerAlloc: () -> Long) {
    ...
    public companion object {
        ...
        public fun copyAndWrapNative(otherEasingFunctionNativePointer: Long): EasingFunction =
            when (EasingFunctionNative.getParametersType(otherEasingFunctionNativePointer)) {
                ...
                2 -> Linear.copyAndWrapNative(otherEasingFunctionNativePointer)
                3 -> Steps.copyAndWrapNative(otherEasingFunctionNativePointer)
    ...
    private constructor(@JvmField internal val value: Int, private val name: String) :
        ...
        override fun toString(): String = "EasingFunction.Predefined.$name"

        public companion object {
            private val VALUE_TO_INSTANCE = MutableIntObjectMap<Predefined>()

            private fun fromInt(value: Int): Predefined =
                checkNotNull(VALUE_TO_INSTANCE.get(value)) { "Invalid Predefined value: $value" }

            internal fun copyAndWrapNative(otherEasingFunctionNativePointer: Long): Predefined =
                fromInt(
                    EasingFunctionNative.getPredefinedValueInt(otherEasingFunctionNativePointer)
                )

            /** The linear identity function: accepts and returns values outside [0, 1]. */
            @JvmField public val LINEAR: Predefined = Predefined(0, "LINEAR")
             ...
             */
            @JvmField public val EASE: Predefined = Predefined(1, "EASE")
             ...
             * and @see [CubicBezier] about input values outside [0, 1])
             */
            @JvmField public val EASE_IN: Predefined = Predefined(2, "EASE_IN")
             ...
             * and @see [CubicBezier] about input values outside [0, 1])
             */
            @JvmField public val EASE_OUT: Predefined = Predefined(3, "EASE_OUT")
             ...
             * and @see [CubicBezier] about input values outside [0, 1])
             */
            @JvmField public val EASE_IN_OUT: Predefined = Predefined(4, "EASE_IN_OUT")
             ...
             */
            @JvmField public val STEP_START: Predefined = Predefined(5, "STEP_START")
             ...
             */
            @JvmField public val STEP_END: Predefined = Predefined(6, "STEP_END")
        }
    }
     ...
     *
     * A step function is defined by the number of equal-sized steps into which the [0, 1) interval
     * of input-x is split and the behavior at the extremes. When x < 0, the output will always be
     * 0. When x >= 1, the output will always be 1. The output of the first and last steps is
     *    governed by the [StepPosition].
     *
    ...
    internal constructor(@JvmField internal val value: Int, private val name: String) {
        init {
            check(value !in VALUE_TO_INSTANCE) { "Duplicate StepPosition value: $value." }
            VALUE_TO_INSTANCE[value] = this
        }
        ...
        public companion object {
            ...
            internal fun fromInt(value: Int): StepPosition =
                checkNotNull(VALUE_TO_INSTANCE.get(value)) { "Invalid StepPosition value: $value" }

            /**
             * The step function "jumps" at the end of [0, 1): For x in [0, 1/step_count) => y = 0.
             * For x in [1 - 1/step_count, 1) => y = 1 - 1/step_count.
             */
            @JvmField public val JUMP_END: StepPosition = StepPosition(0, "JUMP_END")
            /**
             * The step function "jumps" at the start of [0, 1): For x in [0, 1/step_count) => y =
             * 1/step_count. For x in [1 - 1/step_count, 1) => y = 1.
             */
            @JvmField public val JUMP_START: StepPosition = StepPosition(1, "JUMP_START")
            /**
             ...
             * 1).
             */
            @JvmField public val JUMP_BOTH: StepPosition = StepPosition(2, "JUMP_BOTH")

            /**
             * The step function does not "jump" at either boundary: For x in [0, 1/step_count) => y
             * = 0. For x in [1 - 1/step_count, 1) => y = 1.
             */
            @JvmField public val JUMP_NONE: StepPosition = StepPosition(3, "JUMP_NONE")
        }
    }
}

expect internal object EasingFunctionNative {
