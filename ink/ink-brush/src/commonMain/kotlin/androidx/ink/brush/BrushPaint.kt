...
private constructor(
    nativeAlloc: () -> Long,
    // The [textureLayers] val below is a defensive copy of this parameter if provided.
    textureLayers: List<TextureLayer>? = null,
    // The [colorFunctions] val below is a defensive copy of this parameter if provided.
    ...
    public val textureLayers: List<TextureLayer> =
        unmodifiableList(
            // If we just constructed this in Kotlin, use the list of TextureLayers provided.
            textureLayers
                // Otherwise, we need to copy the native TextureLayers into Kotlin objects.
                ?: (0 until BrushPaintNative.getTextureLayerCount(nativePointer)).map { index ->
                    TextureLayer.wrapNative(
                        BrushPaintNative.getTextureLayerMappingInt(nativePointer, index)
                    ) {
                        BrushPaintNative.newCopyOfTextureLayer(nativePointer, index)
                    }
                }
        )

    /**
     * An immutable list of the transformations to apply to the base brush color (in order) before
     * drawing this coat of paint. When this list is empty, the base brush color will be used
     * unchanged.
     */
    public val colorFunctions: List<ColorFunction> =
        unmodifiableList(
            // If we just constructed this in Kotlin, use the list of ColorFunctions provided.
            colorFunctions
                // Otherwise, we need to copy the native ColorFunctions into Kotlin objects.
                ?: (0 until BrushPaintNative.getColorFunctionCount(nativePointer)).map { index ->
                    ColorFunction.wrapNative(
                        BrushPaintNative.getColorFunctionParametersTypeInt(nativePointer, index)
                    ) {
                        BrushPaintNative.newCopyOfColorFunction(nativePointer, index)
                    }
                }
        )
    ...
    public abstract class TextureLayer internal constructor(nativeAlloc: () -> Long) {
        ...
        public val blendMode: BlendMode =
            BrushPaint.TextureLayer.BlendMode.fromInt(
                TextureLayerNative.getBlendModeInt(nativePointer)
            )
        ...
        public companion object {
            ...
            public fun wrapNative(mappingInt: Int, nativeAlloc: () -> Long): TextureLayer =
                when (mappingInt) {
                    ...
                    MAPPING_STAMPING -> StampingTexture.wrapNative(nativeAlloc)
                    else -> throw IllegalArgumentException("Invalid texture mapping mode")
                }
        }
     ...
     */
    public class TilingTexture private constructor(nativeAlloc: () -> Long) :
        TextureLayer(nativeAlloc) {
        ...
        public val sizeX: Float = TilingTextureNative.getSizeX(nativePointer)

        /** The height of the texture, specified in `sizeUnit`s. */
        @FloatRange(from = 0.0, fromInclusive = false, toInclusive = false)
        public val sizeY: Float = TilingTextureNative.getSizeY(nativePointer)

        /** The horizontal offset for the texture, specified as a fraction of the texture width. */
        public val offsetX: Float = TilingTextureNative.getOffsetX(nativePointer)

        /** The vertical offset for the texture, specified as a fraction of the texture height. */
        public val offsetY: Float = TilingTextureNative.getOffsetY(nativePointer)
         ...
         */
        @AngleDegreesFloat
        public val rotationDegrees: Float = TilingTextureNative.getRotationDegrees(nativePointer)

        /** The units in which this texture layer's width and height are measured. */
        public val sizeUnit: SizeUnit =
            BrushPaint.TextureLayer.SizeUnit.fromInt(
                TilingTextureNative.getSizeUnitInt(nativePointer)
            )

        /** The origin that will be used for positioning this texture layer. */
        public val origin: Origin = Origin.fromInt(TilingTextureNative.getOriginInt(nativePointer))

        /** The horizontal wrapping mode for this texture layer. */
        public val wrapX: Wrap =
            BrushPaint.TextureLayer.Wrap.fromInt(TilingTextureNative.getWrapXInt(nativePointer))

        /** The vertical wrapping mode for this texture layer. */
        public val wrapY: Wrap =
            BrushPaint.TextureLayer.Wrap.fromInt(TilingTextureNative.getWrapYInt(nativePointer))
         ...
         * returned builder to build a copy of a [TilingTexture]. Kotlin developers, see [copy]
         * method.
         */
        ...
        override fun equals(other: Any?): Boolean {
            if (other !is TilingTexture) return false
        ...
        override fun toString(): String =
            "BrushPaint.TilingTexture(clientTextureId=$clientTextureId, sizeX=$sizeX, " +
                "sizeY=$sizeY, offset=[$offsetX, $offsetY], rotationDegrees=$rotationDegrees, " +
                "sizeUnit=$sizeUnit, origin=$origin, wrapX=$wrapX, wrapY=$wrapY, blendMode=$blendMode)"
         ...
         *
         * For Java developers, use `TilingTexture.Builder` to construct a [TilingTexture] with
         * default values, overriding only as needed. For example: `TilingTexture layer =
         * TilingTexture.builder().setClientTextureId(id).setSizeX(width).setSizeY(height).build();`
         */
        @Suppress(
            "ScopeReceiverThis"
        ...
        internal constructor(
            ...
            public fun build(): TilingTexture {
                val clientTextureId =
    ...
    /**
     * A texture that is "stamped" onto each particle of the stroke, scaled or rotated appropriately
     * to cover the whole particle.
     */
    public class StampingTexture private constructor(nativeAlloc: () -> Long) :
        TextureLayer(nativeAlloc) {
        /**
        ...
        public fun copy(
            ...
            if (clientTextureId == this.clientTextureId && blendMode == this.blendMode) {
                return this
            }
            return StampingTexture(
                clientTextureId = clientTextureId,
                blendMode = blendMode,
        ...
        /**
         * Returns a [Builder] with values set equivalent to `this`. Java developers, use the
         * returned builder to build a copy of a [StampingTexture]. Kotlin developers, see [copy]
         * method.
         */
        public fun toBuilder(): Builder =
            Builder(
        ...
        override fun equals(other: Any?): Boolean {
            if (other !is StampingTexture) return false
            return clientTextureId == other.clientTextureId &&
         ...
         *
         * For Java developers, use `StampingTexture.Builder` to construct a [StampingTexture] with
         * default values, overriding only as needed. For example: `StampingTexture layer =
         * StampingTexture.builder().setClientTextureId(id).build();`
         */
        @Suppress(
            "ScopeReceiverThis"
        ) // Builder pattern supported for Java clients, despite being an anti-pattern in Kotlin.
        ...
        internal constructor(
             ...
             */
            @Suppress("Range") // we check() before passing floats
            public fun build(): StampingTexture {
                val clientTextureId =
                    checkNotNull(this.clientTextureId) {
                        "must set clientTextureId before calling build()"
                    }
    ...
    public abstract class ColorFunction private constructor(nativeAlloc: () -> Long) {
        ...
        public class OpacityMultiplier internal constructor(nativeAlloc: () -> Long) :
...
expect internal object BrushPaintNative {
    ...
    fun getTextureLayerCount(nativePointer: Long): Int

    fun getTextureLayerMappingInt(nativePointer: Long, index: Int): Int
     ...
     */
    fun newCopyOfTextureLayer(nativePointer: Long, index: Int): Long

    fun getColorFunctionCount(nativePointer: Long): Int

    fun getColorFunctionParametersTypeInt(nativePointer: Long, index: Int): Int
...
expect internal object TilingTextureNative {
    ...
    fun getSizeY(nativePointer: Long): Float

    fun getOffsetX(nativePointer: Long): Float

    fun getOffsetY(nativePointer: Long): Float

    fun getRotationDegrees(nativePointer: Long): Float

    fun getSizeUnitInt(nativePointer: Long): Int

    fun getOriginInt(nativePointer: Long): Int

    fun getWrapXInt(nativePointer: Long): Int

    fun getWrapYInt(nativePointer: Long): Int
}

expect internal object StampingTextureNative {
    fun create(
        clientTextureId: String,
        animationFrames: Int,
        animationRows: Int,
