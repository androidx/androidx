...
private constructor(
...
) : TerminalNode(nativeAlloc, listOf(input)) {
    ...
    public constructor(
        ...
        { TargetNodeNative.create(target.value, targetModifierRangeStart, targetModifierRangeEnd) },
        input,
    )

    internal companion object {
        internal fun wrapNative(nativeAlloc: () -> Long, inputStack: ArrayDeque<ValueNode>) =
            TargetNode(nativeAlloc, input = inputStack.removeLast())
    }

    /** The brush tip property to affect. */
    public val target: Target =
        TargetNode.Target.fromInt(TargetNodeNative.getTargetInt(nativePointer))
 ...
 */
expect internal object TargetNodeNative {
    fun create(target: Int, targetModifierRangeStart: Float, targetModifierRangeEnd: Float): Long

    fun getTargetInt(nativePointer: Long): Int
