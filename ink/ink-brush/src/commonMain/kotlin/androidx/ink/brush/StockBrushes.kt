...
public object StockBrushes {
    ...
    private val markerV1 by lazy {
        BrushFamily.wrapNative { StockBrushesNative.createMarker(MarkerVersion.V1.value) }
    }
    ...
    private val pressurePenV1 by lazy {
        BrushFamily.wrapNative { StockBrushesNative.createPressurePen(PressurePenVersion.V1.value) }
    }
        ...
        listOf(SelfOverlap.ANY, SelfOverlap.ACCUMULATE, SelfOverlap.DISCARD).associateWith {
            lazy {
                BrushFamily.wrapNative {
                    StockBrushesNative.createHighlighter(it.value, HighlighterVersion.V1.value)
                }
            }
        }
    ...
    private val dashedLineV1 by lazy {
        BrushFamily.wrapNative { StockBrushesNative.createDashedLine(DashedLineVersion.V1.value) }
    }
     ...
     *
     * In order to use this brush, the renderer implementation must map the [clientTextureId] to a
     * bitmap; otherwise, no texture will be visible. The emoji bitmap should be a square, though
     * the image can have a transparent background for emoji shapes that aren't square.
     *
     * @param clientTextureId The client texture ID of the emoji to appear at the end of the stroke.
     *   This ID should map to a square bitmap with a transparent background (for example, in the
     *   implementation of `TextureBitmapStore` passed to
    ...
    public fun emojiHighlighter(
        ...
        return BrushFamily.wrapNative {
            StockBrushesNative.createEmojiHighlighter(
                clientTextureId,
                showMiniEmojiTrail,
                selfOverlap.value,
                version.value,
...
expect internal object StockBrushesNative {
    fun createMarker(version: Int): Long

    fun createDashedLine(version: Int): Long

    fun createPressurePen(version: Int): Long

    fun createHighlighter(selfOverlap: Int, version: Int): Long

    fun createEmojiHighlighter(
        clientTextureId: String,
    ...
    fun createPredictionFadeOutBehavior(): Long
}
