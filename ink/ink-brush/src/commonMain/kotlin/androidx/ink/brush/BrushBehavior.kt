...
private constructor(
    nativeAlloc: () -> Long,
    // The [terminalNodes] val below is a defensive copy of this parameter if provided.
    terminalNodes: List<TerminalNode>? = null,
    ...
    public val terminalNodes: List<TerminalNode> =
        unmodifiableList(
            // If we just constructed this in Kotlin, use the list of TerminalNodes provided.
            terminalNodes
                // Otherwise, we need to copy the native nodes into Kotlin objects.
                ?: run {
                    val terminalNodes = mutableListOf<TerminalNode>()
                    val inputStack = ArrayDeque<ValueNode>()
                    for (i in 0 until BrushBehaviorNative.getNodeCount(nativePointer)) {
                        when (
                            val node =
                                Node.wrapNative(
                                    { BrushBehaviorNative.newCopyOfNode(nativePointer, i) },
                                    BrushBehaviorNative.getNodeTypeInt(nativePointer, i),
                                    inputStack,
                                )
                        ) {
    ...
    public val developerComment: String =
        ...
        developerComment
            // Otherwise, read it from the native object.
            ?: BrushBehaviorNative.getDeveloperComment(nativePointer)
    ...
    public constructor(
        ...
        { newBrushBehaviorFromTerminalNodes(terminalNodes, developerComment) },
        terminalNodes,
        developerComment,
    )
    ...
    override fun equals(other: Any?): Boolean {
        if (other !is BrushBehavior) return false
    ...
    public companion object {
        private fun newBrushBehaviorFromTerminalNodes(
            ...
            return BrushBehaviorNative.createFromOrderedNodes(
...
expect internal object BrushBehaviorNative {
    fun createFromOrderedNodes(orderdNodeNativePointers: LongArray, developerComment: String): Long

    fun free(nativePointer: Long)

    fun getNodeCount(nativePointer: Long): Int

    fun getNodeTypeInt(nativePointer: Long, index: Int): Int
