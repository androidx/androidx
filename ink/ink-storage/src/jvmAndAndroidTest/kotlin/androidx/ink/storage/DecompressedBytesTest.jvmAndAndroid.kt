...
class DecompressedBytesTest {
    ...
    fun decompress_belowInitialCapacity() {
        ...
        assertThat(decompressedBytes.buffer.size)
        ...
        assertThat(decompressedBytes.buffer.slice(0 until decompressedBytes.size).toByteArray())
        ...
        assertThat(
                decompressedBytes.buffer
                    .slice(decompressedBytes.size until decompressedBytes.buffer.size)
                    .toByteArray()
            )
            .isEqualTo(ByteArray(decompressedBytes.buffer.size - decompressedBytes.size) { 0 })
    }
    ...
    fun decompress_exactlyAtInitialCapacity() {
        ...
        assertThat(decompressedBytes.size).isEqualTo(originalBytes.size)
        // The decompressed array starts with the decompressed bytes, then is padded with zeros.
        assertThat(decompressedBytes.buffer.slice(0 until decompressedBytes.size))
