    ...
    fun add_meshToNonEmptyEnvelope_updatesEnvelope() {
        val envelope =
            BoxAccumulator()
                .add(
                    MutableBox()
                        .populateFromTwoPoints(ImmutableVec(11F, 11F), ImmutableVec(20F, 25F))
                )
        val mesh = buildTestShape()

        envelope.add(mesh)

        // Verify that the original lower-bounds for envelope (11F, 11F) are updated after adding
        // the
        // mesh.
        assertThat(envelope.isEmpty()).isFalse()
        val box = envelope.box!!
        assertThat(box.xMin).isLessThan(11f)
        assertThat(box.yMin).isLessThan(11f)
    }
