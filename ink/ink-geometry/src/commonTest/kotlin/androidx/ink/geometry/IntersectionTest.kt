...
class IntersectionTest {
    ...
    fun intersects_forPointAndPartitionedMesh_callsJniAndReturnsBool() {
        ...
        val intersectingAfterScalePoint = ImmutableVec(7f, 0.7f)
        val intersectingPoint = ImmutableVec(15f, 4f)
        val nonIntersectingPoint = ImmutableVec(100f, 200f)
        val pointToMeshTransform = ImmutableAffineTransform.scale(2f, 5f)
        val meshToPointTransform = pointToMeshTransform.computeInverse()

        assertThat(intersectingPoint.intersects(mesh, AffineTransform.IDENTITY)).isTrue()
        assertThat(intersectingAfterScalePoint.intersects(mesh, AffineTransform.IDENTITY)).isFalse()
        assertThat(nonIntersectingPoint.intersects(mesh, AffineTransform.IDENTITY)).isFalse()
        assertThat(mesh.intersects(intersectingPoint, AffineTransform.IDENTITY)).isTrue()
        assertThat(mesh.intersects(intersectingAfterScalePoint, AffineTransform.IDENTITY)).isFalse()
        assertThat(mesh.intersects(nonIntersectingPoint, AffineTransform.IDENTITY)).isFalse()

        assertThat(mesh.intersects(intersectingPoint, meshToPointTransform)).isFalse()
        assertThat(mesh.intersects(nonIntersectingPoint, meshToPointTransform)).isFalse()
        assertThat(mesh.intersects(intersectingAfterScalePoint, meshToPointTransform)).isTrue()
        assertThat(intersectingPoint.intersects(mesh, meshToPointTransform)).isFalse()
        assertThat(nonIntersectingPoint.intersects(mesh, meshToPointTransform)).isFalse()
        assertThat(intersectingAfterScalePoint.intersects(mesh, meshToPointTransform)).isTrue()
        assertThat(mesh.intersects(intersectingPoint.x, intersectingPoint.y, meshToPointTransform))
            .isFalse()
        assertThat(
                mesh.intersects(
                    intersectingAfterScalePoint.x,
                    intersectingAfterScalePoint.y,
                    meshToPointTransform,
                )
            )
            .isTrue()
        assertThat(
                mesh.intersects(
                    nonIntersectingPoint.x,
                    nonIntersectingPoint.y,
                    meshToPointTransform,
                )
            )
            .isFalse()
    ...
    fun intersects_forSegmentAndPartitionedMesh_callsJniAndReturnsBool() {
        val mesh =
            PartitionedMesh(
                ImmutableTriangle(
                    ImmutableVec(5f, 0.5f),
                    ImmutableVec(10f, 0.5f),
                    ImmutableVec(10f, 1f),
                )
            )
        val intersectingAfterTransformSegment =
            ImmutableSegment(start = ImmutableVec(10f, 3f), end = ImmutableVec(16f, 3f))
        val nonIntersectingSegment =
            ImmutableSegment(start = ImmutableVec(100f, 200f), end = ImmutableVec(300f, 400f))
        val meshToSegmentTransform = ImmutableAffineTransform.scale(2f, 5f)

        assertThat(mesh.intersects(intersectingAfterTransformSegment, meshToSegmentTransform))
            .isTrue()
        assertThat(mesh.intersects(nonIntersectingSegment, meshToSegmentTransform)).isFalse()
        assertThat(intersectingAfterTransformSegment.intersects(mesh, meshToSegmentTransform))
            .isTrue()
        assertThat(nonIntersectingSegment.intersects(mesh, meshToSegmentTransform)).isFalse()

        assertThat(intersectingAfterTransformSegment.intersects(mesh, AffineTransform.IDENTITY))
            .isFalse()
        assertThat(nonIntersectingSegment.intersects(mesh, AffineTransform.IDENTITY)).isFalse()
        assertThat(mesh.intersects(intersectingAfterTransformSegment, AffineTransform.IDENTITY))
            .isFalse()
        assertThat(mesh.intersects(nonIntersectingSegment, AffineTransform.IDENTITY)).isFalse()
    }
    ...
    fun intersects_forTriangleAndPartitionedMesh_callsJniAndReturnsBool() {
        ...
        val scaleTransform = ImmutableAffineTransform.scale(2f, 5f)

        assertThat(mesh.intersects(intersectingTriangle, AffineTransform.IDENTITY)).isTrue()
        assertThat(mesh.intersects(nonIntersectingTriangle, scaleTransform)).isFalse()
        assertThat(intersectingTriangle.intersects(mesh, AffineTransform.IDENTITY)).isTrue()
        assertThat(nonIntersectingTriangle.intersects(mesh, scaleTransform)).isFalse()
    }
    ...
    fun intersects_forBoxAndPartitionedMesh_callsJniAndReturnsBool() {
        ...
        assertThat(mesh.intersects(nonIntersectingBox, scaleTransform)).isFalse()
        ...
        assertThat(nonIntersectingBox.intersects(mesh, AffineTransform.IDENTITY)).isFalse()
        assertThat(nonIntersectingBox.intersects(mesh, scaleTransform)).isFalse()
    }
    ...
    fun intersects_forParallelogramAndPartitionedMesh_callsJniAndReturnsBool() {
        ...
        assertThat(mesh.intersects(nonIntersectingParallelogram, AffineTransform.IDENTITY))
            .isFalse()
        assertThat(mesh.intersects(nonIntersectingParallelogram, scaleTransform)).isFalse()
        ...
        assertThat(nonIntersectingParallelogram.intersects(mesh, AffineTransform.IDENTITY))
            .isFalse()
        assertThat(nonIntersectingParallelogram.intersects(mesh, scaleTransform)).isFalse()
    }
    ...
    fun intersects_forTwoPartitionedMeshes_callsJniAndReturnsBool() {
        ...
        val nonIntersectingShape =
            PartitionedMesh(
                ImmutableTriangle(
                    ImmutableVec(100f, 3f),
                    ImmutableVec(200f, 3f),
                    ImmutableVec(200f, 5f),
                )
            )
        val scaleTransform = ImmutableAffineTransform.scale(2f, 5f)
        assertThat(
        ...
        assertThat(
            ...
            .isFalse()
        assertThat(nonIntersectingShape.intersects(mesh, AffineTransform.IDENTITY, scaleTransform))
            .isFalse()
        assertThat(nonIntersectingShape.intersects(mesh, scaleTransform, AffineTransform.IDENTITY))
            .isFalse()
        assertThat(nonIntersectingShape.intersects(mesh, scaleTransform, scaleTransform)).isFalse()
    }
}
