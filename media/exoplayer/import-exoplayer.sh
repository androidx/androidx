...
DESTINATION_DIRECTORY="${SCRIPT_DIRECTORY}/../src/main/java/${TO_PACKAGE//./\/}"
...
# Hide top-level interfaces/classes with javadoc.
find . -name "*.java" -type f -print0 | xargs -0 \
    perl -p -i -e \
    'BEGIN {undef $/;} s/^ \*\/\n^(.*)(interface|class)/ *\n * \@hide\n *\/\n\@RestrictTo(LIBRARY_GROUP)\n\1\2/m'
...
# Run google-java-format to enforce AOSP style and organize imports.
find . -name "*.java" -type f -print0 | xargs -0 \
