...
public abstract class SafeParcel {
    ...
    /**
     * Create a sub-parcel to be used for a child SafeParcelable
     */
    protected abstract SafeParcel createSubParcel();
    ...
    /**
     * Flatten the name of the class of the SafeParcelable and its contents
     * into the parcel.
     *
     * @param p The SafeParcelable object to be written.
    ...
    /**
     * Configure the SafeParcel for current serialization method.
     */
    public void setSerializationFlags(boolean allowSerialization, boolean ignoreParcelables) {
        // Don't care except in SafeParcelStream
    }
     ...
     * type of the objects in the list must be one that implements SafeParcelable,
        ...
        if (n > 0) {
            ...
            switch (type) {
                ...
                case TYPE_SAFE_PARCELABLE:
                    while (i < n) {
                        writeSafeParcelable((SafeParcelable) val.get(i));
                        i++;
                    }
                    break;
     ...
     * the current dataPosition() and growing dataCapacity() if needed.  The
     * type of the objects in the array must be one that implements SafeParcelable,
     * Parcelable, String, or Serializable.
     *
     * @param val The list of objects to be written.
     * @see #readList
    ...
    public <T> void writeArray(T[] val) {
        ...
        if (n > 0) {
            ...
            switch (type) {
                ...
                case TYPE_SAFE_PARCELABLE:
                    while (i < n) {
                        writeSafeParcelable((SafeParcelable) val[i]);
                        i++;
                    }
                    break;
                case TYPE_SERIALIZABLE:
    ...
    private <T> int getType(T t) {
        ...
        } else if (t instanceof SafeParcelable) {
     ...
     * @param p The SafeParcelable object to be written.
     */
    public void writeSafeParcelable(SafeParcelable p, int fieldId) {
        setOutputField(fieldId);
    ...
    /**
     */
    public void writeSafeParcelable(SafeParcelable p) {
        if (p == null) {
            writeString(null);
            return;
        }
        ...
        SafeParcel subParcel = createSubParcel();
        writeToParcel(p, subParcel);
        subParcel.closeField();
    }

    private void writeSafeParcelableCreator(SafeParcelable p) {
        String name = p.getClass().getName();
        writeString(name);
    }

    /**
     * Write a generic serializable object in to a SafeParcel.  It is strongly
     * recommended that this method be avoided, since the serialization
     * overhead is extremely large, and this approach will be much slower than
     * using the other approaches to writing data in to a SafeParcel.
     */
    ...
    public void writeException(Exception e, int fieldId) {
        ...
        if (e instanceof Parcelable
            ...
            // We only send SafeParcelable exceptions that are in the
    ...
    /**
     * Special function for writing information at the front of the SafeParcel
     ...
     * will throw the exception for you if it had been written to the SafeParcel,
     * otherwise return and let you read the normal result data from the SafeParcel.
     *
     * @see #writeException
     * @see #writeNoException
     */
    ...
    /**
     * Parses the header of a Binder call's response SafeParcel and
    ...
    /**
     */
    public <T extends SafeParcelable> T readSafeParcelable(T def, int fieldId) {
        if (!readField(fieldId)) {
            return def;
        }
    ...
    /**
     * Read and return a new SafeParcelable from the parcel.
     *
     * @return Returns the newly created SafeParcelable, or null if a null
     * object has been written.
     * @throws BadParcelableException Throws BadSafeParcelableException if there
     *                                was an error trying to instantiate the SafeParcelable.
     */
    @SuppressWarnings({"unchecked", "TypeParameterUnusedInFormals"})
    public <T extends SafeParcelable> T readSafeParcelable() {
        ...
        try {
            ...
            if (!SafeParcelable.class.isAssignableFrom(parcelableClass)) {
                throw new BadParcelableException("SafeParcelable protocol requires subclassing "
                        + "from SafeParcelable on class " + name);
            }
            Constructor<?> constructor = parcelableClass.getDeclaredConstructor();
            if (constructor == null) {
                throw new BadParcelableException("SafeParcelable protocol requires "
                        + "there be a constructor taking SafeParcel on " + name);
            }
    ...
    public Serializable readSerializable() {
        ...
        if (name == null) {
            ...
            // is nothing left in the SafeParcel to read, or the next value wasn't a String), so
    ...
    /**
     */
    public static <T extends SafeParcelable> void readFromParcel(T val, SafeParcel safeParcel) {
        try {
            Class cls = findParcelClass(val);
            cls.getDeclaredMethod("read", val.getClass(), SafeParcel.class)
                    .invoke(null, val, safeParcel);
        } catch (IllegalAccessException e) {
            throw new RuntimeException("SafeParcel encountered IllegalAccessException", e);
        } catch (InvocationTargetException e) {
            throw new RuntimeException("SafeParcel encountered InvocationTargetException", e);
        } catch (NoSuchMethodException e) {
            throw new RuntimeException("SafeParcel encountered NoSuchMethodException", e);
        } catch (ClassNotFoundException e) {
            throw new RuntimeException("SafeParcel encountered ClassNotFoundException", e);
        }
    }

    /**
     */
    public static <T extends SafeParcelable> void writeToParcel(T val, SafeParcel safeParcel) {
        try {
            Class cls = findParcelClass(val);
            cls.getDeclaredMethod("write", val.getClass(), SafeParcel.class)
                    .invoke(null, val, safeParcel);
        } catch (IllegalAccessException e) {
            throw new RuntimeException("SafeParcel encountered IllegalAccessException", e);
        } catch (InvocationTargetException e) {
    ...
    private static <T extends SafeParcelable> Class findParcelClass(T val)
            throws ClassNotFoundException {
        Class<? extends SafeParcelable> cls = val.getClass();
