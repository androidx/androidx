 ...
 */
package androidx.safeparcelable.annotation;
...
/**
 * Processes annotations from SafeParcelables.
 */
@SupportedAnnotationTypes({SafeParcelProcessor.SAFE_PARCELIZE,
        SafeParcelProcessor.PARCEL_FIELD,
        SafeParcelProcessor.NON_PARCEL_FIELD})
@SupportedSourceVersion(SourceVersion.RELEASE_7)
public class SafeParcelProcessor extends AbstractProcessor {

    public static final String SAFE_PARCELIZE = "androidx.safeparcelable.SafeParcelize";
    public static final String PARCEL_FIELD = "androidx.safeparcelable.ParcelField";
    public static final String NON_PARCEL_FIELD = "androidx.safeparcelable.NonParcelField";
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        ...
        for (Element element : roundEnvironment.getElementsAnnotatedWith(field)) {
            ...
            if (!safeParcelables.contains(clsElement)) {
        ...
        if (safeParcelables.isEmpty()) {
            error("No SafeParcels found");
            return true;
        }
        for (Element safeParcelable : safeParcelables) {
            ArrayList<String> takenIds = new ArrayList<>();
            AnnotationMirror annotation = findAnnotationMirror(
                    safeParcelable.getAnnotationMirrors(), SAFE_PARCELIZE);
            ...
            checkClass(safeParcelable, takenIds);
            generateSerialization(safeParcelable, fields, allowSerialization, ignoreParcelables);
        }
    ...
    private void generateSerialization(Element safeParcelable, Set<Element> fields,
        ...
        TypeSpec.Builder genClass = TypeSpec
                .classBuilder(safeParcelable.getSimpleName() + GEN_SUFFIX)
                .addJavadoc("@hide")
        ...
        MethodSpec.Builder readBuilder = MethodSpec
                ...
                .addParameter(TypeName.get(safeParcelable.asType()), "obj")
                .addParameter(ClassName.get("androidx.safeparcelable", "SafeParcel"), "parcel");
        ...
        try {
            JavaFile.builder(getPkg(safeParcelable),
        ...
        if (element.getKind() == ElementKind.FIELD) {
            if (!element.getModifiers().contains(Modifier.STATIC)) {
                ...
                if (i == annotations.size()) {
                    mMessager.printMessage(Diagnostic.Kind.ERROR,
                            "SafeParcelables must have all fields annotated with ParcelField"
                                    + " or NonParcelField");
                    return;
                }
            }
