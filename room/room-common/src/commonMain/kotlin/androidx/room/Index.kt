...
/**
 * Declares an index on an [Entity]. see:
 ...
 * is worth the gain.
 *
 * There are 2 ways to define an index in an [Entity]. You can either set [ColumnInfo.index]
 * property to index individual fields or define composite indices via [Entity.indices].
 *
 * If an indexed field is embedded into another Entity via [Embedded], it is **NOT** added as an
...
public annotation class Index(
     ...
     * covering index is created for it: `CREATE INDEX `song_name_length` on `Song` (`name` ASC,
     * `length` DESC)`, then a query containing an ORDER BY clause with matching order of the index
     * will be able to avoid a table scan by using the index, but a mismatch in order won't.
     * Therefore the columns order of the index should be the same as the most frequently executed
     ...
     * {"bar", "baz"}, generated index name will be "index_Foo_bar_baz". If you need to specify the
