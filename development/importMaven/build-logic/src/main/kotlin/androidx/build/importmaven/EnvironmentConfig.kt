...
object EnvironmentConfig {
    private val config: Properties by lazy {
        ...
        EnvironmentConfig::class
            ...
            .let { checkNotNull(it) { "Cannot find properties file: $resourceName" } }
            .openStream()
            .use { config.load(it) }
        config
    }
    ...
    val supportRoot: Path by lazy {
        checkNotNull(config["supportRoot"]) { "missing supportRoot property" }
            .let { File(it.toString()).toOkioPath() }
    }
}
