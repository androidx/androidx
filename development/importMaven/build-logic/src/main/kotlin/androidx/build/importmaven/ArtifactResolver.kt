...
internal object ArtifactResolver {
    ...
    internal val mavenCentralMirror =
...
--------------------------------------------------------------------------------"""
            ...
            return withProxyServer(downloadObserver = downloadObserver) {
                    println("Initialized proxy servers")
                ...
                .also { result ->
                    ...
                    println("Resolved files: ${artifacts.size}")
                    check(artifacts.isNotEmpty()) {
                        "Didn't resolve any artifacts from $artifacts. Try --verbose for more " +
                            "information"
                    }
                    artifacts.forEach { artifact -> println(artifact.id.toString()) }
                }
        }
         ...
         */
        private fun resolveArtifacts(configuration: Configuration): ArtifactsResolutionResult {
            val artifacts =
         ...
         */
        private fun createKmpConfigurations(vararg dependencies: Dependency): List<Configuration> {
