...
class LocalMavenRepoDownloader(
    ...
    override fun onDownload(path: String, bytes: ByteArray) {
        if (path.substringAfterLast('.') in checksumExtensions) {
            ...
            logger.trace { "Skipping $path because we'll sign it locally" }
            return
        }
        val internal = isInternalArtifact(path)
        val folder = if (internal) internalFolder else externalFolder
        logger.trace { "Downloading $path. internal? $internal" }
        folder.resolve(path).let { file ->
            ...
            if (file.name.endsWith(".pom")) {
                ...
                copyWithDigest(targetFolder = targetFolder, fileName = file.name, bytes = bytes)
            }
        }
    }

    /** Creates the file in the given [targetFolder]/[fileName] pair. */
    private fun copyWithoutDigest(targetFolder: Path, fileName: String, bytes: ByteArray) {
     ...
     * sha1 checksums.
     */
    private fun copyWithDigest(targetFolder: Path, fileName: String, bytes: ByteArray) {
    ...
    private fun FileSystem.writeBytes(file: Path, contents: ByteArray) {
        writtenFilesMap.put(file.normalized(), true)
        file.parent?.let(fileSystem::createDirectories)
        write(file = file, mustCreate = false) { write(contents) }
        logger.info { "Saved ${file.normalized()} (${contents.size} bytes)" }
    }

    /**
     * Certain prebuilts might have improper downloads. This method traverses all folders into which
     * we've fetched an artifact and deletes files that are not fetched by us.
     *
        ...
        val folders =
            writtenFiles()
                .filter {
                    ...
                    !isDirectory && it.name.substringAfterLast(".") in EXTENSIONS_FOR_CLEANUP
                }
                .mapNotNull { it.parent }
                .distinct()
        logger.info { "Cleaning up local repository. Folders to clean: ${folders.size}" }

        // traverse all folders and make sure they are in the written files list
        folders.forEachIndexed { index, folder ->
            logger.trace { "Cleaning up $folder ($index of ${folders.size})" }
            fileSystem.list(folder).forEach { candidateToDelete ->
                if (!writtenFiles().contains(candidateToDelete.normalized())) {
                    logger.trace { "Deleting $candidateToDelete since it is not re-downloaded" }
                    fileSystem.delete(candidateToDelete)
                } else {
                    logger.trace { "Keeping $candidateToDelete" }
                }
            }
        }
    }
    ...
    private fun transformInternalPomFile(bytes: ByteArray): ByteArray {
        ...
        return bytes
            ...
            .lineSequence()
            .map { it.replace("<type>aar</type>", "<!--<type>aar</type>-->") }
            .joinToString("\n")
    ...
    companion object {
        ...
        /**
         * If we downloaded an artifact with one of these extensions, we can cleanup that folder for
         * files that are not re-downloaded.
         */
        private val EXTENSIONS_FOR_CLEANUP = listOf("jar", "aar", "klib")
        private val INTERNAL_ARTIFACT_PREFIXES =
            listOf("android/arch", "com/android/support", "androidx")

        // Need to exclude androidx.databinding
        private val FORCE_EXTERNAL_PREFIXES = setOf("androidx/databinding")

        /** Checks if an artifact is *internal*. */
        fun isInternalArtifact(path: String): Boolean {
            if (FORCE_EXTERNAL_PREFIXES.any { path.startsWith(it) }) {
                return false
            }
            return INTERNAL_ARTIFACT_PREFIXES.any { path.startsWith(it) }
        }
