...
class LicenseDownloaderTest {
    ...
    fun githubLicense() {
        ...
        val artifactPath =
            EnvironmentConfig.supportRoot /
                "../../prebuilts/androidx/external/" /
                "org/hamcrest/hamcrest-parent/1.3/hamcrest-parent-1.3.pom"
        val fetchedBytes =
            FileSystem.SYSTEM.read(artifactPath) {
                licenseDownloader.fetchLicenseFromPom(bytes = readByteArray())
            }
        assertThat(fetchedBytes).isNotNull()
    }
    ...
    private fun checkLocalLicense(localPom: Path, localLicense: Path) {
        val licenseDownloader = LicenseDownloader(enableGithubApi = false)
        val fetchedBytes =
            FileSystem.SYSTEM.read(localPom) {
                    licenseDownloader.fetchLicenseFromPom(bytes = readByteArray())
                }
                ?.toString(Charsets.UTF_8)
        val localBytes = FileSystem.SYSTEM.read(localLicense) { readUtf8() }
        assertThat(fetchedBytes).isEqualTo(localBytes)
    }
}
