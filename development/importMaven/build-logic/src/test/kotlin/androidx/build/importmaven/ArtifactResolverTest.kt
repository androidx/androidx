...
class ArtifactResolverTest {
    private val fakeFileSystem = FakeFileSystem().also { it.emulateUnix() }
    ...
    fun downloadAndroidXPrebuilt() {
        ArtifactResolver.resolveArtifacts(
            ...
            additionalRepositories = listOf(ArtifactResolver.createAndroidXRepo(8657806)),
            downloadObserver = downloader,
        )
        assertThat(fakeFileSystem.allPathStrings())
            .containsAtLeast(
        ...
        // into files.
        val roomRuntimeFiles =
            fakeFileSystem.list("/internal/androidx/room/room-runtime/2.5.0-SNAPSHOT/".toPath())
        assertWithMessage(roomRuntimeFiles.joinToString("\n") { it.toString() })
            .that(
                roomRuntimeFiles.any {
                    it.name.startsWith("room-runtime-") && it.name.endsWith("aar")
                }
            )
            .isTrue()
    }
    ...
    fun testAndroidArtifactsWithMetadata() {
        ...
        assertThat(fakeFileSystem.allPathStrings())
            .containsAtLeastElementsIn(
                "/internal/androidx/room/room-runtime/2.4.2/"
                    .toPath()
                    .expectedAar(signed = false, "room-runtime-2.4.2") +
                    "/internal/androidx/room/room-common/2.4.2/"
                        .toPath()
                        .expectedJar(signed = false, "room-common-2.4.2") +
                    "/internal/androidx/sqlite/sqlite-framework/2.2.0"
                        .toPath()
                        .expectedAar(signed = false, "sqlite-framework-2.2.0") +
                    "/internal/androidx/annotation/annotation/1.1.0"
                        .toPath()
                        .expectedFiles(
                            signed = false,
        ...
        assertThat(fakeFileSystem.allPathStrings())
            .doesNotContain("/internal/androidx/room/room-runtime/2.4.2/LICENSE")
    }
    ...
    fun ensureInternalPomsAreReWritten() {
        val bytes = this::class.java.getResourceAsStream("/pom-with-aar-deps.pom")!!.readBytes()
        downloader.onDownload("notAndroidx/subject.pom", bytes)
        val externalContents =
            fakeFileSystem
                .read("external/notAndroidx/subject.pom".toPath()) { this.readUtf8() }
                .lines()
                .map { it.trim() }
        ...
        val internalContents =
            fakeFileSystem
                .read("internal/androidx/subject.pom".toPath()) { this.readUtf8() }
                .lines()
                .map { it.trim() }
        ...
        val sha1 =
            LocalMavenRepoDownloader.digest(contents = bytes, fileName = "_", algorithm = "SHA1")
                .second
                .toString(Charsets.UTF_8)
        val md5 =
            LocalMavenRepoDownloader.digest(contents = bytes, fileName = "_", algorithm = "MD5")
                .second
                .toString(Charsets.UTF_8)
        assertThat(fakeFileSystem.readText("external/notAndroidx/subject.pom.md5")).isEqualTo(md5)
        assertThat(fakeFileSystem.readText("internal/androidx/subject.pom.md5")).isEqualTo(md5)
        assertThat(fakeFileSystem.readText("external/notAndroidx/subject.pom.sha1")).isEqualTo(sha1)
        assertThat(fakeFileSystem.readText("internal/androidx/subject.pom.sha1")).isEqualTo(sha1)
    }

    @Test fun testKotlinArtifactsWithKmp_fetchInherited() = testKotlinArtifactsWithKmp(true)

    @Test fun testKotlinArtifactsWithKmp() = testKotlinArtifactsWithKmp(false)
    ...
    private fun testKotlinArtifactsWithKmp(explicitlyFetchInherited: Boolean) {
        ...
        val expectedKlibs =
            listOf(
                ...
                .map { "/external/org/jetbrains/kotlinx/$it" }
        assertThat(fakeFileSystem.allPathStrings())
            .containsAtLeastElementsIn(
                "/external/org/jetbrains/kotlinx/kotlinx-coroutines-core/1.6.1"
                    .toPath()
                    .expectedFiles(
                        signed = true,
                    ...
                    "/external/org/jetbrains/kotlin/kotlin-stdlib-common/1.6.0"
                        .toPath()
                        .expectedFiles(
                            signed = true,
        ...
        val atomicFuJvmFiles =
            "/external/org/jetbrains/kotlinx/atomicfu-jvm/0.17.0"
                .toPath()
                .expectedJar(signed = true, "atomicfu-jvm-0.17.0")
        if (explicitlyFetchInherited) {
            assertThat(fakeFileSystem.allPathStrings()).containsAtLeastElementsIn(atomicFuJvmFiles)
        } else {
            assertThat(fakeFileSystem.allPathStrings()).doesNotContain(atomicFuJvmFiles)
        }
    }
    ...
    fun testSignatureFiles() {
        ...
        assertThat(fakeFileSystem.allPathStrings())
            .containsAtLeastElementsIn(
                "/external/org/jetbrains/kotlinx/kotlinx-coroutines-test-linuxx64/1.6.1/"
                    .toPath()
                    .expectedFiles(
                        signed = true,
    ...
    fun testSignedArtifactWithoutKeyServerEntry() {
        ...
        assertThat(fakeFileSystem.allPathStrings())
            .containsAtLeastElementsIn(
                "/external/org/assertj/assertj-core/3.11.1/"
                    .toPath()
                    .expectedFiles(
                        signed = true,
    ...
    fun noArtifactsAreFetchedWhenInternalRepoIsProvided() {
        val localRepoUris =
            EnvironmentConfig.supportRoot.let {
                listOf(
    ...
    fun testMetalavaDownload() {
        ArtifactResolver.resolveArtifacts(
            artifacts = listOf("com.android.tools.metalava:metalava:1.0.0-alpha06"),
            additionalRepositories = listOf(ArtifactResolver.createMetalavaRepo(8634556)),
            downloadObserver = downloader,
        )

        assertThat(fakeFileSystem.allPathStrings())
            .containsAtLeastElementsIn(
                "/external/com/android/tools/metalava/metalava/1.0.0-alpha06/"
                    .toPath()
                    .expectedJar(signed = false, "metalava-1.0.0-alpha06")
                    .filterNot {
    ...
    fun oldArtifactWithoutMetadata() {
        ArtifactResolver.resolveArtifacts(
            artifacts = listOf("androidx.databinding:viewbinding:4.1.2"),
            downloadObserver = downloader,
        )
        assertThat(fakeFileSystem.allPathStrings())
            .containsAtLeastElementsIn(
                "/external/androidx/databinding/viewbinding/4.1.2"
                    .toPath()
                    .expectedAar(signed = false, "viewbinding-4.1.2")
                    .filterNot {
    ...
    fun testRunner() {
        ArtifactResolver.resolveArtifacts(
            artifacts = listOf("androidx.test:runner:1.4.0"),
            downloadObserver = downloader,
        )
        assertThat(fakeFileSystem.allPathStrings())
            .containsAtLeastElementsIn(
                "/internal/androidx/test/runner/1.4.0/"
                    .toPath()
                    .expectedAar(signed = false, "runner-1.4.0")
                    .filterNot {
    ...
    fun isolateConfigurations() {
        ArtifactResolver.resolveArtifacts(
            listOf("androidx.room:room-runtime:2.4.2", "androidx.room:room-runtime:2.4.0"),
            downloadObserver = downloader,
        )
        assertThat(fakeFileSystem.allPathStrings())
            .containsAtLeastElementsIn(
                "/internal/androidx/room/room-common/2.4.2/"
                    .toPath()
                    .expectedJar(signed = false, "room-common-2.4.2") +
                    "/internal/androidx/room/room-common/2.4.0/"
                        .toPath()
                        .expectedJar(signed = false, "room-common-2.4.0")
            )
    }
    ...
    private fun FakeFileSystem.allPathStrings() = allPaths.map { it.toString() }

    private fun FileSystem.readText(path: String) =
        this.openReadOnly(path.toPath()).source().buffer().use { it.readUtf8() }

    /** Utility method to easily create files in a given folder path */
    private fun Path.expectedFiles(signed: Boolean, vararg fileNames: String): List<String> {
        val originals =
            if (signed) {
                fileNames.flatMap { listOf(it, "$it.asc") }
            } else {
                fileNames.toList()
            }

        return originals.map { "$this/$it" }.flatMap { listOf(it, "$it.md5", "$it.sha1") }
    }

    private fun Path.expectedAar(signed: Boolean, prefix: String) =
        expectedFiles(signed = signed, *COMMON_FILES_FOR_AARS.map { "$prefix$it" }.toTypedArray()) +
            expectedFiles(
    ...
    private fun Path.expectedJar(signed: Boolean, prefix: String) =
        expectedFiles(signed = signed, *COMMON_FILES_FOR_JARS.map { "$prefix$it" }.toTypedArray()) +
            expectedFiles(
                // gradle doesn't need the signature for pom when there is a module file.
                signed = false,
                "$prefix.pom",
