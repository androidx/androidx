...
/**
 * Main entry point for the A/B macrobenchmarking tool. This file is responsible for parsing
 * command-line arguments and orchestrating the overall workflow.
 */
fun main(args: Array<String>) {
    ...
    benchmarkRevA.forEach { (benchmarkName, benchmarkData) ->
        ...
        val dataB = benchmarkRevB[benchmarkName] ?: emptyMap()
        if (dataA.isNotEmpty() && dataB.isNotEmpty()) {
            val stats = calculateAllMetricsStatistics(dataA, dataB)
            printSummary(benchmarkName, stats)
            printCsvOutput(benchmarkName, stats)
            createAllMetricsHistogramPlots(benchmarkName, dataA, dataB, outputPath)
        } else {
            println(
