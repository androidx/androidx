 ...
 *
 * You can create an implementation of the class via [androidx.room3.Room.databaseBuilder] or
 ...
 * logical layer which will be much easier to stub in tests (compared to running direct SQL
 * queries). It also automatically does the conversion from the results of a
 * [androidx.sqlite.SQLiteStatement] to your application data classes so you don't need to deal with
 * lower level database APIs for most of your data access.
 *
 * Room also verifies all of your queries in [Dao] classes while the application is being compiled
 * so that if there is a problem in one of the queries, you will be notified during compilation.
 *
 * To automatically generate a migration between two versions of the database, assuming you have the
 * relevant schema files, you are recommended to use [AutoMigration] annotations. Note that if
 * [autoMigrations] is not empty, `exportSchema` must be `true`.
 *
...
public annotation class Database(
    /**
     * The list of entities included in the database. Each class represents a table in the database.
     *
    ...
    /**
     * The list of database views included in the database. Each class represents a view in the
     * database.
     *
     ...
     * have version history of your schema in your codebase. If you do export schemas then you
     * should commit the schema files into your version control system (but don't ship them with
     * your app!).
     *
     * When `room.schemaLocation` is set, Room will check this value and if it is set to `true`, the
     * database schema will be exported into the given folder.
     *
