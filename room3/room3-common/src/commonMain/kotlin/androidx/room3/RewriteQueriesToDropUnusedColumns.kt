 ...
 * [Query] functions such that only the columns that are used in the return type are queried from
 * the database.
 *
 ...
 * `lastName` fields in a data object class, you could write the query like this:
 * ```
 * @Dao
 * interface MyDao {
 *     @Query("SELECT * FROM User")
 *     suspend fun getAll(): List<NameAndLastName>
 * }
 *
 ...
 * additional columns that are not used in the return type response. You can annotate the function
 * with [RewriteQueriesToDropUnusedColumns] to inform Room to rewrite your query at compile time to
 ...
 *
 * When the annotation is used on a [Dao] function annotated with [Query], it will only affect that
 * query. You can put the annotation on the [Dao] annotated class / interface or the [Database]
 * annotated class where it will impact all functions in the DAO / database respectively.
 *
