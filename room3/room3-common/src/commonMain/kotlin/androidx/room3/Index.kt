 ...
 * `INSERT` or `UPDATE`. You should be careful when adding indices to ensure that this additional
 * cost is worth it.
 *
 * There are two ways to define an index in an [Entity]. You can either set [ColumnInfo.index]
 * property to index individual properties or define composite indices via [Entity.indices].
 *
 * If an indexed property is embedded into another entity via [Embedded], it is **NOT** added as an
 * index to the containing [Entity]. If you want to keep it indexed, you must re-declare it in the
 * containing [Entity].
 *
 * Similarly, if an [Entity] extends another class, indices from the super classes are **NOT**
 * inherited. You must re-declare them in the child [Entity] or set [Entity.inheritSuperIndices] to
 * `true`.
 *
 * See also [SQLite Index Documentation](https://sqlite.org/lang_createindex.html)
 */
...
public annotation class Index(
     ...
     *
     * The order of columns is important as it defines when SQLite can use a particular index. See
     * [SQLite documentation](https://www.sqlite.org/optoverview.html) for details on index usage in
     * the query optimizer.
     *
     ...
     * [SQLite documentation](https://www.sqlite.org/queryplanner.html#_sorting_by_index) for
     * details on sorting by index and the usage of the sort order by the query optimizer.
     *
     * As an example, consider a table called 'Song' with two columns, 'name' and 'length'. If a
     * covering index is created for it: `CREATE INDEX song_name_length on Song (name ASC, length
     * DESC)`, then a query containing an `ORDER BY` clause with matching order of the index will be
     * able to avoid a table scan by using the index, but a mismatch in order won't. Therefore, the
     * columns order of the index should be the same as the most frequently executed query with sort
     * order.
     *
