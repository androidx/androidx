...
/**
 * Creates a new TypeReference from [this] where the resolved type [replacement] but everything else
 * is the same (e.g. location).
 */
internal fun KSTypeReference.swapResolvedType(replacement: KSType): KSTypeReference {
    return DelegatingTypeReference(original = this, resolved = replacement)
}
...
private class DelegatingTypeReference(val original: KSTypeReference, val resolved: KSType) :
    KSTypeReference by original {
    override fun resolve() = resolved
}

private class NoLocationTypeReference(val resolved: KSType) : KSTypeReference {
