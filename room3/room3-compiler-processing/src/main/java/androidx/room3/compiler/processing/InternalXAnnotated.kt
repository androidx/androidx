...
internal interface InternalXAnnotated : XAnnotated {
     ...
     * this internal wrapper, we handle that inconsistency by finding the container class and asking
     * implementers to implement the 2 arg version instead.
     *
     * see: https://github.com/google/ksp/issues/358 see:
    ...
    override fun hasAnnotation(annotation: KClass<out Annotation>) =
        hasAnnotation(annotation = annotation, containerAnnotation = annotation.containerAnnotation)

    /**
     * Returns `true` if this element is annotated with the given [annotation].
     *
     * Note that this method should check for both [annotation] and [containerAnnotation] to support
