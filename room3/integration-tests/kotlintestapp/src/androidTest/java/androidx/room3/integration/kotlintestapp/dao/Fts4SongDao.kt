...
@Dao
interface Fts4SongDao {
    @Insert fun insert(song: Song)

    @Insert fun insert(songs: List<Song>)

    @Query("SELECT * FROM Fts4SongDescription WHERE Fts4SongDescription MATCH :searchQuery")
    fun getSongDescriptions(searchQuery: String): List<Fts4SongDescription>

    @Query(
        """
            SELECT s.mSongId, s.mTitle, s.mArtist, s.mAlbum, s.mLength, s.mReleasedYear
            FROM Song as s JOIN Fts4SongDescription as fts ON (docid = mSongId)
    ...
    @Query("SELECT * FROM Fts4SongDescription")
    fun getFlowSongDescription(): Flow<List<Fts4SongDescription>>
}
