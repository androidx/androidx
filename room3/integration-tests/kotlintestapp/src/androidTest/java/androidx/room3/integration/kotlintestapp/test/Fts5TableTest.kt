...
import androidx.room3.integration.kotlintestapp.Fts5TestDatabase
...
class Fts5TableTest {
    ...
    private lateinit var database: Fts5TestDatabase
    private lateinit var mailDao: Fts5MailDao
    private lateinit var songDao: Fts5SongDao

    @Before
    fun setup() {
        val context: Context = ApplicationProvider.getApplicationContext()
        database =
            Room.inMemoryDatabaseBuilder<Fts5TestDatabase>(context)
    ...
    @Test
    fun readWrite() {
        val item = TestUtil.createFts5Mail(1, "Hello old friend", "How are you? Wanna grab coffee?")
        mailDao.insert(item)
    ...
    fun prefixQuery() {
        val item1 =
            TestUtil.createFts5Mail(1, "Linux problem", "Hi - Need help with my linux machine.")
        val item2 =
            TestUtil.createFts5Mail(
                2,
                "Math help needed",
                "Anyone able to help me with linear algebra?",
            )
        val item3 = TestUtil.createFts5Mail(3, "Chef needed", "Need a cheeseburger check ASAP")
    ...
    fun prefixQuery_subject() {
        ...
        val item3 = TestUtil.createFts5Mail(3, "Chef needed", "Need a cheeseburger check ASAP")
        mailDao.insert(listOf(item1, item2, item3))
        val loaded = mailDao.getMail("subject:help algebra")
        assertThat(loaded).hasSize(1)
    ...
    fun highlightQuery() {
        ...
        mailDao.insert(listOf(item1, item2))
        val loaded = mailDao.getMailBodyHighlight("help")
        assertThat(loaded).hasSize(2)
        assertThat(loaded[0]).isEqualTo("Hi - Need <b>help</b> with my linux machine.")
        assertThat(loaded[1])
            .isEqualTo(
                "Hello dear friends. I am in desperate need for some <b>help</b>. " +
                    "I've taken a lot of tutorials online but I still don't understand. " +
                    "Is anyone available to please <b>help</b> with some linear algebra?"
            )
    }
