...
abstract class BaseConnectionPoolTest {
    ...
    fun readerIsReadOnlyConnection() = runTest {
        ...
        pool.useReaderConnection { connection ->
            assertThrows<SQLiteException> {
                    connection.executeSQL("INSERT INTO Pet (id, name) VALUES (100, 'Pelusa')")
                }
                .assertMsg()
            assertThrows<SQLiteException> { connection.executeSQL("CREATE TABLE Foo (id)") }
                .assertMsg()
            // Yup, temp tables even though local to a connection are considered write operations.
            assertThrows<SQLiteException> {
                    connection.executeSQL("CREATE TEMP TABLE TempFoo (id)")
                }
                .assertMsg()
            assertThrows<SQLiteException> { connection.executeSQL("PRAGMA user_version = 100") }
                .assertMsg()
    ...
    fun rollbackTransaction() = runTest {
        ...
        pool.useWriterConnection { connection ->
            connection.exclusiveTransaction {
                executeSQL("INSERT INTO Pet (id, name) VALUES (100, 'Pelusa')")
        ...
        pool.useWriterConnection { connection ->
            connection.exclusiveTransaction {
                ...
                withNestedTransaction<Unit> {
                    executeSQL("INSERT INTO Pet (id, name) VALUES (101, 'Tom')")
                    rollback(Unit)
                }
            }
            connection.usePrepared("SELECT * FROM Pet WHERE id = 100") {
        ...
        pool.useWriterConnection { connection ->
            connection.exclusiveTransaction {
                ...
                withNestedTransaction<Unit> {
                    executeSQL("INSERT INTO Pet (id, name) VALUES (101, 'Tom')")
                    withNestedTransaction {
                        executeSQL("INSERT INTO Pet (id, name) VALUES (102, 'Simba')")
                    }
                    rollback(Unit)
                }
            }
    ...
    fun connectionElementIsolation() = runTest {
        ...
        poolOne.useWriterConnection {
            it.executeSQL("CREATE TABLE Dog (id INTEGER, name TEXT, PRIMARY KEY(id))")
        }
        poolTwo.useWriterConnection {
            it.executeSQL("CREATE TABLE Cat (id INTEGER, name TEXT, PRIMARY KEY(id))")
        }
        poolOne.useWriterConnection { c1 ->
            c1.executeSQL("INSERT INTO Dog (id, name) VALUES (1, 'Frida')")
            poolTwo.useWriterConnection { c2 ->
                c2.executeSQL("INSERT INTO Cat (id, name) VALUES (1, 'Tom')")
            }
        }
    ...
    internal fun newSingleConnectionPool(driver: SQLiteDriver, fileName: String): ConnectionPool =
        newSingleConnectionPool(connectionFactory = { driver.open(fileName) })
