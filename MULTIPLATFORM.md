...
2. Publish core libraries
   ...
   ```bash
   ./gradlew :mpp:publishComposeJbToMavenLocal -Pcompose.platforms=all -Pjetbrains.publication.version.COMPOSE=999.0.0-alpha01+dev1000 -Pjetbrains.publication.version.LIFECYCLE=999.0.0-alpha01+dev1000
   ```
...
- Update `gradle.properties` by setting `compose.version` and `deploy.version` to the version you've published (999.0.0-alpha01+dev1000` in example above).
