...
/**
 * Platform-independent [MeshGradientRenderer] that tessellates a mesh gradient into a triangle
 * mesh.
 *
 * All of the tessellation math (Bezier surface evaluation, Catmull-Rom / bilinear color
 * interpolation, adaptive subdivision and buffer management) lives here and is shared by every
 * backend.
 *
 * Subclasses might be stateful and reuse the internal buffers across frames to avoid per-frame
 * allocations.
 */
internal abstract class BaseMeshGradientRenderer : MeshGradientRenderer {
    ...
    /**
     * Draws the tessellated triangle mesh. This is the only part of the render pipeline that
    ...
    /**
     * Allocates the per-vertex color buffer for [vertexCount] vertices. Backends that need a
     * different layout (e.g. Android pre-Q) may override this.
     */
    ...
    override fun DrawScope.draw(config: MeshGradientConfig) {
        ...
        val (subdivisionsU, subdivisionsV) =
            calculateMeshGradientSubdivisions(rows, columns, positions, size)
     ...
     * @param outPositions The output list to store the calculated
     *   [androidx.compose.ui.geometry.Offset] for each vertex.
     */
