 ...
 */
@Stable public fun Velocity(x: Float, y: Float): Velocity = Velocity(packFloats(x, y))

/** A two dimensional velocity in pixels per second. */
@Immutable
@kotlin.jvm.JvmInline
public value class Velocity internal constructor(private val packedValue: Long) {
    ...
    ): Velocity = Velocity(packFloats(x, y))
     ...
     */
    @Stable public operator fun unaryMinus(): Velocity = Velocity(packedValue xor DualFloatSignBit)
     ...
     */
    @Stable
    public operator fun times(operand: Float): Velocity =
        Velocity(
     ...
     */
    @Stable
    public operator fun div(operand: Float): Velocity =
        Velocity(
            packFloats(unpackFloat1(packedValue) / operand, unpackFloat2(packedValue) / operand)
        )
     ...
     */
    @Stable
    public operator fun rem(operand: Float): Velocity =
        Velocity(
            packFloats(unpackFloat1(packedValue) % operand, unpackFloat2(packedValue) % operand)
        )

    public override fun toString(): String = "($x, $y) px/sec"
}
