...
class TextLayoutFillBoundingBoxesTest {
    ...
    fun singleCharacter() {
        val text = "a"
        val layout = createTextLayout(text)
        assertThat(layout.getBoundingBoxes(0, text.length))
            .isEqualTo(ltrCharacterBoundariesForTestFont(text))
    }

    @Test
    fun singleCharacterLineHeight() {
        val text = "a"
        val layout = createTextLayout(text = text, lineSpacingMultiplier = 2f)
    ...
    fun singleCharacterRtl() {
        ...
        val layout = createTextLayout(text = text, width = width)

        assertThat(layout.getBoundingBoxes(0, text.length))
            .isEqualTo(rtlCharacterBoundariesForTestFont(text, width))
    }
    ...
    fun bidiLtrLine() {
        ...
        assertThat(layout.getBoundingBoxes(0, text.length))
            .isEqualTo(arrayOf(expected[0], expected[2], expected[1], expected[3]))
    }
    ...
    fun bidiRtlLine() {
        val text = "\u05D0" + "ab" + "\u05D1"
        val width = text.length * 2 * fontSize // a width wider than text
        val layout = createTextLayout(width = width, text = text)
    ...
    fun multiLineCenterAligned() {
        ...
        val layout =
            createTextLayout(text = text, width = width, alignment = LayoutCompat.ALIGN_CENTER)
    ...
    private fun FloatArray.asRectF(): Array<RectF> {
        return Array((size) / 4) { index ->
            RectF(this[4 * index], this[4 * index + 1], this[4 * index + 2], this[4 * index + 3])
        }
    }
