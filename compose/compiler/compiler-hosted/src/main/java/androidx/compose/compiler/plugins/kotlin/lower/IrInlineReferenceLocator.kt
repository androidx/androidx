...
class ComposeInlineLambdaLocator(private val context: IrPluginContext) {
    ...
    // We can't rely on the annotation on irFunction in FIR.
    fun isNonComposableInlineLambda(irFunction: IrFunction): Boolean =
        inlineLambdaToParameter[irFunction.symbol]?.type?.let {
...
fun IrType.isSyntheticComposableFunction() =
    classFqName?.asString()?.startsWith("androidx.compose.runtime.internal.ComposableFunction") == true
