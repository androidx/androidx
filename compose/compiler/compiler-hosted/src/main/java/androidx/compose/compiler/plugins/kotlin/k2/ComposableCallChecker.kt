...
object ComposableFunctionCallChecker : FirFunctionCallChecker() {
    ...
    private fun checkInvoke(
        ...
        val valueParameter = (expression.dispatchReceiver as? FirPropertyAccessExpression)
            ?.calleeReference
            ?.toResolvedValueParameterSymbol()
            ?: return
        if (valueParameter.resolvedReturnTypeRef.hasDisallowComposableCallsAnnotation(context.session) ||
            !valueParameter.containingFunctionSymbol.isInline) {
            return
        }
...
private fun CheckerContext.findValueParameterForLambdaAtIndex(elementIndex: Int): FirValueParameter? {
