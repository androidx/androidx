...
@Composable
fun RcSimpleClock1() {
    RemoteBox { RemoteText(text = RemoteString("Clock Placeholder")) }
}

@Composable
fun AiAgent() {
    RemoteBox { RemoteText(text = RemoteString("AI Agent Placeholder")) }
}

class MyWidget {
    @Composable
    fun Content(context: Context, widgetId: Int) {
        RemoteBox { RemoteText(text = RemoteString("Counter Widget Placeholder")) }
    }
}
...
fun MultiDocumentDemo() {
    ...
    UberDocumentCompositor(
        ...
        RemoteLayer(name = "DemoHomeScreen", visibility = 1) { DemoHomeScreen() },
    )
}
