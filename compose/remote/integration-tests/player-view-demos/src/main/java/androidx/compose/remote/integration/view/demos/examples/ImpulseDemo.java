...
/** Demos of impulse and particle systems */
@SuppressLint("RestrictedApiAndroidX")
public class ImpulseDemo {
    static AndroidxRcPlatformServices sPlatform = new AndroidxRcPlatformServices();
    ...
    /**
     * @return The impluse doc
     */
    @SuppressLint("RestrictedApiAndroidX")
    public static @NonNull RemoteComposeWriter balloonDemo(@NonNull Bitmap image) {
        RemoteComposeWriterAndroid rcDoc =
                new RemoteComposeWriterAndroid(300, 300, "HeartsDemo", 6,
                0, sPlatform);
        rcDoc.root(
                () -> {
                    rcDoc.startBox(
                    ...
                    //   float rad = rcDoc.floatExpression(RemoteContext.FLOAT_WINDOW_HEIGHT, 0
                    //   .3f, MUL);
                    ...
                    rcDoc.drawRoundRect(
                            0, 0, FLOAT_WINDOW_WIDTH, RemoteContext.FLOAT_WINDOW_HEIGHT, 72, 72);
                    ...
                    float ty =
                            rcDoc.addTouch(
                                    ...
                                    RemoteComposeWriter.STOP_ABSOLUTE_POS,
                                    0,
                                    0,
                                    null,
                                    null,
                    ...
                    RemotePath circle =
                            circleEngine(rcDoc, imgTop, imgLeft, imgRight, imgBottom, false);

                    RemotePath circle2 =
                            circleEngine(rcDoc, imgTop, imgLeft, imgRight, imgBottom, true);
                    //                    rcDoc.impulse(1, event);
                    //                    rcDoc.impulseProcess();
                    //
                    rcDoc.getPainter().setColor(Color.BLUE).setStyle(Paint.Style.FILL).commit();
                    rcDoc.drawCircle(tx, ty, 40);
    ...
    @SuppressLint("RestrictedApiAndroidX")
    static @NonNull RemotePath ballon() {
    ...
    @SuppressLint("RestrictedApiAndroidX")
    static void drawBalloon(@NonNull RemoteComposeWriter rcDoc, float cx, float cy, float dx,
            float dy) {
    ...
    static void balloons(RemoteComposeWriterAndroid rcDoc, float event, float tx, float ty) {
        {
            ...
            float ps =
                    rcDoc.createParticles(
                            variables,
                            new float[][]{
                                    {FLOAT_WINDOW_WIDTH, RAND, MUL}, // x
            ...
            //            float tx1  = variables[6];
            //            float ty1  = variables[7];
            //            float tx2  = variables[8];
            //            float ty2  = variables[9];
            rcDoc.getPainter().setColor(Color.RED).setStyle(Paint.Style.FILL).commit();
            rcDoc.impulseProcess(); // current_time - envnt_time  < 0.5
            //  float frac = rcDoc.floatExpression(RemoteContext.FLOAT_ANIMATION_TIME, event, SUB);
            // float alpha = rcDoc.floatExpression(1, frac, SUB);
            // rcDoc.getPainter().setAlpha(alpha).commit();
            rcDoc.particlesLoop(
                    ps,
                    new float[]{y, -1, MUL},
                    new float[][]{
                            {x, dx, FLOAT_ANIMATION_DELTA_TIME, MUL, ADD},
                            {y, dy, FLOAT_ANIMATION_DELTA_TIME, MUL, ADD},
                            {cx, x, SUB, RAND, MUL, 0.01f, MUL, dx, ADD},
                            {dy},
                            {h},
                            {alpha, 0.999f, MUL}
                    },
            ...
            //                    float alpha = rcDoc.floatExpression(1, frac, SUB);
            //                    rcDoc.getPainter().setAlpha(alpha).commit();
            //                    float rad = rcDoc.floatExpression(frac, 1,MOD ,cx, MUL);
            //                    rcDoc.drawCircle(tx, ty, rad);
            rcDoc.impulseEnd();
            rcDoc.impulseEnd();
        }
    }

    /** Create something that approaximates a circular shape */
    @SuppressLint("RestrictedApiAndroidX")
    static RemotePath circleEngine(
            RemoteComposeWriter rc,
            float top,
            float left,
            float right,
            float bottom,
        ...
        // c.addArc(top, left, right, bottom, 0, 360);
        rp.close();
        return rp;
    }
                ...
                () -> {
                    ...
                    float anim =
                            rcDoc.floatExpression(
                                    RemoteContext.FLOAT_CONTINUOUS_SEC, 2, MOD, 1, SUB);
                    float ty =
                            rcDoc.addTouch(
                                    cy,
                                    0,
                    ...
                    float event = RemoteContext.FLOAT_TOUCH_EVENT_TIME;
                    // float event = rcDoc.floatExpression(RemoteContext.FLOAT_ANIMATION_TIME, 3,
                    // MOD );
    ...
    @SuppressLint("RestrictedApiAndroidX")
    static void hearts(@NonNull RemoteComposeWriterAndroid rcDoc, float event, float tx, float ty) {
        {
            rcDoc.getPainter().setTextSize(64f).commit();
                    ...
                    rcDoc.createParticles(
                            ...
                            new float[][]{
                                    ...
                                    {ty, RAND, 300f, MUL, 150, SUB, ADD},
                                    {
                                            ...
                                            10f,
                                            MUL,
                                            RemoteContext.FLOAT_TOUCH_VEL_X,
                                            ADD
                                    },
                                    {
                                            RAND,
                                            ...
                                            10f,
                                            MUL,
                                            RemoteContext.FLOAT_TOUCH_VEL_Y,
                                            ADD
                                    },
                                    {RAND, 2, MUL},
                                    {1f}
            ...
            rcDoc.particlesLoop(
                    ps,
                    new float[]{y, RemoteContext.FLOAT_WINDOW_HEIGHT, SUB},
                    new float[][]{
                            {x, dx, ADD},
                            {y, dy, ADD},
                            {dx},
                            {dy, 9.8f, FLOAT_ANIMATION_DELTA_TIME, MUL, ADD},
                            {h},
                            {alpha, 0.99f, MUL}
                    },
                    () -> {
    ...
    static void confettiEngine(RemoteComposeWriterAndroid rcDoc, float event, float width,
            float height) {
        {
            ...
            rcDoc.getPainter()
                    ...
                    .commit();
//            int ballId = rcDoc.addBitmap(sBall);
...
//            float frac =
//                    rcDoc.floatExpression(
//                            RemoteContext.FLOAT_ANIMATION_TIME, event, SUB, 10, ADD, 6, MUL);
            // rcDoc.drawCircle(200,200,frac);

            rcDoc.particlesLoop(
                    ps,
                    new float[]{y, height, SUB},
                    new float[][]{
                            {x, dx, dt, MUL, ADD},
                            {y, dist, dt, 10, MUL, MUL, ADD},
                            {dx},
                            {dy, 98f, dist, ADD, dt, MUL, 2, MUL, ADD},
                            {dist},
                            {alpha}
                    },
                    () -> {
