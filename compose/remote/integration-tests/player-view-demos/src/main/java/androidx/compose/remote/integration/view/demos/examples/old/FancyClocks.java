...
/**
 * A collection of fancy clock demos using procedural animations.
 */
...
public class FancyClocks {
    ...
    /**
     * Creates a fancy clock demo with animated colors and gradients.
     *
     * @return a {@link RemoteComposeWriter} containing the drawing commands.
     */
    public static @NonNull RemoteComposeWriter fancyClock1() {
        ...
        rc.root(() -> {
            ...
            rc.rotate(a2, cx, cy);
            float hue = rc.floatExpression(FLOAT_CONTINUOUS_SEC, 5, DIV, 1, MOD);
            short col1 = rc.addColorExpression(255, hue, 0.66f, 0.28f);
            short col2 = rc.addColorExpression(255, hue, 0.43f, 0.53f);
            short col1a = rc.addColorExpression(128, hue, 0.66f, 0.28f);
            short col2a = rc.addColorExpression(128, hue, 0.43f, 0.53f);
            rc.getPainter().setSweepGradient(cx, cy,
                    new int[]{col2, col1, col1, col1, col2, col1, col2}, 127, null).commit();
            ...
            rc.getPainter().setSweepGradient(cx, cy,
                    new int[]{col2a, col1a, col1a, col1a, col2a, col1a, col2a}, 127,
                    null).commit();
    ...
    /**
     * Creates a fancy clock demo where the base color is based on the theme.
     *
     * @return a {@link RemoteComposeWriter} containing the drawing commands.
     */
    public static @NonNull RemoteComposeWriter fancyClock3() {
        RemoteComposeWriterAndroid rc = new RemoteComposeWriterAndroid(500, 500, "sd", 6, 0,
                sPlatform);
        ...
        rc.root(() -> {
            ...
            int baseColor = rc.addColor(0xFFFFFF00);
            rc.setColorName(baseColor, "android.colorAccent");
            int pat2 = genPath(rc, cx, cy, clipRad1, rad2);
            rc.addClipPath(pat2);
            float hue = rc.getColorAttribute(baseColor, ColorAttribute.COLOR_HUE);
            float sat = rc.getColorAttribute(baseColor, ColorAttribute.COLOR_SATURATION);

            rc.save();
            float a2 = rc.floatExpression(FLOAT_CONTINUOUS_SEC, 360f, MOD);
            rc.rotate(a2, cx, cy);
            // float hue = rc.floatExpression(FLOAT_CONTINUOUS_SEC, 5, DIV, 1, MOD);
            short col1 = rc.addColorExpression(255, hue, sat, 0.28f);
            short col2 = rc.addColorExpression(255, hue, sat, 0.53f);
            short col1a = rc.addColorExpression(128, hue, sat, 0.28f);
            short col2a = rc.addColorExpression(128, hue, sat, 0.53f);
            rc.getPainter().setSweepGradient(cx, cy,
                    new int[]{col2, col1, col1, col1, col2, col1, col2}, 127, null).commit();
...
//            float angDeg = rc.floatExpression(second, 6, MUL);
    ...
    static void drawClock(RemoteComposeWriterAndroid rc, float centerX, float centerY) {
        float secondAngle = rc.floatExpression(FLOAT_CONTINUOUS_SEC, 60f, MOD, 6f, MUL);
    ...
    /**
     * Creates a fancy clock demo with a fixed layout and sweep gradients.
     *
     * @return a {@link RemoteComposeWriter} containing the drawing commands.
     */
