...
@Composable
@Preview
fun RcDrawWithContentPreview() {
    RemoteDocumentPreview(RcDrawWithContent())
}

@Suppress("RestrictedApiAndroidX")
fun RcDrawWithContent(): RemoteComposeContext {
    return RemoteComposeContextAndroid(
        ...
        root {
            column(Modifier.fillMaxSize().background(Color.WHITE).padding(60)) {
                box(
                    Modifier.size(400).drawWithContent {
                        // Draw background
                        painter.setColor(Color.LTGRAY).commit()
                        drawRect(0f, 0f, 400f, 400f)
                        ...
                        drawCircle(200f, 200f, 50f)
                    }
                ) {
                    box(Modifier.fillMaxSize().padding(100).background(Color.BLUE))
                }
            }
        }
    }
