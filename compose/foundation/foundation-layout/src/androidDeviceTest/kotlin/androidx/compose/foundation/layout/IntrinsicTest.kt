...
class IntrinsicTest : LayoutTest() {
    ...
    @Test
    fun testIntrinsicWidth_skippedWhenIncomingWidthIsFixed() =
        with(density) {
            val positionedLatch = CountDownLatch(2)
            val minMeasuredSize = Ref<IntSize>()
            val maxMeasuredSize = Ref<IntSize>()
            val minMeasurePolicy = IntrinsicCallCountingMeasurePolicy()
            val maxMeasurePolicy = IntrinsicCallCountingMeasurePolicy()

            show {
                Column {
                    Layout(
                        modifier =
                            Modifier.width(50.dp).width(IntrinsicSize.Min).onGloballyPositioned {
                                minMeasuredSize.value = it.size
                                positionedLatch.countDown()
                            },
                        measurePolicy = minMeasurePolicy,
                    )
                    Layout(
                        modifier =
                            Modifier.width(50.dp).width(IntrinsicSize.Max).onGloballyPositioned {
                                maxMeasuredSize.value = it.size
                                positionedLatch.countDown()
                            },
                        measurePolicy = maxMeasurePolicy,
                    )
                }
            }

            assertTrue(positionedLatch.await(1, TimeUnit.SECONDS))
            assertEquals(0, minMeasurePolicy.totalCalls)
            assertEquals(0, maxMeasurePolicy.totalCalls)
            assertEquals(IntSize(50.dp.roundToPx(), 10.dp.roundToPx()), minMeasuredSize.value)
            assertEquals(IntSize(50.dp.roundToPx(), 10.dp.roundToPx()), maxMeasuredSize.value)
        }

    @Test
    fun testIntrinsicWidth_notSkippedWhenIncomingWidthIsNotFixed() =
        with(density) {
            val positionedLatch = CountDownLatch(2)
            val minMeasuredSize = Ref<IntSize>()
            val maxMeasuredSize = Ref<IntSize>()
            ...
            show {
                Column(Modifier.widthIn(min = 10.dp, max = 100.dp)) {
                    Layout(
                        modifier =
            ...
            assertTrue(positionedLatch.await(1, TimeUnit.SECONDS))
            assertEquals(1, minMeasurePolicy.totalCalls)
            assertEquals(1, maxMeasurePolicy.totalCalls)
            assertEquals(IntSize(20.dp.roundToPx(), 10.dp.roundToPx()), minMeasuredSize.value)
            assertEquals(IntSize(20.dp.roundToPx(), 10.dp.roundToPx()), maxMeasuredSize.value)
        }

    @Test
    fun testIntrinsicHeight_skippedWhenIncomingHeightIsFixed() =
        with(density) {
            val positionedLatch = CountDownLatch(2)
            val minMeasuredSize = Ref<IntSize>()
            val maxMeasuredSize = Ref<IntSize>()
            ...
            show {
                Column {
                    Layout(
                        modifier =
                            Modifier.height(50.dp).height(IntrinsicSize.Min).onGloballyPositioned {
                                minMeasuredSize.value = it.size
                                positionedLatch.countDown()
                            },
                        measurePolicy = minMeasurePolicy,
                    ...
                    Layout(
                        modifier =
                            Modifier.height(50.dp).height(IntrinsicSize.Max).onGloballyPositioned {
                                maxMeasuredSize.value = it.size
                                positionedLatch.countDown()
                            },
                        measurePolicy = maxMeasurePolicy,
            ...
            assertEquals(0, minMeasurePolicy.totalCalls)
            assertEquals(0, maxMeasurePolicy.totalCalls)
            assertEquals(IntSize(10.dp.roundToPx(), 50.dp.roundToPx()), minMeasuredSize.value)
            assertEquals(IntSize(10.dp.roundToPx(), 50.dp.roundToPx()), maxMeasuredSize.value)
        }

    @Test
    fun testIntrinsicHeight_notSkippedWhenIncomingHeightIsNotFixed() =
        with(density) {
            val positionedLatch = CountDownLatch(2)
            val minMeasuredSize = Ref<IntSize>()
            val maxMeasuredSize = Ref<IntSize>()
            ...
            show {
                Row(Modifier.heightIn(min = 10.dp, max = 100.dp)) {
                    Layout(
                        modifier =
            ...
            assertEquals(1, minMeasurePolicy.totalCalls)
            assertEquals(1, maxMeasurePolicy.totalCalls)
            assertEquals(IntSize(10.dp.roundToPx(), 20.dp.roundToPx()), minMeasuredSize.value)
            assertEquals(IntSize(10.dp.roundToPx(), 20.dp.roundToPx()), maxMeasuredSize.value)
        }

    @Test
    fun testRequiredWidth_notSkippedWhenIncomingWidthIsFixed() =
        with(density) {
            val positionedLatch = CountDownLatch(2)
            val minMeasurePolicy = IntrinsicCallCountingMeasurePolicy()
            val maxMeasurePolicy = IntrinsicCallCountingMeasurePolicy()
    ...
    @Test
    fun testRequiredHeight_notSkippedWhenIncomingHeightIsFixed() =
        with(density) {
            val positionedLatch = CountDownLatch(2)
            val minMeasurePolicy = IntrinsicCallCountingMeasurePolicy()
            val maxMeasurePolicy = IntrinsicCallCountingMeasurePolicy()
...
/**
 * A [MeasurePolicy] used to verify whether intrinsic measurements were queried.
 *
 * It reports intrinsic dimensions of [intrinsicWidth] x [intrinsicHeight] (default 20.dp x 20.dp)
 * and measures content to [measuredWidth] x [measuredHeight] (default 10.dp x 10.dp), constrained
 * by incoming constraints.
 */
private class IntrinsicCallCountingMeasurePolicy(
    private val measuredWidth: Dp = 10.dp,
    private val measuredHeight: Dp = 10.dp,
    private val intrinsicWidth: Dp = 20.dp,
    private val intrinsicHeight: Dp = 20.dp,
) : MeasurePolicy {
    var minWidthCalls = 0
    var maxWidthCalls = 0
    var minHeightCalls = 0
    var maxHeightCalls = 0

    val totalCalls: Int
        get() = minWidthCalls + maxWidthCalls + minHeightCalls + maxHeightCalls
    ...
    override fun MeasureScope.measure(
        ...
        layout(
            constraints.constrainWidth(measuredWidth.roundToPx()),
            constraints.constrainHeight(measuredHeight.roundToPx()),
        ) {}

    override fun IntrinsicMeasureScope.minIntrinsicWidth(
        measurables: List<IntrinsicMeasurable>,
        height: Int,
    ): Int = intrinsicWidth.roundToPx().also { minWidthCalls++ }

    override fun IntrinsicMeasureScope.maxIntrinsicWidth(
        measurables: List<IntrinsicMeasurable>,
        height: Int,
    ): Int = intrinsicWidth.roundToPx().also { maxWidthCalls++ }

    override fun IntrinsicMeasureScope.minIntrinsicHeight(
        measurables: List<IntrinsicMeasurable>,
        width: Int,
    ): Int = intrinsicHeight.roundToPx().also { minHeightCalls++ }

    override fun IntrinsicMeasureScope.maxIntrinsicHeight(
        measurables: List<IntrinsicMeasurable>,
        width: Int,
    ): Int = intrinsicHeight.roundToPx().also { maxHeightCalls++ }
}
