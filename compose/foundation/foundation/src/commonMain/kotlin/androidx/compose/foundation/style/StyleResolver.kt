 ...
 */
@ExperimentalFoundationStyleApi
public sealed class StyleResolver : RememberObserver {
    /**
    ...
    @PublishedApi internal abstract val accessorScope: StyleResolverScope

    internal abstract fun bind(node: StyleResolverNode)

    internal abstract fun unbind()

    internal abstract fun actual(): StyleResolverImpl

    internal abstract val oldStyle: Style?
 ...
 */
@ExperimentalFoundationStyleApi
public fun StyleResolver(
    style: CommonStyle,
    styleState: StyleState = MutableStyleState(null),
): StyleResolver = StyleResolverImpl(style, styleState)

/**
 * Transitionary resolver. Do not use. This will be removed before the Styles API is stable.
 *
 * A proxy resolver to allow binary compatibility with the previous implementation.
 */
@ExperimentalFoundationStyleApi
public fun ProxyStyleResolver(style: Style, styleState: StyleState? = null): StyleResolver =
    ProxyStyleResolverImpl(style, styleState)

@ExperimentalFoundationStyleApi
internal class ProxyStyleResolverImpl(
    internal val style: Style,
    internal val styleState: StyleState?,
) : StyleResolver() {
    private var proxy: StyleResolverImpl? = null

    override val accessorScope: StyleResolverScope
        get() = proxyRequired().accessorScope

    override fun actual(): StyleResolverImpl = proxyRequired()

    override val oldStyle = style

    override fun bind(node: StyleResolverNode) {
        when (val resolver = node.styleResolverField) {
            is ProxyStyleResolverImpl -> {
                buildNodeResolver(node)
            }
            is StyleResolverImpl -> {
                if (
                    (styleState != null && resolver.styleState != styleState) ||
                        resolver.oldStyle !== style
                ) {
                    buildNodeResolver(node)
                } else {
                    proxy = resolver
    ...
    private fun proxyRequired(): StyleResolverImpl =
        proxy ?: error("Cannot resolve an unbound proxy scope")

    private fun buildNodeResolver(node: StyleResolverNode) {
        val newState = styleState ?: MutableStyleState(null)
        val newStyle = style.toCommonStyle()
        val newResolver = StyleResolverImpl(newStyle, newState)
        newResolver.bind(node)
        node.styleResolverField = newResolver
        node.oldStyle = style
        proxy = newResolver
    }
}
...
@ExperimentalFoundationStyleApi
internal class StyleResolverImpl(
    ...
    override fun actual(): StyleResolverImpl = this
                    ...
                    if (isLocal) {
                        if (this in resolvedLocals) {
                            ...
                            node.traverseAncestors(StyleResolverNodeKey) {
                                ...
                                result = this@isSet in it.styleResolver.actual().resolvedLocals
                                !result
                            }
                            result
                        }
            ...
            override fun anySet(properties: Set<StyleProperty<*>>): Boolean {
                ...
                if (!result && anyLocals) {
                    node.traverseAncestors(StyleResolverNodeKey) { ancestor ->
                        ...
                        result =
                            properties.any { it in ancestor.styleResolver.actual().resolvedLocals }
                        !result
                    }
                }
                return result
    ...
    internal fun <T> resolveLocal(local: StyleProperty<T>): T? {
        ...
        node.traverseAncestors(StyleResolverNodeKey) {
            if (it !is StyleResolverNode) return@traverseAncestors true
            result = it.styleResolver.actual().resolvedLocals.getOrNull(local)
            result == null
        }
        return result
    }
