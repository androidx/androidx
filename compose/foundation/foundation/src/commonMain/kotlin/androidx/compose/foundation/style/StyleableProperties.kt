...
internal class StyleableProperties {
    ...
    internal fun copy(): StyleableProperties = StyleableProperties().also { copyInto(it) }

    internal fun copyInto(target: StyleableProperties) {
    ...
    internal fun copyInto(target: StyleableProperties, flags: Int) {
    ...
    internal fun copyInto(target: StyleableProperties, primitivesFilter: Long, objectsFilter: Int) {
        val primitivesSet = primitivesSet and primitivesFilter
        if (primitivesSet != 0L) {
    ...
    internal fun diffPrimitives(other: StyleableProperties, filter: Long = -1L): Long {
    ...
    internal fun diffObjects(other: StyleableProperties, filter: Int = -1): Int {
    ...
    /**
     * Given another StyleableProperties instance, this function will return a bitmask of the
     * property phases should be invalidated due to the differences between the two.
     *
     ...
     */
    internal fun diff(other: StyleableProperties, filterFlags: Int = PhaseFlagMask): Int {
...
internal fun lerpOuterLayout(
    primitivesSet: Long,
    a: StyleableProperties,
    b: StyleableProperties,
    animations: StyleableAnimations,
    result: StyleableProperties,
) {
    with(result) {
        if (primitivesSet.hasId(ExternalPaddingStartId)) {
...
@ExperimentalFoundationStyleApi
private inline fun <T> StyleableProperties.animateAB(
    a: StyleableProperties,
    b: StyleableProperties,
    propertyId: Byte,
    primitivesSet: Long,
    animations: StyleableAnimations,
    get: StyleableProperties.() -> T,
    set: StyleableProperties.(T) -> Unit,
) {
...
@ExperimentalFoundationStyleApi
internal val EmptyStyleProperties = StyleableProperties()
