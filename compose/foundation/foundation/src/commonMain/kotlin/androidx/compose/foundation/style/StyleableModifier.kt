...
internal class StyleOuterNode(
    ...
    // StyleableProperties instances are expensive to allocate. We will always need at least one, so
    // we
    // go ahead and allocate that here. This is a pointer to the "currently valid StyleProperties"
    // of
    // this node, however we also store a StyleableProperties in _bufferOrNull that we flip
    ...
    // until we need it.
    private var _resolved = StyleableProperties()
    private var _bufferOrNull: StyleableProperties? = null
    private val bufferNonNull: StyleableProperties
        get() {
            if (_bufferOrNull == null) _bufferOrNull = StyleableProperties()
    ...
    internal fun resolveAnimatedStyleFor(
        flags: Int,
        base: StyleableProperties = _resolved,
    ): StyleableProperties {
        return if (builder.animatingFlags and flags != 0) {
            val properties = StyleableProperties()
    ...
    fun ContentDrawScope.drawInnerShadow(resolved: StyleableProperties) {
    ...
    fun ContentDrawScope.drawDropShadow(resolved: StyleableProperties) {
...
private inline fun <T> StyleableProperties.hasOrElse(
    propertyId: Byte,
    value: T,
    read: StyleableProperties.() -> T,
) = if (hasId(propertyId)) read() else value

private inline fun StyleableProperties.hasOrZero(
    propertyId: Byte,
    read: StyleableProperties.() -> Float,
) = if (hasId(propertyId)) read() else 0f

private inline fun StyleableProperties.hasOrOne(
    propertyId: Byte,
    read: StyleableProperties.() -> Float,
) = if (hasId(propertyId)) read() else 1f

private inline fun <T : Any> StyleableProperties.hasOrNull(
    propertyId: Int,
    read: StyleableProperties.() -> T,
...
private fun StyleableProperties.shouldPlaceRelativeToRight() = !hasId(LeftId) && hasId(RightId)

private fun StyleableProperties.shouldPlaceRelativeToBottom() = hasId(BottomId) && !hasId(TopId)
