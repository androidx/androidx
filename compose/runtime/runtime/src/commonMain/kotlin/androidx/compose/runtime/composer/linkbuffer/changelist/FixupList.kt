...
import androidx.compose.runtime.composer.linkbuffer.changelist.Operation.InsertNodeFixup
import androidx.compose.runtime.composer.linkbuffer.changelist.Operation.PostInsertNodeFixup
...
internal class FixupList : DebugStringFormattable() {
    ...
    fun createAndInsertNode(factory: () -> Any?, insertIndex: Int, groupHandle: GroupHandle) {
        operations.push(InsertNodeFixup) {
            setObject(InsertNodeFixup.Factory, factory)
            setInt(InsertNodeFixup.InsertIndex, insertIndex)
            setLong(InsertNodeFixup.GroupHandleHigh, InsertNodeFixup.GroupHandleLow, groupHandle)
        }

        pendingOperations.push(PostInsertNodeFixup) {
            setInt(PostInsertNodeFixup.InsertIndex, insertIndex)
            setLong(
                PostInsertNodeFixup.GroupHandleHigh,
                PostInsertNodeFixup.GroupHandleLow,
                groupHandle,
            )
        }
    }

    fun createAndInsertNodeByAnchor(factory: () -> Any?, insertIndex: Int, anchor: LinkAnchor) {
        operations.push(Operation.InsertNodeFixupByAnchor) {
            setObject(Operation.InsertNodeFixupByAnchor.Factory, factory)
            setInt(Operation.InsertNodeFixupByAnchor.InsertIndex, insertIndex)
            setObject(Operation.InsertNodeFixupByAnchor.Anchor, anchor)
        }

        pendingOperations.push(Operation.PostInsertNodeFixupByAnchor) {
            setInt(Operation.PostInsertNodeFixupByAnchor.InsertIndex, insertIndex)
            setObject(Operation.PostInsertNodeFixupByAnchor.Anchor, anchor)
        }
    }
