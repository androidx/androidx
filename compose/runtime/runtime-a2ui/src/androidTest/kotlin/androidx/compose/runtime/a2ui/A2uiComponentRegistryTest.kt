...
class A2uiComponentRegistryTest {
    ...
    @Test
    fun observe_unknownComponent_registersDependencyAndUpdatesWhenLoaded() =
    ...
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun observe_componentUpdate_triggersRecomposition() =
        runComposeUiTest(effectContext = StandardTestDispatcher()) {
    ...
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun observe_componentError_triggersRecomposition() =
        runComposeUiTest(effectContext = StandardTestDispatcher()) {
            val componentId = "error_prone_comp"
            registry.update(
                listOf(
                    A2uiComponentPayload(componentId, "Text", mapOf("text" to "Initial Content"))
                )
            )
    ...
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun observe_identicalComponentUpdate_doesNotTriggerRecomposition() =
        runComposeUiTest(effectContext = StandardTestDispatcher()) {
            registry.update(
                listOf(A2uiComponentPayload("comp_1", "Text", mapOf("text" to "Initial")))
            )
    ...
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun observe_updateUnrelatedComponent_doesNotTriggerRecomposition() =
        runComposeUiTest(effectContext = StandardTestDispatcher()) {
            registry.update(
                listOf(
