...
internal enum class ChildWeightPreset(val label: String) {
    NONE("None (unweighted)"),
    EQUAL("Equal (1 : 1)"),
    WEIGHTED("Weighted (2 : 1)"),
}

@Composable
internal fun RowSample(
    ...
    Column(modifier = modifier.fillMaxWidth()) {
        ...
        ControlCard(
            title = "Vertical Alignment (align)",
            subtitle = "Alignment of children along the vertical cross axis",
        ) {
            ChoiceChips(
        ...
        ControlCard(
            title = "Child Weights",
            subtitle = "Relative proportional weight applied to child buttons",
        ) {
            ChoiceChips(
...
private fun updatePayload(
    ...
    val childIds = (1..itemCount).map { "btn$it" }
