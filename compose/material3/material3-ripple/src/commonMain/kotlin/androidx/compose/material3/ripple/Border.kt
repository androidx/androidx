...
internal class BorderLogic {
    ...
    private var borderInset: (() -> Dp)? = null
    ...
    internal fun drawBorder(
        ...
        inset: () -> Dp,
        brush: Brush,
        // We will sometimes create a graphics layer and use it for caching if necessary,
        // the caller if responsible for the lifecycle.
    ...
    private inline fun DrawScope.strokeWidthPx(): Float {
        ...
        return min(
                ...
                ceil((size.minDimension - strokeInsetPx() * 2) / 2),
            )
            .coerceAtLeast(0f)
    }

    private inline fun DrawScope.strokeInsetPx(): Float {
        return ceil(borderInset!!.invoke().toPx())
    }
     ...
     */
    private inline fun topLeft(strokeWidthPx: Float, strokeInsetPx: Float): Offset {
        val halfStroke = strokeWidthPx / 2
        return Offset(halfStroke + strokeInsetPx, halfStroke + strokeInsetPx)
    }
     ...
     */
    private inline fun DrawScope.borderSize(strokeWidthPx: Float, strokeInsetPx: Float): Size {
        return Size(
            size.width - strokeWidthPx - strokeInsetPx * 2,
            size.height - strokeWidthPx - strokeInsetPx * 2,
        )
    }
     ...
     *   stroke).
     */
    private inline fun DrawScope.fillArea(strokeWidthPx: Float, strokeInsetPx: Float): Boolean {
        // The stroke is larger than the drawing area so just draw a full shape instead
        return strokeInsetPx == 0f && (strokeWidthPx * 2) > size.minDimension
    }
    ...
    private fun createDrawGenericBorder(
        ...
        return {
            ...
            val strokeInset = strokeInsetPx()
            val fillArea = fillArea(strokeWidth, strokeInset)
            if (fillArea) {
                drawPath(outline.path, brush = brush)
            } else {
                translate(pathBounds.left, pathBounds.top) {
                    with(borderCache!!) {
                        drawBorderCache(pathBoundsSize, graphicsLayerProvider) {
                            ...
                            translate(-pathBounds.left, -pathBounds.top) {
                                ...
                                drawPath(
                                    path = outline.path,
                                    brush = brush,
                                    style = Stroke((strokeWidth + strokeInset) * 2),
                                )

                                drawPath(
                                    path = outline.path,
                                    brush = brush,
                                    style = Stroke(strokeInset * 2),
    ...
    private fun createDrawRoundRectBorder(
        ...
        if (outline.roundRect.isSimple) {
            return {
                val strokeWidth = strokeWidthPx()
                val strokeInset = strokeInsetPx()
                val topLeft = topLeft(strokeWidth, strokeInset)
                val borderSize = borderSize(strokeWidth, strokeInset)
                val fillArea = fillArea(strokeWidth, strokeInset)
                val cornerRadius = outline.roundRect.topLeftCornerRadius.shrink(strokeInset)
            ...
            return {
                ...
                val strokeInsetPx = strokeInsetPx()
                val fillArea = fillArea(strokeWidthPx, strokeInsetPx)
    ...
    private fun createDrawRectBorder(brush: Brush): DrawScope.() -> Unit {
        return {
            val strokeWidthPx = strokeWidthPx()
            val strokeInset = strokeInsetPx()
            val topLeft = topLeft(strokeWidthPx, strokeInset)
            val borderSize = borderSize(strokeWidthPx, strokeInset)
            val fillArea = fillArea(strokeWidthPx, strokeInset)
