...
final class CaptureSession {
  ...
  @Nullable private CameraCaptureSession cameraCaptureSession;
  ...
  private final List<CaptureRequestConfiguration> captureRequestConfigurations = new ArrayList<>();
  ...
  enum State {
    ...
    INITIALIZED,
    /**
     * Transitional state when the {@link CameraCaptureSession} is in the process of being opened.
     */
    OPENING,
    /**
     * Stable state where the {@link CameraCaptureSession} has been successfully opened. During this
     * state if a valid {@link SessionConfiguration} has been set then the {@link CaptureRequest}
     * will be issued.
     */
    OPENED,
    /**
     * Stable state where the session has been closed. However the {@link CameraCaptureSession} is
     * still valid. It will remain valid until a new instance is opened at which point {@link
     * CameraCaptureSession.StateCallback#onClosed(CameraCaptureSession)} will be called to do final
     * cleanup.
     */
    CLOSED,
    ...
    RELEASING,
    /**
     * Terminal state where the session has been cleaned up. At this point the session should not be
     * used as nothing will happen in this state.
     */
    RELEASED
  }
  ...
  private final class StateCallback extends CameraCaptureSession.StateCallback {
     ...
     * {@inheritDoc}
     *
     * <p>Once the {@link CameraCaptureSession} has been configured then the capture request will be
     * immediately issued.
     */
    @Override
    ...
    public void onReady(CameraCaptureSession session) {
      synchronized (stateLock) {
        switch (state) {
          case UNINITIALIZED:
            throw new IllegalStateException("onReady() should not be possible in state: " + state);
          default:
        }
    ...
    public void onClosed(CameraCaptureSession session) {
      synchronized (stateLock) {
        switch (state) {
          case UNINITIALIZED:
            throw new IllegalStateException("onClosed() should not be possible in state: " + state);
          default:
   ...
   *     this is null then when asynchronous methods are called on this session they will attempt to
   ...
   *
   * @param sessionConfiguration has the configuration that will currently active in issuing capture
   *     request. The surfaces contained in this must be a subset of the surfaces that were used to
   *     open this capture session.
   */
   ...
   *     contains configurations which may or may not be currently active in issuing capture
   *     requests.
  ...
  void open(SessionConfiguration sessionConfiguration, CameraDevice cameraDevice)
    ...
    synchronized (stateLock) {
      switch (state) {
        case UNINITIALIZED:
          throw new IllegalStateException("open() should not be possible in state: " + state);
        case INITIALIZED:
          configuredSurfaces =
              new ArrayList<>(DeferrableSurfaces.surfaceSet(sessionConfiguration.getSurfaces()));
          ...
          CameraCaptureSession.StateCallback comboCallback =
              CameraCaptureSessionStateCallbacks.createComboCallback(
                  captureSessionStateCallback, sessionConfiguration.getSessionStateCallback());
  ...
  void close() {
    synchronized (stateLock) {
      switch (state) {
        case UNINITIALIZED:
          throw new IllegalStateException("close() should not be possible in state: " + state);
        case INITIALIZED:
          state = State.RELEASED;
          break;
        case OPENING:
  ...
  void release() {
    synchronized (stateLock) {
      switch (state) {
        case UNINITIALIZED:
          throw new IllegalStateException("release() should not be possible in state: " + state);
        case INITIALIZED:
          state = State.RELEASED;
          break;
        case OPENING:
   ...
   */
  void issueSingleCaptureRequests(List<CaptureRequestConfiguration> captureRequestConfigurations) {
    synchronized (stateLock) {
      switch (state) {
        case UNINITIALIZED:
          throw new IllegalStateException(
              "issueSingleCaptureRequests() should not be possible in state: " + state);
        case INITIALIZED:
        case OPENING:
  ...
  private void issueRepeatingCaptureRequests() {
    ...
    try {
      ...
      CaptureRequest.Builder builder =
          captureRequestConfiguration.buildCaptureRequest(cameraCaptureSession.getDevice());
      if (builder == null) {
      ...
      cameraCaptureSession.setRepeatingRequest(builder.build(), comboCaptureCallback, handler);
  ...
  private void applyImplementationOptionTCaptureBuilder(
    ...
    for (Option<?> option : camera2Config.getCaptureRequestOptions()) {
      /* Although type is erased below, it is safe to pass it to CaptureRequest.Builder because
      these option are created via Camera2Configuration.Extender.setCaptureRequestOption
      (CaptureRequest.Key<ValueT> key, ValueT value) and hence the type compatibility of key and
  ...
  private void issueCaptureRequests() {
    ...
    for (CaptureRequestConfiguration captureRequestConfiguration : captureRequestConfigurations) {
