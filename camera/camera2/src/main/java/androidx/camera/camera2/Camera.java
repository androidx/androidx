...
final class Camera implements BaseCamera, Camera2RequestRunner {
  ...
  enum State {
     ...
     *
     * <p>At this state the {@link CameraDevice} should be invalid, but threads should be still in a
     * valid state. Whenever a camera device is fully closed the camera should return to this state.
     *
     * <p>After an error occurs the camera returns to this state so that the device can be cleanly
     * reopened.
     */
    INITIALIZED,
    /**
     ...
     *
     * <p>At the end of this state, the camera should move into either the OPENED or CLOSING state.
     */
    OPENING,
    /**
     * A stable state where the camera has been opened.
     *
     * <p>During this state the camera device should be valid. It is at this time a valid capture
     * session can be active. Capture requests should be issued during this state only.
     */
    OPENED,
    /**
    ...
    CLOSING,
    /**
     * A transitional state where the camera was previously closing, but not fully closed before a
     * call to open was made.
     *
     * <p>At the end of this state, the camera should move into one of two states. The OPENING state
     * if the device becomes fully closed, since it must restart the process of opening a camera.
     * The OPENED state if the device becomes opened, which can occur if a call to close had been
     * done during the OPENING state.
     */
    REOPENING,
    /**
     ...
     *
     * <p>During this state all resources should be released and all operations on the camera will
     * do nothing.
     */
    RELEASED
  }
  ...
  private void resetCaptureSession() {
    ...
    SessionConfiguration previousSessionConfiguration = captureSession.getSessionConfiguration();
    ...
    // When the previous capture session has not reached the open state, the issued single capture
  ...
  private void checkAndAttachRepeatingSurface(
    ...
    for (BaseUseCase useCase : activeUseCases) {
      ...
      if (!surfaces.isEmpty()) {
        // When an use case is active, all surfaces in its CaptureRequestConfiguration are added to
