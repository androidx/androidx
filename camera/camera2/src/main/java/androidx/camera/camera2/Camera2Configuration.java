...
public final class Camera2Configuration implements Configuration.Reader {
  ...
  public <ValueT> ValueT getCaptureRequestOption(
    ...
    Option<ValueT> opt = (Option<ValueT>) Camera2Configuration.createCaptureRequestOption(key);
  ...
  public static final class Extender {
     ...
     */
    public <ValueT> Extender setCaptureRequestOption(CaptureRequest.Key<ValueT> key, ValueT value) {
     ...
     * callback methods responsibly. Generally safe usages include: (1) querying the device for its
     * id, (2) using the callbacks to determine what state the device is currently in. Generally
     * unsafe usages include: (1) creating a new {@link CameraCaptureSession}, (2) creating a new
     * {@link CaptureRequest}, (3) closing the device. When the caller uses the device beyond the
     * safe usage limits, the usage may still work in conjunction with CameraX, but any strong
     ...
     *
     * <p>The caller is expected to use the {@link CameraCaptureSession} instance accessed through
     * the callback methods responsibly. Generally safe usages include: (1) querying the session for
     * its properties, (2) using the callbacks to determine what state the session is currently in.
     * Generally unsafe usages include: (1) submitting a new {@link CaptureRequest}, (2) stopping an
     * existing {@link CaptureRequest}, (3) closing the session, (4) attaching a new {@link Surface}
     * to the session. When the caller uses the session beyond the safe usage limits, the usage may
     * still work in conjunction with CameraX, but any strong gurantees provided by CameraX about
     * the validity of the camera state become void.
     *
     ...
     * <p>The caller is expected to use the {@link CameraCaptureSession} instance accessed through
     * the callback methods responsibly. Generally safe usages include: (1) querying the session for
     * its properties. Generally unsafe usages include: (1) submitting a new {@link CaptureRequest},
     * (2) stopping an existing {@link CaptureRequest}, (3) closing the session, (4) attaching a new
     * {@link Surface} to the session. When the caller uses the session beyond the safe usage
     * limits, the usage may still work in conjunction with CameraX, but any strong gurantees
     * provided by CameraX about the validity of the camera state become void.
     *
     * <p>The caller is generally free to use the {@link CaptureRequest} and {@link CaptureResult}
   ...
   *
   * <p>Use {@link Builder} for creating {@link Configuration} which contains camera2 options only.
  ...
  static final Option<CameraCaptureSession.StateCallback> SESSION_STATE_CALLBACK_OPTION =
      Option.create(
          "camera2.cameraCaptureSession.stateCallback", CameraCaptureSession.StateCallback.class);
