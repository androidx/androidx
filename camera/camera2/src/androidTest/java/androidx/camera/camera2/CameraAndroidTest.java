...
public class CameraAndroidTest {
  ...
  private static class UseCase extends FakeUseCase {
    ...
    UseCase(FakeUseCaseConfiguration configuration, ImageReader.OnImageAvailableListener listener) {
      super(configuration);
  ...
  public void teardown() throws InterruptedException {
    // Need to release the camera no matter what is done, otherwise the CameraDevice is not closed.
    // When the CameraDevice is not closed, then it can cause problems with interferes with other
    // test cases.
    if (camera != null) {
      camera.release();
      camera = null;
