...
class PrefHelper {
    companion object {
        ...
        internal fun getNumTests(activity: MainActivity): Int {
            val sharedPref: SharedPreferences =
                PreferenceManager.getDefaultSharedPreferences(activity)
            return (sharedPref.getString(activity.getString(R.string.settings_numtests_key), "30")
                    ?: "30")
                .toInt()
        }
        ...
        internal fun getImageSizes(activity: MainActivity): ArrayList<ImageCaptureSize> {
            val sharedPref: SharedPreferences =
                PreferenceManager.getDefaultSharedPreferences(activity)
            val defSizes: HashSet<String> =
                HashSet(
                    activity.resources.getStringArray(R.array.array_settings_imagesize).asList()
                )
        ...
        internal fun getCameraIds(
            ...
            for (params in cameraParams) cameraIds.add(params.value.id)

            // Before 28, no physical camera access
            if (Build.VERSION.SDK_INT < 28) return cameraIds

            // For logical cameras, we remove all physical camera ids
            if (onlyLogical) {
                for (params in cameraParams) {
                    for (physicalId in params.value.physicalCameras) cameraIds.remove(physicalId)
                }
                ...
                for (params in cameraParams) {
                    if (params.value.hasMulti) cameraIds.remove(params.value.id)
                }
            }
    ...
    internal class ApiComparator : Comparator<CameraAPI> {
        override fun compare(api1: CameraAPI, api2: CameraAPI): Int {
            if (api1 == api2) return 0
            if (api1 == CameraAPI.CAMERA2) return -1
            if (api2 == CameraAPI.CAMERA2) return 1
            if (api1 == CameraAPI.CAMERAX) return -1
            if (api2 == CameraAPI.CAMERAX) return 1
    ...
    internal class ImageSizeComparator : Comparator<ImageCaptureSize> {
        override fun compare(size1: ImageCaptureSize, size2: ImageCaptureSize): Int {
            ...
            if (size1 == ImageCaptureSize.MAX) return -1
            if (size2 == ImageCaptureSize.MAX) return 1
    ...
    internal class FocusModeComparator : Comparator<FocusMode> {
        override fun compare(mode1: FocusMode, mode2: FocusMode): Int {
            ...
            if (mode1 == FocusMode.AUTO) return -1
            if (mode2 == FocusMode.AUTO) return 1
