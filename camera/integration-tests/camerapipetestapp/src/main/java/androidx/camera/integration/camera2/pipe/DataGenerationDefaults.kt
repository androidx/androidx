...
private fun DataGenerationParams1D.generateNewBoolOrNull(prev: Any?): Boolean? {
    ...
    return if ((1..this.changeModeOnceInHowMany).random() == 1 || prev == null) {
        val temp = if ((1..this.nullValueOnceInHowMany).random() == 1) null else (0..1).random()
        when (temp) {
            0 -> false
            1 -> true
            else -> null
...
val DATA_GENERATION_PARAMS: Map<CameraMetadataKey, DataGenerationParams1D> =
    mapOf(
        CameraMetadataKey.LENS_FOCUS_DISTANCE to
