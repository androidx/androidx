...
fun configureFullScreenCameraWindow(activity: Activity) {
    ...
    windowParams.rotationAnimation =
...
open class CameraPermissionActivity : Activity() {
    ...
    override fun onRequestPermissionsResult(
        ...
        if (grantResults.isEmpty()) {
            Log.w("CXCP-App", "Permission request was canceled, there were no grantResults")
            return
        }
        ...
        if (!isStarted) {
            Log.w("CXCP-App", "Received permission results, but activity is not started.")
            return
        }
        ...
        val success =
            grantResults.zip(permissions).fold(true) { currentSuccess, (result, permission) ->
