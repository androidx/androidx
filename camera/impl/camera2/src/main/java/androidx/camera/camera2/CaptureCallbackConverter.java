...
final class CaptureCallbackConverter {
  ...
  /**
   * Converts {@link CameraCaptureCallback} to one or more {@link CaptureCallback} and put them into
   * the input capture callback list.
   *
  ...
  static void toCaptureCallback(
      CameraCaptureCallback cameraCaptureCallback, List<CaptureCallback> captureCallbackList) {
    if (cameraCaptureCallback instanceof ComboCameraCaptureCallback) {
      // Recursively convert callback inside the combo callback.
      ComboCameraCaptureCallback comboCallback = (ComboCameraCaptureCallback) cameraCaptureCallback;
