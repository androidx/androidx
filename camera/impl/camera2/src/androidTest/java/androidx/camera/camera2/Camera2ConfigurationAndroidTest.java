...
public final class Camera2ConfigurationAndroidTest {
  ...
  public void canExtendWithSessionCaptureCallback() {
    ...
    new Camera2Configuration.Extender(builder).setSessionCaptureCallback(SESSION_CAPTURE_CALLBACK);
  ...
  public void canSetAndRetrieveCaptureRequestKeys() {
    ...
    assertThat(
            config.getCaptureRequestOption(
                CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, /*valueIfMissing=*/ null))
        .isSameAs(fakeRange);
    assertThat(
            config.getCaptureRequestOption(
                CaptureRequest.COLOR_CORRECTION_MODE, INVALID_COLOR_CORRECTION_MODE))
    ...
    config.findOptions(
        ...
        option -> {
          ...
          assertThat(option.getToken())
              .isAnyOf(
                  CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, CaptureRequest.COLOR_CORRECTION_MODE);
          return true;
        });
