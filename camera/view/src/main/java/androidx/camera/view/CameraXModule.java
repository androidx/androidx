...
final class CameraXModule {
  ...
  @Nullable private ImageCaptureUseCase imageCaptureUseCase;
  @Nullable private VideoCaptureUseCase videoCaptureUseCase;
  @Nullable private ViewFinderUseCase viewFinderUseCase;
  @Nullable private LifecycleOwner currentLifecycle;
  @Nullable private LifecycleOwner newLifecycle;
  ...
  @Nullable private Rect cropRegion;
  @Nullable private CameraX.LensFacing cameraLensFacing = LensFacing.BACK;
  ...
  void bindToLifecycleAfterViewMeasured() {
    ...
    try {
      ...
      // Do not attempt to create use cases for a null cameraLensFacing. This could occur if the
    ...
    // ratio as 16:9 if it is VIDEO or MIXED mode. Then, it will be WYSIWYG when the view finder is
  ...
  public void setZoomLevel(float zoomLevel) {
    ...
    if (viewFinderUseCase == null) {
      // Nothing to zoom on yet since we don't have a viewfinder. Defer calculating crop region.
      return;
    }
  ...
  private static Rect rescaleViewRectToSensorRect(Rect view, Rect sensor) {
    ...
    int leftOffset =
        Math.round((view.left + halfViewDimension) * sensor.width() / (float) MAX_VIEW_DIMENSION)
            + sensor.left;
    int topOffset =
        Math.round((view.top + halfViewDimension) * sensor.height() / (float) MAX_VIEW_DIMENSION)
            + sensor.top;
