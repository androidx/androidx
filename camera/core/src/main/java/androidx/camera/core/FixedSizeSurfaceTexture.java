...
final class FixedSizeSurfaceTexture extends SurfaceTexture {
   ...
   * ANativeWindow_lock, or that call will fail. When producing image content with OpenGL ES, {@link
   * #releaseTexImage()} must be called before the first OpenGL ES function call each frame.
   *
   * @param texName the OpenGL texture object name (e.g. generated via glGenTextures)
   ...
   * #releaseTexImage()} must be called before the first OpenGL ES function call each frame.
   *
   * <p>Unlike {@link SurfaceTexture(int, boolean)}, which takes an OpenGL texture object name, this
   * constructor creates the SurfaceTexture in detached mode. A texture name must be passed in using
   * {@link #attachToGLContext} before calling {@link #releaseTexImage()} and producing image
   * content using OpenGL ES.
   *
   * @param singleBufferMode whether the SurfaceTexture will be in single buffered mode.
   * @param fixedSize the fixed default buffer size
   * @throws Surface.OutOfResourcesException If the SurfaceTexture cannot be created.
