...
public final class ImageAnalysisUseCaseConfiguration
  ...
  public static final class Builder
      implements CameraDeviceConfiguration.Builder<ImageAnalysisUseCaseConfiguration, Builder>,
     ...
     *
     * <p>The image queue depth is the number of images available to the camera to fill with data.
     * This includes the image currently being analyzed by {@link
     * ImageAnalysisUseCase.Analyzer#analyze(ImageProxy, int)}. Increasing the image queue depth may
     * make camera operation smoother, depending on the {@link ImageReaderMode}, at the cost of
     * increased memory usage.
     *
     * <p>When the {@link ImageReaderMode} is set to {@link ImageReaderMode#ACQUIRE_LATEST_IMAGE},
     * increasing the image queue depth will increase the amount of time available to analyze an
     * image before stalling the capture pipeline.
     *
     * <p>When the {@link ImageReaderMode} is set to {@link ImageReaderMode#ACQUIRE_NEXT_IMAGE},
     * increasing the image queue depth may make the camera pipeline run smoother on systems under
     * high load. However, the time spent analyzing an image should still be kept under a single
     * frame period for the current frame rate, on average, to avoid stalling the camera pipeline.
     *
     ...
     *
     * <p>The target resolution attempts to establish a minimum bound for the image resolution. The
     * actual image resolution will be the closest available resolution in size that is not smaller
     * than the target resolution, as determined by the Camera implementation. However, if no
     * resolution exists that is equal to or larger than the target resolution, the nearest
     * available resolution smaller than the target resolution will be chosen.
     *
