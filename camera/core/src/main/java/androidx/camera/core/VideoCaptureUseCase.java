...
public class VideoCaptureUseCase extends BaseUseCase {
   ...
   * {@link VideoCaptureUseCase.OnVideoSavedListener.onError}.
   *
  ...
  public enum UseCaseError {
    ...
    MUXER_ERROR,
    /**
     * An error indicating start recording was called when video recording is still in
     * progress.
     */
  ...
  @NonNull private MediaCodec videoEncoder;
  @NonNull private MediaCodec audioEncoder;
  ...
  /** audio raw data */
  @NonNull private AudioRecord audioRecorder;
  ...
  private static final short[] audioEncoding = {
    AudioFormat.ENCODING_PCM_16BIT, AudioFormat.ENCODING_PCM_8BIT, AudioFormat.ENCODING_PCM_FLOAT
  };
   ...
   */
  public void startRecording(File saveLocation, OnVideoSavedListener listener, Metadata metadata) {
    Log.i(TAG, "startRecording");

    if (!endOfAudioVideoSignal.get()) {
      listener.onError(UseCaseError.RECORDING_IN_PROGRESS, "It is still in video recording!", null);
      return;
    }
    ...
    try {
      synchronized (muxerLock) {
        muxer =
            new MediaMuxer(
                saveLocation.getAbsolutePath(), MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);

        muxer.setOrientationHint(relativeRotation);
        if (metadata.location != null) {
          muxer.setLocation(
              (float) metadata.location.getLatitude(), (float) metadata.location.getLongitude());
        }
      }
    } catch (IOException e) {
      setupEncoder(getAttachedSurfaceResolution(cameraId));
   ...
   * OnVideoSavedListener#onVideoSaved(File)} or {@link
   * OnVideoSavedListener#onError(UseCaseError, String, Throwable)} be called before startRecording.
   */
  ...
  /**
   * Setup the {@link MediaCodec} for encoding video from a camera {@link Surface}
   * and encoding audio from selected audio source.
   */
  private void setupEncoder(Size resolution) {
    ...
    SessionConfiguration.Builder builder = SessionConfiguration.Builder.createFrom(configuration);
    ...
    audioEncoder.reset();
    audioEncoder.configure(createAudioMediaFormat(), null, null, MediaCodec.CONFIGURE_FLAG_ENCODE);
  ...
  private boolean writeAudioEncodedBuffer(int bufferIndex) {
    ...
    if (audioTrackIndex >= 0 && videoTrackIndex >= 0 && audioBufferInfo.size > 0
      ...
      try {
        ...
        Log.e(TAG, "audio error:size=" + audioBufferInfo.size + "/offset="
            + audioBufferInfo.offset + "/timeUs=" + audioBufferInfo.presentationTimeUs);
        e.printStackTrace();
      }
    }
  ...
  private boolean videoEncode(OnVideoSavedListener videoSavedListener) {
    ...
    while (!videoEos && !errorOccurred) {
      ...
      int outputBufferId = videoEncoder.dequeueOutputBuffer(videoBufferInfo, DEQUE_TIMEOUT_USEC);
      switch (outputBufferId) {
        case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
          if (muxerStarted) {
            videoSavedListener.onError(
                UseCaseError.ENCODER_ERROR, "Unexpected change in video encoding format.",
                null);
            errorOccurred = true;
          }
        ...
        case MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED:
          // Ignore output buffers changed since we dequeue a single buffer instead of multiple
          break;
        default:
    ...
    // recording because it requires attaching a new Surface. This causes a glitch so we don't want
    // that to incur latency at the start of capture.
    setupEncoder(getAttachedSurfaceResolution(getCameraIdUnchecked(configuration.getLensFacing())));
    notifyReset();
  ...
  private boolean audioEncode(OnVideoSavedListener videoSavedListener) {
    ...
    try {
      ...
      videoSavedListener.onError(UseCaseError.ENCODER_ERROR, "Audio recorder stop failed!", e);
    }
    ...
    // Use AtomicBoolean to signal because MediaCodec.signalEndOfInputStream() is not thread safe
  ...
  private AudioRecord autoConfigAudioRecordSource(VideoCaptureUseCaseConfiguration configuration) {
      ...
      try {
        int bufferSize =
            AudioRecord.getMinBufferSize(
                audioSampleRate, channelConfig, audioFormat);
        ...
        AudioRecord recorder =
            new AudioRecord(source, audioSampleRate, channelConfig, audioFormat,
                bufferSize * 2);

        if (recorder.getState() == AudioRecord.STATE_INITIALIZED) {
          audioBufferSize = bufferSize;
          Log.i(TAG, "source: " + source + " audioSampleRate: " + audioSampleRate
