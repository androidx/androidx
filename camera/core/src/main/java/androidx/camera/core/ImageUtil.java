...
final class ImageUtil {
  ...
  public static byte[] yuvImageToJpegByteArray(
      ImageProxy image, @Nullable Rect cropRect)
   ...
   * Assuming the rect is at the center of the source.
   */
  ...
  private static byte[] nv21ToJpeg(byte[] nv21, int width, int height, @Nullable Rect cropRect)
    ...
    boolean success = yuv.compressToJpeg(
        cropRect == null ? new Rect(0, 0, width, height) : cropRect,
        100,
        out);
    if (!success) {
