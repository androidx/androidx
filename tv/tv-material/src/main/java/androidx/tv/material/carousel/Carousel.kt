 ...
 * @param timeToDisplaySlide duration for which slide should be visible before moving to the next
 * slide.
...
fun Carousel(
    ...
    timeToDisplaySlide: Duration = CarouselDefaults.TimeToDisplaySlide,
    ...
    AutoScrollSideEffect(timeToDisplaySlide, slideCount, carouselState, focusState)
...
private fun AutoScrollSideEffect(
    timeToDisplaySlide: Duration,
    slideCount: Int,
    carouselState: CarouselState,
    focusState: FocusState?
) {
    val currentTimeToDisplaySlide by rememberUpdatedState(timeToDisplaySlide)
    ...
    if (!(carouselIsFocused || carouselHasFocus)) {
        LaunchedEffect(carouselState) {
            while (true) {
                yield()
                delay(currentTimeToDisplaySlide.toMillis())
...
object CarouselDefaults {
     ...
     */
    val TimeToDisplaySlide: Duration = Duration.ofSeconds(5)
