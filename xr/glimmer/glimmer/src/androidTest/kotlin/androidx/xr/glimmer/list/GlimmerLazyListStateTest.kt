...
class GlimmerLazyListStateTest(orientation: Orientation) :
    ...
    fun saveAndRestoreState() {
        ...
        val original =
            GlimmerLazyListState(firstVisibleItemIndex = 42, firstVisibleItemScrollOffset = 100)

        val saved = with(GlimmerLazyListState.Saver) { allowingScope.save(original) }
        val restored = GlimmerLazyListState.Saver.restore(requireNotNull(saved))
    ...
    fun respectCanScrollForwardAndCanScrollBackward() {
        val state = GlimmerLazyListState(firstVisibleItemIndex = 10)
        rule.setGlimmerThemeContent { TestList(state = state) { Text("Item ($it)") } }
    ...
    fun carriedOverPart_isApplied() {
        ...
        state.applyMeasureResult(
            result = state.layoutInfo as GlimmerLazyListMeasureResult,
            consumedScroll = 0f,
    ...
    fun initialState_withNonZeroParameters_isAppliedCorrectly() {
        ...
        val state =
            GlimmerLazyListState(firstVisibleItemIndex = 50, firstVisibleItemScrollOffset = 42)
    ...
    private fun GlimmerLazyListState.scrollByAndCheckConsumedValue(
        delta: Float,
    ...
    private val GlimmerLazyListState.totalScroll: Float
