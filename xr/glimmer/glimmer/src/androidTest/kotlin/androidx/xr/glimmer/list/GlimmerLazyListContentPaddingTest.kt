...
class GlimmerLazyListContentPaddingTest(orientation: Orientation) :
    ...
    fun contentPaddingIsApplied() {
        lateinit var state: GlimmerLazyListState
        ...
        rule.setContent {
            TestList(
                ...
                state = rememberGlimmerLazyListState().also { state = it },
                contentPadding =
    ...
    fun unevenPaddingWithRtl() {
        ...
        lateinit var state: GlimmerLazyListState
        rule.setContent {
            state = rememberGlimmerLazyListState()
    ...
    private fun GlimmerLazyListState.scrollContentBy(value: Dp) {
        val px = with(rule.density) { value.toPx() }
        runBlocking(Dispatchers.Main) { scrollContentBy(px) }
    }

    private fun GlimmerLazyListState.assertScrollPosition(index: Int, offset: Dp) =
        with(rule.density) {
    ...
    private fun GlimmerLazyListState.assertLayoutInfoOffsetRange(from: Dp, to: Dp) =
        with(rule.density) {
            assertThat(layoutInfo.viewportStartOffset to layoutInfo.viewportEndOffset)
                .isEqualTo(from.roundToPx() to to.roundToPx())
        }

    private fun GlimmerLazyListState.assertVisibleItems(vararg expected: Pair<Int, Dp>) =
        with(rule.density) {
