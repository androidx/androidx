...
internal fun Modifier.autoFocus(state: GlimmerLazyListAutoFocusState): Modifier =
    this then GlimmerLazyListAutoFocusNodeElement(state)

private class GlimmerLazyListAutoFocusNodeElement(
    private val state: GlimmerLazyListAutoFocusState
) : ModifierNodeElement<GlimmerLazyListAutoFocusNode>() {

    override fun create(): GlimmerLazyListAutoFocusNode {
        return GlimmerLazyListAutoFocusNode(state)
    }

    override fun update(node: GlimmerLazyListAutoFocusNode) {
        node.update(state)
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is GlimmerLazyListAutoFocusNodeElement) return false

        return state == other.state
    }
}

private class GlimmerLazyListAutoFocusNode(private var state: GlimmerLazyListAutoFocusState) :
    DelegatingNode(),
    ...
    fun update(state: GlimmerLazyListAutoFocusState) {
        this.state = state
    }
