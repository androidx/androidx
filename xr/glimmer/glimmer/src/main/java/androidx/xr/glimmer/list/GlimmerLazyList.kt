...
import androidx.xr.glimmer.list.GlimmerLazyColumnDefaults.verticalArrangement
import kotlin.math.max

/**
 * This is a scrolling list component that only composes and lays out the currently visible items.
 * It is similar to [androidx.compose.foundation.lazy.LazyColumn], but with extra functionality and
 * customized behavior required for Jetpack Compose Glimmer. For Jetpack Compose Glimmer
 * applications, it is recommended to use [GlimmerLazyColumn] instead of
 ...
 * you can use [GlimmerLazyListScope.item] to add a single item and [GlimmerLazyListScope.items] to
 * add a list of items.
 *
 * See the other [GlimmerLazyColumn] overload for a variant with a title slot.
 *
 * @sample androidx.xr.glimmer.samples.GlimmerLazyColumnSample
 ...
 *   [GlimmerLazyListScope.item] to add a single item or [GlimmerLazyListScope.items] to add a list
 *   of items.
 */
@Composable
 ...
 * should always use GlimmerLazyColumn instead of LazyColumn to ensure correct behavior.
 *
 * The [content] block defines a DSL which allows you to emit items of different types. For example,
 * you can use [GlimmerLazyListScope.item] to add a single item and [GlimmerLazyListScope.items] to
 * add a list of items.
 *
 * This overload of `GlimmerLazyColumn` contains a `title` slot. The title is expected to be a
 ...
 *
 * @sample androidx.xr.glimmer.samples.GlimmerLazyColumnWithTitleChipSample
...
public fun GlimmerLazyColumn(
    ...
    GlimmerLazyColumnWithTitleLayout(
        modifier = modifier,
        title = title,
        list = {
            GlimmerLazyColumn(
...
public object GlimmerLazyColumnDefaults {
     ...
     *
     * @param state The [GlimmerLazyListState] to observe for layout and focus information.
     */
    @Composable
...
@Composable
private fun GlimmerLazyColumnWithTitleLayout(
    modifier: Modifier = Modifier,
    title: @Composable () -> Unit,
 ...
 * [GlimmerLazyListScope.item] to add a single item and [GlimmerLazyListScope.items] to add a list
 * of items.
 *
...
internal fun GlimmerLazyList(
    ...
    val itemProvider = rememberGlimmerLazyListItemProviderLambda(state, content)

    val semanticState = rememberGlimmerLazyListSemanticState(state, orientation)

    val scrollEnabled = isScrollEnabled(userScrollEnabled, state)

    val measurePolicy =
        rememberGlimmerLazyListMeasurePolicy(
    ...
    val beyondBoundsModifier =
        if (scrollEnabled) {
            Modifier.lazyLayoutBeyondBoundsModifier(
                state = rememberGlimmerLazyListBeyondBoundsState(state),
        ...
        modifier =
            modifier
                ...
                .edgeScrim(
                    ...
                    orientation = orientation,
                    maxScrimSize = GlimmerLazyColumnDefaults.ScrimMaxHeight,
                )
                .scrollableArea(
                    state = state,
                    orientation = orientation,
...
@Composable
private fun isScrollEnabled(userScrollEnabled: Boolean, state: GlimmerLazyListState): Boolean {
