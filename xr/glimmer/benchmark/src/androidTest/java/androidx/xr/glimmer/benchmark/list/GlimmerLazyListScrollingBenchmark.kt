...
class GlimmerLazyListScrollingBenchmark {
    ...
    fun glimmerLazyColumn_measureAndLayoutPhases_afterProgrammaticScroll() {
        val listState = GlimmerLazyListState(firstVisibleItemIndex = 0)
        benchmarkRule.toggleStateBenchmark { ProgrammaticScrollTestCase(listState) }
    }

    @Test
    fun glimmerLazyColumn_measureAndLayoutPhases_afterIndirectScroll() {
        val listState = GlimmerLazyListState(firstVisibleItemIndex = 0)
     ...
     */
    @Test
    fun glimmerLazyColumn_drawPhase_afterScroll() {
        val listState = GlimmerLazyListState(firstVisibleItemIndex = 0)
        benchmarkRule.toggleStateBenchmarkDraw { IndirectTouchScrollTestCase(listState) }
    }
}

internal class ProgrammaticScrollTestCase(listState: GlimmerLazyListState) :
    ScrollableGlimmerLazyListTestCase(listState) {
    override fun toggle() {
...
internal class IndirectTouchScrollTestCase(listState: GlimmerLazyListState) :
    ScrollableGlimmerLazyListTestCase(listState) {
...
internal abstract class ScrollableGlimmerLazyListTestCase(val listState: GlimmerLazyListState) :
    GlimmerLazyListBenchmarkTestCase, LayeredComposeTestCase() {
