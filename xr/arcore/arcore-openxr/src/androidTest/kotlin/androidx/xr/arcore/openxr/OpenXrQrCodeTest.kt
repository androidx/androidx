...
class OpenXrQrCodeTest {
    ...
    private lateinit var underTest: OpenXrQrCode

    @Before
    fun setUp() {
        timeSource = OpenXrTimeSource()
        xrResources = XrResources(timeSource)
        underTest = OpenXrQrCode(qrCodeId)
        xrResources.addTrackable(qrCodeId, underTest)
    ...
    @Test
    fun update_updatesData() = initOpenXrRuntimeAndRunTest {
        val xrTime = 50L * 1_000_000 // 50 milliseconds in nanoseconds.
        check(underTest.data == "")

        underTest.update(xrTime)

        assertThat(underTest.data).isEqualTo("Success")
    }
    ...
    private fun initOpenXrRuntimeAndRunTest(testBody: () -> Unit) {
        activityRule.scenario.onActivity {
            ...
            openXrRuntime.configure(Config(qrCodeTracking = QrCodeTrackingMode.DYNAMIC))
