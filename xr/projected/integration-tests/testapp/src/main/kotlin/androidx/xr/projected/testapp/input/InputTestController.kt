...
/** Controller hosting connection monitoring and input event routing for [InputHostActivity]. */
...
class InputTestController(
    ...
    fun launchProjectedActivity() {
        try {
            val intent =
                Intent(context, ProjectedInputActivity::class.java).apply {
                    addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                }
            val options = ProjectedContext.createProjectedActivityOptions(context).toBundle()
            context.startActivity(intent, options)
        } catch (e: Exception) {
            Log.e(TAG, "Error launching ProjectedInputActivity", e)
            viewModel.setStatusMessage("Launch failed: ${e.message}")
        }
    }

    fun clearLogs() {
        viewModel.clearEventLogs()
    }

    override fun close() {}

    private companion object {
        const val TAG = "InputTestController"
    }
}
