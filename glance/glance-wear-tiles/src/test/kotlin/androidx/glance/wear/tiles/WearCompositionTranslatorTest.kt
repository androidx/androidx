...
class WearCompositionTranslatorTest {
    ...
    fun canTranslateBox() =
        fakeCoroutineScope.runTest {
            val content = runAndTranslate { Box {} }.layout

            // runAndTranslate wraps the result in a Box...ensure that the layout generated two
            // Boxes
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslateBoxWithAlignment() =
        fakeCoroutineScope.runTest {
            val content = runAndTranslate { Box(contentAlignment = Alignment.Center) {} }.layout
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslateBoxWithChildren() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        Box {
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslatePaddingModifier() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        Box(
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslateBorderModifier() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        Box(
            ...
            assertThat(border2.width!!.value)
                .isEqualTo(context.resources.getDimension(R.dimen.dimension1))
            assertThat(border2.color!!.argb).isEqualTo(Color.Red.toArgb())
        }

    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslateBackgroundModifier() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate { Box(modifier = GlanceModifier.background(Color(0x11223344))) {} }
                    .layout
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslateBackgroundModifier_resId() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate { Box(modifier = GlanceModifier.background(R.color.color1)) {} }
                    .layout
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslateSemanticsModifier() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        Box(
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslateSemanticsCurvedModifier() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        CurvedRow {
            ...
            val innerArcText =
                innerArc.contents[0] as androidx.wear.tiles.LayoutElementBuilders.ArcText
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun rowWithHorizontalAlignmentInflatesInColumn() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        Row(
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun columnWithVerticalAlignmentInflatesInRow() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        Column(
            ...
            val innerRow =
                (content as androidx.wear.tiles.LayoutElementBuilders.Box).contents[0]
                    as androidx.wear.tiles.LayoutElementBuilders.Row
            val innerColumn =
                innerRow.contents[0] as androidx.wear.tiles.LayoutElementBuilders.Column
    ...
    fun canTranslateCurvedRow() =
        fakeCoroutineScope.runTest {
            ...
            // Remember, 0 degrees is handled differently in Glance (3 o clock) and Tiles (12 o
            // clock).
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun curvedRowWithSizeInflatesInBox() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        CurvedRow(
        ...
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        ...
                        CurvedRow { curvedText(text = "Hello World", style = style) }
                    }
                    .layout
        ...
        fakeCoroutineScope.runTest {
            ...
            val innerArc =
                (content as androidx.wear.tiles.LayoutElementBuilders.Box).contents[0]
                    as androidx.wear.tiles.LayoutElementBuilders.Arc
            val innerArcLine =
                innerArc.contents[0] as androidx.wear.tiles.LayoutElementBuilders.ArcLine
        ...
        fakeCoroutineScope.runTest {
            ...
            val innerArc =
                (content as androidx.wear.tiles.LayoutElementBuilders.Box).contents[0]
                    as androidx.wear.tiles.LayoutElementBuilders.Arc
            val innerArcSpacer =
                innerArc.contents[0] as androidx.wear.tiles.LayoutElementBuilders.ArcSpacer
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslateActionOnCurvedElement() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        CurvedRow {
            ...
            val launchAction =
                arcText.modifiers!!.clickable!!.onClick
            ...
            val arcLineClickable =
                arcLine.modifiers!!.clickable as androidx.wear.tiles.ModifiersBuilders.Clickable
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canTranslateAndroidLayoutElement() =
        fakeCoroutineScope.runTest {
            ...
            val content = runAndTranslate { AndroidLayoutElement(providedLayoutElement) }.layout

            val box = assertIs<androidx.wear.tiles.LayoutElementBuilders.Box>(content)
            val textElement =
                assertIs<androidx.wear.tiles.LayoutElementBuilders.Text>(box.contents[0])
            assertThat(textElement.text!!.value).isEqualTo("Android Layout Element")
        }

    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun otherElementInArcInflatesInArcAdapter() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate { CurvedRow { curvedComposable(false) { Box {} } } }.layout

            val innerArc =
                (content as androidx.wear.tiles.LayoutElementBuilders.Box).contents[0]
                    as androidx.wear.tiles.LayoutElementBuilders.Arc
            val innerArcAdapter =
                innerArc.contents[0] as androidx.wear.tiles.LayoutElementBuilders.ArcAdapter
            assertIs<androidx.wear.tiles.LayoutElementBuilders.Box>(innerArcAdapter.content)
        }

    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun canInflateLaunchAction() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        Text(
            ...
            val launchAction =
                innerText.modifiers!!.clickable!!.onClick
                    as androidx.wear.tiles.ActionBuilders.LaunchAction
            assertThat(launchAction.androidActivity).isNotNull()
    ...
    fun canTranslateSpacer() =
        fakeCoroutineScope.runTest {
            ...
            assertThat(
                    (spacerWithWidth.width as androidx.wear.tiles.DimensionBuilders.DpProp).value
                )
                .isEqualTo(10f)

            val spacerWithHeight =
                content.contents[1] as androidx.wear.tiles.LayoutElementBuilders.Spacer
            assertThat(
                    (spacerWithHeight.height as androidx.wear.tiles.DimensionBuilders.DpProp).value
                )
                .isEqualTo(15f)

            val spacerWithSize =
                content.contents[2] as androidx.wear.tiles.LayoutElementBuilders.Spacer
            assertThat((spacerWithSize.width as androidx.wear.tiles.DimensionBuilders.DpProp).value)
                .isEqualTo(8f)
            assertThat(
                    (spacerWithSize.height as androidx.wear.tiles.DimensionBuilders.DpProp).value
                )
                .isEqualTo(12f)
        }
    ...
    fun canTranslateImage() =
        fakeCoroutineScope.runTest {
            ...
            val image1 =
                (content as androidx.wear.tiles.LayoutElementBuilders.Box).contents[0]
                    as androidx.wear.tiles.LayoutElementBuilders.Image

            assertThat((image1.width as androidx.wear.tiles.DimensionBuilders.DpProp).value)
                .isEqualTo(context.resources.getDimension(R.dimen.dimension1))
            assertThat((image1.height as androidx.wear.tiles.DimensionBuilders.DpProp).value)
                .isEqualTo(context.resources.getDimension(R.dimen.dimension2))
            assertThat(image1.contentScaleMode!!.value)
                .isEqualTo(androidx.wear.tiles.LayoutElementBuilders.CONTENT_SCALE_MODE_FILL_BOUNDS)
            ...
            assertThat(image2.contentScaleMode!!.value)
                .isEqualTo(androidx.wear.tiles.LayoutElementBuilders.CONTENT_SCALE_MODE_CROP)
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun setSizeFromResource() =
        fakeCoroutineScope.runTest {
            val content =
                runAndTranslate {
                        Column(
                            modifier =
                                GlanceModifier.width(R.dimen.dimension1).height(R.dimen.dimension2)
                        ) {}
                    }
                    .layout
    ...
    private suspend fun runAndTranslate(content: @Composable () -> Unit): CompositionResult {
