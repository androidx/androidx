...
class GlanceTileServiceTest {
    ...
    fun setUp() {
        ...
        val buffer =
            ByteArrayOutputStream()
                .apply { ovalBitmap.compress(Bitmap.CompressFormat.PNG, 100, this) }
                .toByteArray()
    ...
    fun tileProviderReturnsTile() =
        fakeCoroutineScope.runTest {
            // Request is currently un-used, provide an empty one.
            val tileRequest = RequestBuilders.TileRequest.Builder().build()

            // Requests need to be split; we need to allow Robolectric to schedule the service calls
            // on
    ...
    @Suppress("deprecation") // For backwards compatibility.
    @Test
    fun tileProviderReturnsTimelineTile() =
        fakeCoroutineScope.runTest {
            // Request is currently un-used, provide an empty one.
            val tileRequest = RequestBuilders.TileRequest.Builder().build()
            ...
            assertWithMessage("tile.resourcesVersion")
                .that(tile.resourcesVersion)
                .isEqualTo(resourcesVersion)
            ...
            checkTimelineEntry(tile.timeline!!.timelineEntries[0], 0, Long.MAX_VALUE, "No event")
    ...
    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun tileProviderReturnsResources() =
        fakeCoroutineScope.runTest {
            val tileRequest = RequestBuilders.TileRequest.Builder().build()
            val tileFuture = tileServiceClient.requestTile(tileRequest)
            shadowOf(Looper.getMainLooper()).idle()
            ...
            val resourcesRequest =
                RequestBuilders.ResourcesRequest.Builder().setVersion(tile.resourcesVersion).build()
            ...
            assertThat(resources.idToImageMapping.containsKey("android_" + R.drawable.oval))
                .isTrue()
        }

    @Test
    @Suppress("deprecation") // For backwards compatibility.
    fun tileProviderReturnsTimelineResources() =
        fakeCoroutineScope.runTest {
            val tileRequest = RequestBuilders.TileRequest.Builder().build()
            val tileFuture = tileServiceClientWithTimeline.requestTile(tileRequest)
            shadowOf(Looper.getMainLooper()).idle()
            ...
            assertThat(resources.idToImageMapping.containsKey("android_" + ovalBitmapHashCode))
                .isTrue()
            assertThat(
                    resources.idToImageMapping.containsKey(
                        "android_" + R.drawable.ic_launcher_background
                    )
                )
                .isTrue()
    ...
    fun tileProviderReturnsTileWithState() = runBlocking {
        tileServiceWithState.updateTileState<Preferences>() { prefs ->
            prefs
                ...
                .apply { this[tileServiceWithState.prefsNameKey] = "AndroidX" }
                .toPreferences()
        }
