...
class RemoteViewsTranslatorKtTest {
    ...
    @Test
    fun canTranslateAndroidRemoteViews_Container_BadSetupShouldFail() = fakeCoroutineScope.runTest {
        assertFailsWith<IllegalStateException> {
            context.runAndTranslate {
                val providedViews = RemoteViews(context.packageName, R.layout.raw_container)
    ...
    fun canTranslateBackground_red() = fakeCoroutineScope.runTest {
        val rv = context.runAndTranslate { Box(modifier = GlanceModifier.background(Color.Red)) {} }
    ...
    fun multipleClickable_shouldLogWarning() = fakeCoroutineScope.runTest {
        context.runAndTranslate {
            Text(
                "text1",
                modifier =
                    GlanceModifier.clickable(actionStartActivity(ComponentName("package", "class")))
    ...
    fun canTranslateColumnSelectableGroupToVerticalRadioGroup() = fakeCoroutineScope.runTest {
        val rv = context.runAndTranslate { Column(modifier = GlanceModifier.selectableGroup()) {} }
