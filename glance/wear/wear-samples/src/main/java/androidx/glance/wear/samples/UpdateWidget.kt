...
@AssociateWithGlanceWearWidget(UpdateWidget::class)
class UpdateWidgetService : GlanceWearWidgetService() {
    override val widget: GlanceWearWidget = UpdateWidget()
}

class UpdateWidget : GlanceWearWidget() {
    override suspend fun provideWidgetData(
        ...
        return WearWidgetDocument(WearWidgetBrush.color(backgroundColor.rc)) {
            UpdateWidgetContent()
        }
    }
}
...
fun UpdateWidgetContent() {
    RemoteBox(modifier = RemoteModifier.fillMaxSize(), contentAlignment = RemoteAlignment.Center) {
        RemoteColumn(
            ...
            val (dateStr, timeStr) = Instant.now().toString().split("T")
            RemoteText(text = "Last Updated:".rs)
            RemoteText(text = dateStr.rs)
            RemoteText(text = timeStr.rs)
        }
    }
}
...
) = WearWidgetPreview(UpdateWidget(), params = params)
